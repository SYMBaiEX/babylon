...
async function getStatus() {
  try {
    ...
    if (!data.game) {
      console.log(
        '⚠️  No game found. Use POST /api/game/control to create and start a game.'
      );
      console.log('\nTo start via API:');
    ...
    if (game.startedAt) {
      console.log(
        `   Started At: ${new Date(game.startedAt).toLocaleString()}`
      );
    }
...
async function controlGame(action: 'start' | 'pause') {
  if (!ADMIN_TOKEN) {
    console.error(
      '❌ ADMIN_TOKEN environment variable is required for start/pause operations'
    );
    console.error(
      '   Set ADMIN_TOKEN in your environment or pass it as an env var'
    );
    process.exit(1);
  }
