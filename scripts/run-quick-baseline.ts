...
async function main() {
  ...
  console.log(`Using agent: ${agent.displayName} (${agent.id.substring(0, 12)}...)\n`);
  ...
  console.log(`   Avg per tick: ${(parseFloat(duration) / result.ticksProcessed).toFixed(2)}s\n`);
  
  console.log('💰 Trading Performance:');
  console.log(`   Total P&L: $${result.metrics.totalPnl.toFixed(2)}`);
  console.log(`   Prediction Trades: ${result.metrics.predictionMetrics.totalPositions}`);
  console.log(`   Correct Predictions: ${result.metrics.predictionMetrics.correctPredictions}`);
  console.log(`   Incorrect Predictions: ${result.metrics.predictionMetrics.incorrectPredictions}`);
  console.log(`   Prediction Accuracy: ${(result.metrics.predictionMetrics.accuracy * 100).toFixed(1)}%`);
  console.log(`   Avg P&L per Position: $${result.metrics.predictionMetrics.avgPnlPerPosition.toFixed(2)}\n`);
  
  console.log('📈 Perp Trading:');
  console.log(`   Total Trades: ${result.metrics.perpMetrics.totalTrades}`);
  console.log(`   Profitable Trades: ${result.metrics.perpMetrics.profitableTrades}`);
  console.log(`   Win Rate: ${(result.metrics.perpMetrics.winRate * 100).toFixed(1)}%`);
  console.log(`   Avg P&L per Trade: $${result.metrics.perpMetrics.avgPnlPerTrade.toFixed(2)}\n`);
  
  console.log('🎯 Overall:');
  console.log(`   Optimality Score: ${result.metrics.optimalityScore.toFixed(1)}%`);
  console.log(`   Avg Response Time: ${result.metrics.timing.avgResponseTime.toFixed(2)}ms\n`);
  ...
  if (result.metrics.predictionMetrics.totalPositions === 0 && result.metrics.perpMetrics.totalTrades === 0) {
    issues.push('❌ No trades executed!');
  }
  
  if (result.metrics.predictionMetrics.totalPositions > 0 && result.metrics.predictionMetrics.accuracy === 0) {
