...
async function main() {
  ...
  const benchmarkPath = path.join(process.cwd(), 'benchmarks/benchmark-week-5-10-3-2-5-99999.json');
  ...
  if (ticks.every(t => t === ticks[0])) {
    ...
    issues.push(`❌ Tick sequence wrong! Expected [0,1,2,3,4,5], got [${ticks.join(',')}]`);
  } else {
  ...
  if (!engine2.isComplete()) {
    ...
    issues.push(`❌ Completion at wrong tick: Expected ${totalTicks}, got ${advanceCount}`);
  } else {
  ...
  console.log(`Prediction positions: ${result3.metrics.predictionMetrics.totalPositions}`);
  
  if (actionResults.length !== result3.actions.length) {
    issues.push(`❌ Action recording broken! Performed ${actionResults.length}, recorded ${result3.actions.length}`);
  } else {
  ...
  const markets0 = await a2a.sendRequest('a2a.getPredictions', {}) as { predictions: Array<{id: string}> };
  engine4.advanceTick();
  engine4.advanceTick();
  const markets2 = await a2a.sendRequest('a2a.getPredictions', {}) as { predictions: Array<{id: string}> };
  ...
  if (market0) {
    ...
    console.log(`Correct bet: Accuracy ${(correctAcc * 100).toFixed(0)}%, P&L $${correctPnl.toFixed(0)}`);
    ...
    console.log(`Incorrect bet: Accuracy ${(incorrectAcc * 100).toFixed(0)}%, P&L $${incorrectPnl.toFixed(0)}`);
    
    if (correctAcc !== 1.0) {
      issues.push(`❌ Correct bet should have 100% accuracy, got ${(correctAcc * 100).toFixed(0)}%`);
    } else {
      console.log('✅ Correct bets get 100% accuracy');
    }
    
    if (incorrectAcc !== 0.0) {
      issues.push(`❌ Incorrect bet should have 0% accuracy, got ${(incorrectAcc * 100).toFixed(0)}%`);
    } else {
      console.log('✅ Incorrect bets get 0% accuracy');
    }
    
    if (correctPnl <= 0) {
      issues.push(`❌ Correct bet should be profitable, got $${correctPnl.toFixed(2)}`);
    } else {
      console.log(`✅ Correct bets are profitable (+$${correctPnl.toFixed(2)})`);
    }
    
    if (incorrectPnl >= 0) {
      issues.push(`❌ Incorrect bet should lose money, got $${incorrectPnl.toFixed(2)}`);
    } else {
  ...
  try {
    const resultsPath = path.join(process.cwd(), 'benchmarks/model-comparison/qwen32b/result.json');
    ...
    if (!sampleAction) {
      ...
      console.log(`  Sample action has correctness: ${!!sampleAction.correctness}`);
    ...
    const predictionActions = savedResults.actions.filter((a: {type: string}) => a.type === 'buy_prediction').length;
    const perpActions = savedResults.actions.filter((a: {type: string}) => a.type === 'open_perp').length;
    
    if (predictionActions !== savedResults.metrics.predictionMetrics.totalPositions) {
      issues.push(`❌ Prediction count mismatch: ${predictionActions} actions vs ${savedResults.metrics.predictionMetrics.totalPositions} metric`);
    } else {
      console.log(`✅ Prediction metrics match action count (${predictionActions})`);
    }
    
    if (perpActions !== savedResults.metrics.perpMetrics.totalTrades) {
      issues.push(`❌ Perp count mismatch: ${perpActions} actions vs ${savedResults.metrics.perpMetrics.totalTrades} metric`);
    } else {
