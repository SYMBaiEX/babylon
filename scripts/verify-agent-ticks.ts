...
async function checkCronConfig(): Promise<boolean> {
  try {
    ...
    if (agentTickCron) {
      logger.info('Agent tick cron configured', { schedule: agentTickCron.schedule }, 'AgentTickVerification')
      console.log(`✅ Agent tick cron configured: ${agentTickCron.schedule}`)
      return true
    } else {
      logger.warn('Agent tick cron NOT configured in vercel.json', undefined, 'AgentTickVerification')
    ...
    console.log(`⚠️  Could not read vercel.json: ${error instanceof Error ? error.message : String(error)}`)
    return false
  }
}
...
async function getAgentStatuses(): Promise<AgentStatus[]> {
  ...
  for (const agent of agents) {
    ...
    if (agent.agentPointsBalance < 1) {
      ...
      const minutesSinceLastTick = (Date.now() - agent.agentLastTickAt.getTime()) / (1000 * 60)
...
async function main(): Promise<void> {
  ...
  logger.info('Agent status check complete', { agentCount: statuses.length }, 'AgentTickVerification')

  if (statuses.length === 0) {
    logger.warn('No agents found with autonomous features enabled', undefined, 'AgentTickVerification')
  ...
  if (!cronConfigured) {
    logger.error('CRITICAL: Agent tick cron is NOT configured', {
    ...
    console.log('   Add to vercel.json: {"path": "/api/cron/agent-tick", "schedule": "*/5 * * * *"}')
    console.log('')
  }
  ...
  if (activeAgents.length === 0 && statuses.length > 0) {
    logger.warn('No active agents found', {
      ...
      possibleReasons: ['Cron job not running', 'Agents ran out of points', 'Agents not configured correctly']
    }, 'AgentTickVerification')
    ...
    logger.info('Active agents found', { count: activeAgents.length }, 'AgentTickVerification')
    console.log(`✅ GOOD: ${activeAgents.length} agent(s) are active`)
    console.log('')
  }

  if (inactiveAgents.length > 0) {
    logger.warn('Inactive agents detected', {
      count: inactiveAgents.length,
      agentIds: inactiveAgents.map(a => a.id)
    }, 'AgentTickVerification')
    console.log(`⚠️  WARNING: ${inactiveAgents.length} agent(s) appear inactive`)
    console.log('   Check agentLastTickAt timestamps - should update every 5 minutes')
    console.log('')
  }

  if (noPointsAgents.length > 0) {
    logger.warn('Agents with no points detected', {
...
main().catch((error) => {
  logger.error('Agent tick verification script failed', error, 'AgentTickVerification')
