...
const rlTrainingVars: EnvVar[] = [
  ...
  },
  {
    ...
    required: false,
    description: 'W&B model for inference (CRITICAL: Only this model available in ART catalog)'
  },
  {
    key: 'BASE_MODEL',
    value: 'OpenPipe/Qwen3-14B-Instruct',
...
function main() {
  ...
  // Show summary
  console.log('📋 Summary:');
  console.log(`   Required vars: ${requiredVars.length - missingRequired.length}/${requiredVars.length}`);
  console.log(`   RL training vars: ${rlTrainingVars.length - missingRL.length}/${rlTrainingVars.length}`);
  console.log('');
  
  if (missingRequired.length === 0 && missingRL.length === 0) {
    console.log('🎉 All environment variables are configured!');
  } else {
    console.log('⚠️  Please update the missing variables with your actual values.');
  }
}
