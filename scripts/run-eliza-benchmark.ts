...
async function runElizaBenchmark(
  config: ElizaBenchmarkConfig
  ...
  while (!engine.isComplete()) {
    ...
    await coordinator
      .executeAutonomousTick(config.agentUserId, runtime)
      .then((tickResult) => {
...
async function main() {
  ...
  const tickInterval = Number.parseInt(
    args.find((a) => a.startsWith('--tick-interval='))?.split('=')[1] || '5000'
  );
  ...
  const markets = Number.parseInt(
    args.find((a) => a.startsWith('--markets='))?.split('=')[1] || '5'
  );
  const perpetuals = Number.parseInt(
    args.find((a) => a.startsWith('--perpetuals='))?.split('=')[1] || '3'
  );
  const agents = Number.parseInt(
    args.find((a) => a.startsWith('--agents='))?.split('=')[1] || '8'
  );
  const seed =
    Number.parseInt(
  ...
  if (!agentId) {
    ...
    console.log('\nUsage:');
    console.log(
      '  npx ts-node scripts/run-eliza-benchmark.ts --agent-id=user-123 [options]'
    );
    console.log('\nOptions:');
    console.log('  --benchmark=path/to/benchmark.json  Use existing benchmark');
    console.log(
      '  --runs=5                             Number of runs (default: 1)'
    );
    console.log('  --output=path/to/results             Output directory');
    console.log(
      '  --tick-interval=5000                 Ms between autonomous ticks (default: 5000)'
    );
    ...
    console.log(
      '  --markets=5                          Number of prediction markets'
    );
    console.log(
      '  --perpetuals=3                       Number of perpetual markets'
    );
    console.log(
      '  --agents=8                           Number of other agents'
    );
    console.log(
  ...
  if (numRuns === 1) {
    ...
    console.log(
      `  Avg Accuracy: ${(comparison.comparison.avgAccuracy * 100).toFixed(1)}%`
    );
    console.log(
      `  Avg Optimality: ${comparison.comparison.avgOptimality.toFixed(1)}%`
    );
    console.log(`  Best Run: ${comparison.comparison.bestRun}`);
    console.log(`  Worst Run: ${comparison.comparison.worstRun}`);
    console.log(`\nReports saved to: ${outputDir}`);
    console.log(
      `View comparison: file://${path.join(outputDir, 'comparison.html')}`
    );
  }
