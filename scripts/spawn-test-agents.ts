...
async function simulateAgent(
  ...
  console.log(
    `Starting agent ${agentNum} (${strategy.name}) for window ${windowId}`
  );
  ...
  while ((Date.now() - startTime) / (1000 * 60) < durationMinutes) {
    ...
    const ticker =
      strategy.preferredAssets[
        Math.floor(Math.random() * strategy.preferredAssets.length)
      ]!;
  ...
  console.log(
    `Agent ${agentNum} completed: P&L=$${pnl.toFixed(2)}, trades=${tradesExecuted}`
  );
 ...
 */
async function spawnTestAgents(count = 5, durationMinutes = 5): Promise<void> {
  ...
  agents.forEach((a) => {
    console.log(
      `  Agent ${a.num}: ${a.strategy.name} (risk: ${a.strategy.riskTolerance})`
    );
  });
  console.log();
  ...
  const results = await Promise.all(
    agents.map((a) =>
      simulateAgent(a.num, a.strategy, windowId, durationMinutes)
    )
  );
...
async function main() {
  ...
  const numAgents = Number.parseInt(
    args.find((a) => a.startsWith('--agents='))?.split('=')[1] || '5'
  );
