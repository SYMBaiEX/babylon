...
async function main() {
  ...
  try {
    ...
    if (result.failed === 0) {
      logger.info(
        '🎉 All NPCs bootstrapped successfully!',
        undefined,
        'Bootstrap'
      );
    } else {
    ...
    logger.info(
      '1. NPCs are now registered in the AgentRegistry',
      undefined,
      'Bootstrap'
    );
    logger.info(
      '2. Runtime instances are created and cached',
      undefined,
      'Bootstrap'
    );
    logger.info(
      '3. NPCs are ready for autonomous actions',
      undefined,
      'Bootstrap'
    );
    logger.info(
      '4. Check status with: bun run scripts/status.ts',
      undefined,
      'Bootstrap'
    );
