...
async function testAgentA2AServer() {
  ...
  try {
    const cardResponse = await fetch(
      `${BASE_URL}/api/agents/${agentId}/.well-known/agent-card`
    );
    ...
    console.log(
      `⚠️  Error testing agent card: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    console.log(
      `⚠️  Error testing A2A server: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    console.log(
      `⚠️  Error testing A2A GET: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    console.log(
      `⚠️  Error testing A2A POST: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    const messageResponse = await fetch(
      `${BASE_URL}/api/agents/${agentId}/a2a`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.log(
      `⚠️  Error testing message/send: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    console.log(
      `⚠️  Error verifying a2aEnabled: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  console.log(
    `   Agent Card URL: ${BASE_URL}/api/agents/${agentId}/.well-known/agent-card`
  );
