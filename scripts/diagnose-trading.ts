...
async function main() {
  ...
  if (!game) {
    logger.error(
      '❌ No continuous game found in database',
      undefined,
      'Diagnostic'
    );
  } else {
    ...
    if (!game.isRunning) {
      logger.warn(
        '⚠️  Game is NOT running - trades will not be generated',
        undefined,
        'Diagnostic'
      );
    } else {
  ...
  if (tradingNPCs.length === 0) {
    logger.error(
      '❌ No NPCs found - no trades will be generated',
      undefined,
      'Diagnostic'
    );
    logger.info(
      'To fix: Create NPC actors in database',
      undefined,
      'Diagnostic'
    );
  } else {
    logger.info(
      `✅ ${tradingNPCs.length} NPCs available`,
      undefined,
      'Diagnostic'
    );
    logger.info(
      'Sample NPCs:',
      tradingNPCs.slice(0, 5).map((n) => ({
        name: n.displayName,
      })),
      'Diagnostic'
    );
  ...
  logger.info(
    'Total NPC trades in database: Checking...',
    undefined,
    'Diagnostic'
  );
  ...
  if (totalTrades === 0) {
    ...
    logger.info(
      `✅ ${totalTrades} NPC trades in database`,
      undefined,
      'Diagnostic'
    );
    logger.info(
  ...
  if (activeQuestions.length === 0) {
    logger.warn(
      '⚠️  No active questions - NPCs have nothing to trade on',
      undefined,
      'Diagnostic'
    );
  } else {
  ...
  if (issues.length === 0) {
    logger.info(
      '✅ Everything looks good - trades should be appearing',
      undefined,
      'Diagnostic'
    );
