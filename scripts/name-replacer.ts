...
export class NameReplacer {
  ...
  private buildPatterns(): void {
    ...
    for (const actor of this.actorsData.actors) {
      ...
      this.addPattern(
        fullOriginal,
        fullReplacement,
        `Full name: ${fullOriginal}`
      );
      this.addPattern(
        fullWithSpaceOriginal,
        fullWithSpaceReplacement,
        `Full name with space: ${fullWithSpaceOriginal}`
      );
    ...
    for (const org of this.actorsData.organizations) {
      ...
      if (originalNoSpace !== originalName) {
        this.addPattern(
          originalNoSpace,
          replacementNoSpace,
          `Org name no space: ${originalNoSpace}`
        );
      }
      ...
      if (originalHandle) {
        this.addPattern(
          `@${originalHandle}`,
          `@${org.id}`,
          `Org handle: @${originalHandle}`
        );
        this.addPattern(
          originalHandle,
          org.id,
          `Org handle without @: ${originalHandle}`
        );
      }
    }
  ...
  private addPattern(
    ...
    const cases = [
      ...
      {
        pattern: escaped.toLowerCase(),
        replacement: replacement.toLowerCase(),
      }, // lowercase
      {
        pattern: escaped.toUpperCase(),
...
if (require.main === module) {
  ...
  if (args[0] === '--test') {
    ...
    if (fs.statSync(targetPath).isDirectory()) {
      ...
      if (result.changed) {
        console.log(
          `✓ File modified: ${path.relative(process.cwd(), targetPath)}`
        );
      } else {
