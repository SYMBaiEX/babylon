...
async function generateMockData() {
    ...
    steps: [
      {
        ...
        action: { type: 'BUY_SHARES', parameters: { amount: 100 }, success: true },
        reward: 50,
      },
    ],
    totalReward: 50 * (i + 1),
  ...
  const benchmarks = [
    { benchmarkId: 'bench-1', modelId: 'llama8b', month: '2025-11', metrics: { totalPnl: 1500, accuracy: 1.0 } },
    { benchmarkId: 'bench-2', modelId: 'qwen', month: '2025-11', metrics: { totalPnl: 1500, accuracy: 1.0 } },
...
async function testOfflineSimulation(outputDir: string) {
  ...
  console.log('Run when database available: npx tsx scripts/run-offline-simulation.ts --month=2025-11\n');
}
