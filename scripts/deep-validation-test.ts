...
async function main() {
  ...
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  console.log(
    'Testing that simulation ACTUALLY works, not just appears to work...\n'
  );
  ...
  if (initialPerpPrice === afterPerpPrice) {
    issues.push(
      `⚠️  Perp prices don't change across ticks (may be ok for static benchmarks)`
    );
  } else {
    console.log(
      `✅ Perp prices evolve ($${Math.abs((afterPerpPrice || 0) - (initialPerpPrice || 0)).toFixed(2)} change)`
    );
  }
  ...
  console.log(
    `Prediction positions: ${result.metrics.predictionMetrics.totalPositions}`
  );
  console.log(`Perp positions: ${result.metrics.perpMetrics.totalTrades}`);

  if (result.actions.length === 0) {
    issues.push(
      '❌ CRITICAL: No actions recorded despite performing 3 actions!'
    );
  } else if (result.actions.length !== 3) {
    issues.push(
      `❌ Action count mismatch: Expected 3, got ${result.actions.length}`
    );
  } else {
  ...
  if (result.metrics.predictionMetrics.totalPositions !== 2) {
    issues.push(
      `❌ Prediction position count wrong: Expected 2, got ${result.metrics.predictionMetrics.totalPositions}`
    );
  } else {
  ...
  if (result.metrics.perpMetrics.totalTrades !== 1) {
    issues.push(
      `❌ Perp trade count wrong: Expected 1, got ${result.metrics.perpMetrics.totalTrades}`
    );
  } else {
  ...
  const predictionPnl =
    result.metrics.predictionMetrics.avgPnlPerPosition *
    result.metrics.predictionMetrics.totalPositions;
  const perpPnl =
    result.metrics.perpMetrics.avgPnlPerTrade *
    result.metrics.perpMetrics.totalTrades;
  ...
  const predictionActions = result.actions.filter(
  ...
  if (predictionActions.length > 0) {
    ...
    if (!firstAction.correctness) {
      ...
      console.log(
        `Correctness tracked: ${JSON.stringify(firstAction.correctness)}`
      );

      // Verify it matches ground truth
      const groundTruth = snapshot.groundTruth.marketOutcomes[marketId];
      const expectedCorrect =
        (outcome === 'YES' && groundTruth) ||
        (outcome === 'NO' && !groundTruth);

      if (firstAction.correctness.predictionCorrect !== expectedCorrect) {
        issues.push(
          `❌ Ground truth not used correctly! Expected ${expectedCorrect}, got ${firstAction.correctness.predictionCorrect}`
        );
      } else {
        console.log(
          `✅ Ground truth used correctly (expected=${expectedCorrect}, actual=${firstAction.correctness.predictionCorrect})`
        );
      }
