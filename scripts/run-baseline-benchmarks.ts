...
async function runBaselineBenchmark(
  ...
  // Idempotent check
  if (
    !config.forceRegenerate &&
    (await fs
      .access(baselineOutputPath)
      .then(() => true)
      .catch(() => false))
  ) {
    logger.info(`Baseline ${model.name} already exists, skipping`, {
      path: baselineOutputPath,
    });
    console.log(
      `  ⏭️  ${model.displayName} baseline already exists: ${baselineOutputPath}`
    );
    return;
  }
  ...
  const baselineResult = {
    ...
    metrics: {
      ...
      incorrectPredictions:
        result.metrics.predictionMetrics.incorrectPredictions,
    },
    timing: {
...
async function main() {
  ...
  if (!benchmarkPath) {
    ...
    console.log('\nOptions:');
    console.log(
      '  --output=path/to/baselines    Output directory (default: benchmarks/baselines)'
    );
    console.log(
      '  --force                       Regenerate even if baselines exist'
    );
    process.exit(1);
  }
  ...
  BASELINE_MODELS.forEach((model) => {
    console.log(
      `  - ${model.displayName}: ${path.join(outputDir, model.outputFile)}`
    );
  });
