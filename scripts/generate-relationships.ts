...
function calculateCompatibility(actor1: ActorData, actor2: ActorData): number {
  ...
  const sharedOrgs =
    actor1.affiliations?.filter((org) => actor2.affiliations?.includes(org)) ||
    [];
...
function determineRelationshipType(
  ...
  if (compatibility > 0.6) {
    return Math.random() > 0.5
      ? RELATIONSHIP_TYPES.FRIENDS
      : RELATIONSHIP_TYPES.ALLIES;
  }
  if (compatibility < 0.3) {
    return Math.random() > 0.5
      ? RELATIONSHIP_TYPES.RIVALS
      : RELATIONSHIP_TYPES.COMPETITORS;
  }
  return RELATIONSHIP_TYPES.ACQUAINTANCES;
}
...
async function main() {
  ...
  for (let i = 0; i < actors.length; i++) {
    ...
    for (const actor2 of partners) {
      ...
      if (relType === RELATIONSHIP_TYPES.MENTOR_STUDENT) {
        ...
        relType === RELATIONSHIP_TYPES.INDUSTRY_LEADER_FOLLOWER ||
        relType === RELATIONSHIP_TYPES.INFLUENCER_FAN
      ) {
        actor2FollowsActor1 = true; // Follower/fan follows leader
      } else if (
        relType === RELATIONSHIP_TYPES.ALLIES ||
        relType === RELATIONSHIP_TYPES.FRIENDS ||
        relType === RELATIONSHIP_TYPES.COLLABORATORS
      ) {
        actor1FollowsActor2 = true;
        actor2FollowsActor1 = true; // Mutual follow
      } else if (
        relType === RELATIONSHIP_TYPES.RIVALS ||
        relType === RELATIONSHIP_TYPES.COMPETITORS
      ) {
      ...
      const relationship: Relationship = {
        ...
        actor1FollowsActor2: isActor1First
          ? actor1FollowsActor2
          : actor2FollowsActor1,
        actor2FollowsActor1: isActor1First
          ? actor2FollowsActor1
          : actor1FollowsActor2,
      };
      ...
      const fileName = createRelationshipFileName(
        sortedActor1Id,
        sortedActor2Id
      );
  ...
  console.log(
    `  Avg per actor: ${((relationships.length * 2) / actors.length).toFixed(1)}`
  );
  ...
  console.log(
    'Server-side code will automatically load them via loadActorsData()'
  );
