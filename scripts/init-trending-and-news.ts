...
const SAMPLE_NEWS_ARTICLES = [
  {
    ...
    summary:
      'Venture capital firms poured billions into artificial intelligence startups, marking the highest quarterly investment in the sector.',
    category: 'Tech',
    sentiment: 'positive',
    slant: 'bullish',
    biasScore: 0.6,
  },
  {
    ...
    summary:
      'Central bank officials hint at policy adjustments as economic indicators show mixed signals across major sectors.',
    category: 'Finance',
    sentiment: 'neutral',
    slant: 'cautious',
    biasScore: 0.0,
  },
  {
    ...
    summary:
      'Government agencies announce plans for enhanced oversight of major technology companies amid growing concerns over market dominance.',
    category: 'Politics',
    sentiment: 'negative',
    slant: 'bearish',
    biasScore: -0.4,
  },
  {
    ...
    summary:
      'Digital asset prices fluctuate as investors react to new regulatory frameworks and institutional adoption announcements.',
    category: 'Finance',
    sentiment: 'neutral',
    slant: 'neutral',
    biasScore: 0.1,
  },
  {
    ...
    summary:
      'Fortune 500 companies commit billions to clean energy and sustainability initiatives, accelerating green technology development.',
    category: 'Tech',
    sentiment: 'positive',
    slant: 'optimistic',
    biasScore: 0.5,
  },
  {
    ...
    summary:
      'Employment data reveals persistent worker shortages in key industries despite broader economic uncertainty and sector-specific slowdowns.',
    category: 'Finance',
    sentiment: 'neutral',
    slant: 'mixed',
    biasScore: 0.0,
  },
  {
    ...
    summary:
      'Private space companies successfully complete record number of launches, advancing commercial space flight capabilities significantly.',
    category: 'Tech',
    sentiment: 'positive',
    slant: 'bullish',
    biasScore: 0.7,
  },
  {
    ...
    summary:
      'Retail analysts document major changes in shopping behaviors as consumers adapt to new economic realities and digital alternatives.',
    category: 'Finance',
    sentiment: 'neutral',
    slant: 'analytical',
    biasScore: 0.0,
  },
  {
    ...
    summary:
      'Researchers announce significant progress in gene therapy and personalized medicine, potentially revolutionizing treatment approaches.',
    category: 'Tech',
    sentiment: 'positive',
    slant: 'optimistic',
    biasScore: 0.6,
  },
  {
    ...
    summary:
      'Major corporations reorganize logistics networks and manufacturing locations in response to geopolitical tensions and trade policies.',
    category: 'Finance',
    sentiment: 'neutral',
    slant: 'cautious',
    biasScore: -0.1,
  },
  {
    ...
    summary:
      'Schools and universities expand digital learning platforms as remote and hybrid education models become permanent fixtures.',
    category: 'Tech',
    sentiment: 'positive',
    slant: 'progressive',
    biasScore: 0.4,
  },
  {
    ...
    summary:
      'Traditional energy companies navigate transition to renewable sources while maintaining grid reliability and profitability.',
    category: 'Finance',
    sentiment: 'neutral',
    slant: 'balanced',
    biasScore: 0.0,
...
async function initializeTrending() {
  ...
  if (existingTrending >= MIN_TRENDING_TAGS) {
    logger.info(
      `✅ Trending already populated (${existingTrending} tags)`,
      undefined,
      'InitTrendingNews'
    );
    return;
  ...
  if (recentPosts.length > 0) {
    logger.info(
      `Linking ${recentPosts.length} posts to tags...`,
      undefined,
      'InitTrendingNews'
    );
    ...
    for (const post of recentPosts) {
      ...
      if (content.includes('ai') || content.includes('artificial'))
        matchingTags.push('ai');
      if (content.includes('market') || content.includes('stock'))
        matchingTags.push('markets');
      if (content.includes('crypto') || content.includes('bitcoin'))
        matchingTags.push('crypto');
      if (content.includes('politic') || content.includes('election'))
        matchingTags.push('politics');
      ...
      if (content.includes('breaking') || content.includes('news'))
        matchingTags.push('breaking');
      if (content.includes('startup') || content.includes('founder'))
        matchingTags.push('startup');
      if (content.includes('invest') || content.includes('trade'))
        matchingTags.push('investing');
      if (content.includes('space') || content.includes('spacex'))
        matchingTags.push('spacex');
      if (content.includes('climate') || content.includes('green'))
        matchingTags.push('climate');
    ...
    logger.info(
      `Created ${linksCreated} post-tag links`,
      undefined,
      'InitTrendingNews'
    );
  }
  ...
  if (trendingCreated < MIN_TRENDING_TAGS) {
    logger.info(
      'Creating sample trending entries...',
      undefined,
      'InitTrendingNews'
    );

    for (
      let i = trendingCreated;
      i < MIN_TRENDING_TAGS && i < createdTags.length;
      i++
    ) {
  ...
  logger.info(
    `✅ Created ${trendingCreated} trending entries`,
    undefined,
    'InitTrendingNews'
  );
}
...
async function initializeNews() {
  ...
  if (existingArticles >= MIN_NEWS_ARTICLES) {
    logger.info(
      `✅ News already populated (${existingArticles} articles)`,
      undefined,
      'InitTrendingNews'
    );
    return;
  ...
  if (newsOrgs.length === 0) {
    logger.warn(
      '⚠️  No news organizations found. Skipping news creation.',
      undefined,
      'InitTrendingNews'
    );
    logger.info(
      '💡 News will be generated automatically by game ticks',
      undefined,
      'InitTrendingNews'
    );
    return;
  ...
  logger.info(
    `✅ Created ${articlesCreated} news articles`,
    undefined,
    'InitTrendingNews'
  );
}
...
async function main() {
  logger.info(
    '🚀 Initializing Trending and News...',
    undefined,
    'InitTrendingNews'
  );
  ...
  if (trendingCount >= MIN_TRENDING_TAGS && newsCount >= MIN_NEWS_ARTICLES) {
    logger.info(
      '✅ System ready! Trending and news are populated.',
      undefined,
      'InitTrendingNews'
    );
  } else {
    logger.warn(
      '⚠️  Minimum thresholds not met:',
      undefined,
      'InitTrendingNews'
    );
    if (trendingCount < MIN_TRENDING_TAGS) {
      logger.warn(
        `  Trending: ${trendingCount}/${MIN_TRENDING_TAGS}`,
        undefined,
        'InitTrendingNews'
      );
    }
    if (newsCount < MIN_NEWS_ARTICLES) {
      logger.warn(
        `  News: ${newsCount}/${MIN_NEWS_ARTICLES}`,
        undefined,
        'InitTrendingNews'
      );
    }
    logger.info(
      '💡 More content will be generated by game ticks',
      undefined,
      'InitTrendingNews'
    );
  }
