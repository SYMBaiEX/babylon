...
async function registerBabylonOnAgent0() {
  ...
  if (!process.env.PINATA_JWT) {
    throw new Error(
      'PINATA_JWT not set in environment (needed for IPFS metadata storage)'
    );
  }
  ...
  if (configValue?.agentId) {
    ...
    try {
      ...
      if (agent) {
        ...
        const agentWithA2a = agent as typeof agent & {
          a2aEndpoint?: string;
          a2aSkills?: string[];
        };
  ...
  try {
    ...
    console.log(
      `   Updated At: ${new Date(registration.updatedAt * 1000).toISOString()}`
    );
    ...
    try {
      ...
      if (agent) {
        ...
        if (agent.a2aSkills && agent.a2aSkills.length > 0) {
          ...
          agent.a2aSkills.forEach((skill: string) =>
            console.log(`     - ${skill}`)
          );
        }
      } else {
    ...
    console.log(
      `   4. View on explorer: https://testnet.agent0.network/agents/${registration.agentId}`
    );
  } catch (error) {
