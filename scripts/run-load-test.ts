...
async function main() {
  ...
  for (const [endpoint, stats] of sortedEndpoints) {
    ...
    console.log(
      `    Requests: ${stats.count.toLocaleString()} (${stats.successCount} successful)`
    );
    console.log(
      `    Avg Time: ${stats.avgResponseTime.toFixed(2)}ms (successful only)`
    );
  ...
  console.log(
    `  Total Queries:     ${queryStats.totalQueries.toLocaleString()}`
  );
  console.log(
