...
class WandbModelUsageVerifier {
  ...
  async verifyTrainedModelExists(): Promise<VerificationResult> {
    ...
    try {
      ...
      console.log(
        `     Trained At: ${latestModel.metadata.trainedAt.toISOString()}\n`
      );

      // Verify model path format (should be W&B format)
      const isWandbFormat =
        latestModel.modelPath.includes('/') ||
        latestModel.modelPath.includes(':');
   ...
   */
  async verifyAgentRuntimeLoadsModel(
    agentId: string
    ...
    try {
      ...
      const modelVersion = (runtime as ExtendedAgentRuntime)
        .currentModelVersion;
   ...
   */
  async verifyGroqPluginUsesWandb(
    agentId: string
  ): Promise<VerificationResult> {
  ...
  async verifyActualLLMCall(agentId: string): Promise<VerificationResult> {
    ...
    try {
      ...
      // Make a test call
      const testPrompt =
        'Say "W&B RL Model Test" if you are a trained reinforcement learning model.';

      try {
        const response = await groqPlugin.models?.['text_large']?.(runtime, {
          prompt: testPrompt,
          maxTokens: 50,
          temperature: 0.7,
        });
        ...
        if (recentCalls.length === 0) {
          return {
            ...
            passed: false,
            details:
              'LLM call succeeded but no log entry found. Trajectory logging may not be enabled.',
  ...
  async verifyModelUsageStats(agentId: string): Promise<VerificationResult> {
    ...
    try {
      ...
      console.log(
        `     Is Trained Model: ${stats.isTrainedModel ? '✅ YES' : '❌ NO'}`
      );
  ...
  async run(agentId?: string): Promise<void> {
    ...
    if (!step1.passed) {
      ...
      await db.$disconnect();
      process.exit(
        step1.details.includes('Database connection failed') ? 1 : 0
      );
      return;
    }
  ...
  private printSummary(): void {
    ...
    if (passed === total) {
      console.log(
        '🎉 100% CONFIDENCE: Agents are using trained W&B RL model!\n'
      );
