...
async function testTrajectoryExtraction() {
  ...
  for (const traj of trajectories.slice(0, 2)) {
    ...
    try {
      ...
      console.log(`  ✅ Metadata JSON: ${Object.keys(metadata).length} metadata fields`);
      
      // Check step structure
      if (steps.length > 0) {
        const firstStep = steps[0];
        console.log(`  ✅ Step structure:`);
        console.log(`     - Has environmentState: ${!!firstStep.environmentState}`);
        console.log(`     - Has action: ${!!firstStep.action}`);
        console.log(`     - Has reward: ${typeof firstStep.reward === 'number'}`);
        console.log(`     - Has LLM calls: ${Array.isArray(firstStep.llmCalls)}`);
      }
    } catch (error) {
      console.log(`  ❌ Failed to parse JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  let exportResult: { success: boolean; trajectoriesExported: number; error?: string } | null = null;
  ...
  try {
    ...
    console.log(`❌ Export error: ${error instanceof Error ? error.message : String(error)}`);
    console.log(`   This may be expected if export directory cannot be created`);
  ...
  for (const traj of completeTrajectories) {
    try {
      ...
      if (steps.length > 0) {
        const step = steps[0];
        if (step.llmCalls && Array.isArray(step.llmCalls) && step.llmCalls.length > 0) {
          hasLLMCalls++;
        }
        if (step.action) {
          hasActions++;
  ...
  console.log(`  Trajectories with LLM calls: ${hasLLMCalls}/${completeTrajectories.length}`);
  console.log(`  Trajectories with actions: ${hasActions}/${completeTrajectories.length}`);
  console.log(`  Trajectories with rewards: ${hasRewards}/${completeTrajectories.length}`);
  ...
  console.log(`✅ Export function works: ${exportResult?.success ? 'YES' : 'NO'}`);
  console.log('═'.repeat(60) + '\n');
  
  console.log('✅ VERIFICATION COMPLETE');
  console.log('   Trajectories from ElizaOS agents are collectable and extractable!\n');
