...
async function findRouteFiles(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const hasExport =
        /export\s+(async\s+)?function\s+(GET|POST|PUT|PATCH|DELETE)|export\s+const\s+(GET|POST|PUT|PATCH|DELETE)/.test(
          content
        );
...
async function main() {
  ...
  console.log(
    `📝 Routes with OpenAPI path definitions: ${routesWithPaths.length}`
  );
  ...
  const routesNotInSpec = routes.filter((r) => {
    ...
    return !documentedPaths.some(
      (dp) =>
        dp === apiPath ||
        dp.replace(/\{[^}]+\}/g, '{}') === apiPath.replace(/\{[^}]+\}/g, '{}')
    );
  });
  ...
  console.log(
    `Routes with @openapi tags: ${routes.filter((r) => r.hasOpenapi).length}`
  );
  console.log(
    `Routes with exports: ${routes.filter((r) => r.hasExport).length}`
  );
