...
async function main() {
  ...
  if (result.performanceMetrics) {
    ...
    console.log(
      `  Operations:        ${pm.cache.operations.get + pm.cache.operations.set + pm.cache.operations.delete}`
    );

    console.log('\nDatabase:');
    console.log(
      `  Slow Query Rate:   ${(pm.database.slowQueryRate * 100).toFixed(2)}%`
    );
    ...
    if (pm.database.slowestOperations.length > 0) {
      ...
      pm.database.slowestOperations.slice(0, 5).forEach((op, i) => {
        ...
        console.log(
          `       Avg: ${op.avgDuration.toFixed(2)}ms (${op.count} calls)`
        );
      });
    }
    ...
    console.log(
      `  Avg RPS:           ${pm.system.avgRequestsPerSecond.toFixed(2)}`
    );
  }
  ...
  const criticalIssues =
    result.bottlenecks?.filter((b) => b.severity === 'critical').length || 0;
