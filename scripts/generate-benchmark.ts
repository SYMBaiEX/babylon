...
async function main() {
  ...
  const durationMinutes = Number.parseInt(
    args.find((a) => a.startsWith('--duration'))?.split('=')[1] || '30'
  );
  const seed =
    Number.parseInt(
  ...
  console.log(
    `Duration: ${durationMinutes} minutes (${(durationMinutes / 60).toFixed(1)} hours)`
  );
  ...
  console.log(
    `Social Opportunities: ${snapshot.groundTruth.socialOpportunities.length}`
  );
