...
async function main() {
  ...
  if (!['localnet', 'testnet', 'mainnet'].includes(env)) {
    logger.error(
      'Invalid environment. Use: local, testnet, or mainnet',
      undefined,
      'Script'
    );
    process.exit(1);
  ...
  if (result.valid && result.deployed) {
    ...
    if (result.contracts.diamond) {
      logger.info(
        `  Diamond: ${result.contracts.diamond}`,
        undefined,
        'Script'
      );
      logger.info(
        `  Explorer: ${config.explorerUrl}/address/${result.contracts.diamond}`,
        undefined,
        'Script'
      );
    }
    if (result.contracts.identityRegistry) {
      logger.info(
        `  Identity Registry: ${result.contracts.identityRegistry}`,
        undefined,
        'Script'
      );
    }
    if (result.contracts.reputationSystem) {
      logger.info(
        `  Reputation System: ${result.contracts.reputationSystem}`,
        undefined,
        'Script'
      );
    }
