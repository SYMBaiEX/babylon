...
async function getStatus() {
  ...
  logger.info(
    `   Current Date: ${game.currentDate.toLocaleString()}`,
    undefined,
    'Game Control'
  );
  logger.info(
    `   Active Questions: ${game.activeQuestions}`,
    undefined,
    'Game Control'
  );
  logger.info(
    `   Speed: ${game.speed}ms between ticks`,
    undefined,
    'Game Control'
  );
  logger.info(
  ...
  if (game.startedAt) {
    logger.info(
      `   Started At: ${game.startedAt.toLocaleString()}`,
      undefined,
      'Game Control'
    );
  }
  ...
  if (game.pausedAt) {
    logger.info(
      `   Paused At: ${game.pausedAt.toLocaleString()}`,
      undefined,
      'Game Control'
    );
  }
  ...
  if (!game.isRunning) {
    logger.info(
      '💡 To start the game, run: bun run game:start',
      undefined,
      'Game Control'
    );
  } else {
    logger.info(
      '💡 To pause the game, run: bun run game:pause',
      undefined,
      'Game Control'
    );
  }
...
async function main() {
  ...
  switch (action) {
    ...
    default:
      ...
      logger.info(
        'Valid actions: start, pause, status',
        undefined,
        'Game Control'
      );
      process.exit(1);
