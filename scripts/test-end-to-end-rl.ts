...
async function testAgentRuntimeConfiguration() {
  ...
  addResult('Agent Found', true, `Testing with: ${agent.displayName || agent.username}`);
  
  // Get runtime (this triggers model loading)
  const runtime = await agentRuntimeManager.getRuntime(agent.id);
  addResult('Runtime Created', !!runtime, runtime ? 'Runtime initialized' : 'Failed to create runtime');
...
async function testAgentActions(agentInfo: { agent: { id: string; displayName: string | null }; runtime: unknown } | null) {
  console.log('\n━━━ 3. AGENT ACTIONS ━━━\n');
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════════════════╗');
  console.log('║              RL Training System - End-to-End Verification                  ║');
  console.log('╚════════════════════════════════════════════════════════════════════════════╝');
