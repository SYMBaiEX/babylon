...
console.info('Setting up localnet development environment...', undefined, 'Script')
console.info('='.repeat(60), undefined, 'Script')

// 0. Kill any processes on port 3000 to prevent port conflicts
console.info('Checking for processes on port 3000...', undefined, 'Script')
const killedCount = await killPort(3000)
if (killedCount > 0) {
  console.info(`✅ Killed ${killedCount} process(es) on port 3000`, undefined, 'Script')
} else {
  console.info('✅ Port 3000 is free', undefined, 'Script')
}
...
try {
  if (existsSync(nextLockPath)) {
    console.info('Cleaning up Next.js lock file...', undefined, 'Script')
    unlinkSync(nextLockPath)
    console.info('✅ Next.js lock file removed', undefined, 'Script')
  }
} catch (error) {
  console.warn('Could not remove Next.js lock file (may not exist)', undefined, 'Script')
}
...
await $`docker info`.quiet().catch(() => {
  console.error('❌ Docker is not running', undefined, 'Script')
  console.info('Please start Docker Desktop or Docker daemon', undefined, 'Script')
  process.exit(1)
})
console.info('✅ Docker is running', undefined, 'Script')

// 2. Check/create .env file
const envPath = join(process.cwd(), '.env')
if (!existsSync(envPath)) {
  console.info('Creating .env file...', undefined, 'Script')
...
`
  ...
  console.info('✅ .env created from template', undefined, 'Script')
}
...
console.info('Checking port 8545 for Hardhat node...', undefined, 'Script')

// Kill any process on port 8545 to ensure clean start
const killed8545 = await killPort(8545)
if (killed8545 > 0) {
  console.info(`✅ Killed ${killed8545} process(es) on port 8545`, undefined, 'Script')
  // Wait a moment for port to be fully released
  await new Promise(resolve => setTimeout(resolve, 1000))
} else {
  console.info('✅ Port 8545 is free', undefined, 'Script')
}

console.info('Note: Hardhat node will be started automatically by the dev script', undefined, 'Script')
console.info('      Contracts will be deployed once Hardhat is ready', undefined, 'Script')
...
if (postgresRunning.trim() !== POSTGRES_CONTAINER) {
  ...
  while (attempts < 30) {
    ...
    if (health.trim() === 'healthy') {
      console.info('✅ PostgreSQL is ready', undefined, 'Script')
      break
    }
  ...
  if (attempts === 30) {
    console.error('❌ PostgreSQL health check timeout', undefined, 'Script')
    process.exit(1)
  }
} else {
...
if (redisRunning.trim() !== REDIS_CONTAINER) {
  console.info('Starting Redis...', undefined, 'Script')
  await $`docker-compose up -d redis`.then(async () => {
    await new Promise(resolve => setTimeout(resolve, 2000))
    console.info('✅ Redis started', undefined, 'Script')
  }).catch(() => {
    console.warn('⚠️  Redis start failed (optional, continuing)', undefined, 'Script')
  })
} else {
...
if (minioRunning.trim() !== MINIO_CONTAINER) {
  console.info('Starting MinIO...', undefined, 'Script')
  await $`docker-compose up -d minio`.then(async () => {
    await new Promise(resolve => setTimeout(resolve, 2000))
    console.info('✅ MinIO started', undefined, 'Script')
  }).catch(() => {
    console.warn('⚠️  MinIO start failed (optional, continuing)', undefined, 'Script')
  })
} else {
  console.info('✅ MinIO is running', undefined, 'Script')
}
...
async function runMigrations(): Promise<void> {
  ...
  console.info('Running database migrations (drizzle-kit push --force)...', undefined, 'Script')
  ...
  const migrationPromise = (async () => {
    ...
    const result = await $`yes | DATABASE_URL=${LOCAL_DATABASE_URL} bunx drizzle-kit push --config packages/db/drizzle.config.ts --force`.nothrow()
    ...
    console.info('✅ Migrations completed', undefined, 'Script')
  })()
...
try {
  ...
  console.info(`✅ Database connected (${actorCount} actors)`, undefined, 'Script')
} catch (error: unknown) {
  const errorMessage = error instanceof Error ? error.message : String(error)
  if (errorMessage.includes('does not exist') || errorMessage.includes('relation')) {
    console.info('Database tables not found, running migrations...', undefined, 'Script')
...
console.info('', undefined, 'Script')
console.info('='.repeat(60), undefined, 'Script')
console.info('✅ Localnet environment ready!', undefined, 'Script')
console.info('', undefined, 'Script')
console.info('Services:', undefined, 'Script')
console.info('  Hardhat:    http://localhost:8545 (will be started automatically)', undefined, 'Script')
console.info('  PostgreSQL: localhost:5433', undefined, 'Script')
console.info('  Redis:      localhost:6380', undefined, 'Script')
console.info('  MinIO:      http://localhost:9000 (console: :9001)', undefined, 'Script')
console.info('', undefined, 'Script')
console.info('App Routes:', undefined, 'Script')
console.info('  Main:       http://localhost:3000', undefined, 'Script')
console.info('  Betting:    http://localhost:3000/betting (Oracle-powered markets)', undefined, 'Script')
console.info('', undefined, 'Script')
console.info('Starting services (Hardhat, Next.js, Cron)...', undefined, 'Script')
console.info('='.repeat(60), undefined, 'Script')