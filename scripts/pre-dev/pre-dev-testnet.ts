...
import { $ } from 'bun'
import { DeploymentEnv, printValidationResult, validateEnvironment } from '../../packages/contracts/src/deployment/env-detection'
import { validateDeployment, ValidationResult } from '../../packages/contracts/src/deployment/validation'


function printDeploymentResult(result: ValidationResult, _env: DeploymentEnv): void {
  if (!result.deployed) {
...
console.info('Setting up testnet development environment...', undefined, 'Script')
...
if (!envValidation.valid) {
  ...
  console.info('  1. Copy .env.testnet.example to .env.testnet', undefined, 'Script')
  console.info('  2. Fill in required values', undefined, 'Script')
  console.info('  3. Deploy contracts: bun run contracts:deploy:testnet', undefined, 'Script')
  process.exit(1)
}
...
import { actors, checkDatabaseHealth, closeDatabase, count, db } from '../../packages/db/src'
...
if (contractValidation.contracts.diamond) {
  ...
  console.info('  Diamond: ' + contractValidation.contracts.diamond, undefined, 'Script')
}
console.info('', undefined, 'Script')
