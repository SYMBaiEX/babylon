...
import { renderPrompt, getPromptParams } from '@/prompts';
...
async function testPromptDebugLogging() {
  ...
  const debugEnabled = process.env.DEBUG_PROMPTS === 'true' || process.env.DEBUG_PROMPTS === '1';
  logger.info(`Debug prompts enabled: ${debugEnabled}`, {}, 'TestScript');
  
  if (!debugEnabled) {
    logger.warn('⚠️  DEBUG_PROMPTS not enabled. Set DEBUG_PROMPTS=true to see debug output.', {}, 'TestScript');
  }
  ...
  const prompt = renderPrompt(questionGeneration, {
    ...
    numToGenerate: '2',
    exampleQuestions: '✅ "Will AIlon Musk tweet 100 times today?"\n✅ "Will OpenAGI announce GPT-7?"',
  });
  ...
  const questions = 'response' in response && response.response
    ? response.response.questions
    : (response as { questions: Array<{ text: string; scenario: number; daysUntilResolution: number; expectedOutcome: boolean }> }).questions;
  
  logger.info(`Generated ${questions.length} questions`, { questions }, 'TestScript');
  
  if (debugEnabled) {
    logger.info('✅ Debug log should be created in debug-prompts/ directory', {}, 'TestScript');
    logger.info('   Check debug-prompts/<timestamp>_question-generation.md', {}, 'TestScript');
  }
}
