...
async function main() {
  ...
  if (!testAgent) {
    testAgent = await db.user.create({
      data: {
        ...
        isTest: true,
        agentSystem:
          'You are a trading agent in Babylon prediction markets. Make profitable decisions.',
        agentModelTier: 'pro',
  ...
  console.log(
    '📊 Step 2: Creating test trajectories with varying performance...\n'
  );
  ...
  for (let i = 0; i < performanceLevels.length; i++) {
    ...
    console.log(
      `    ✅ Created trajectory ${trajId.substring(0, 12)}... (${level.steps} steps, P&L: $${level.pnl})\n`
    );
  }
  ...
  for (const traj of created) {
    console.log(
      `    ${traj.trajectoryId.substring(0, 12)}... - ${traj.episodeLength} steps, P&L: $${traj.finalPnL}, scored: ${traj.aiJudgeReward !== null ? '✅' : '❌'}`
    );
  }
  ...
  console.log(
    '  This will call Groq API to judge trajectories relative to each other...\n'
  );
  ...
  try {
    ...
    for (let i = 0; i < scoredTrajectories.length; i++) {
      ...
      const level = performanceLevels.find(
        (l) => Math.abs(l.pnl - (traj.finalPnL || 0)) < 50
      );
      ...
      if (traj.aiJudgeReasoning) {
        console.log(
          `     Reasoning: ${traj.aiJudgeReasoning.substring(0, 100)}...`
        );
      }
      console.log('');
    }
    ...
    if (scores.length !== trajectoryIds.length) {
      console.log(
        `  ❌ ERROR: Expected ${trajectoryIds.length} scores, got ${scores.length}\n`
      );
      process.exit(1);
    }
    ...
    if (invalidScores.length > 0) {
      console.log(
        `  ❌ ERROR: Found invalid scores: ${invalidScores.join(', ')}\n`
      );
      process.exit(1);
    }
    ...
    if (uniqueScores.size < 2) {
      console.log(
        `  ⚠️  WARNING: Scores are too similar (${uniqueScores.size} unique values)`
      );
      console.log(
        `     This might indicate the judge isn't differentiating properly.\n`
      );
    } else {
      console.log(
        `  ✅ Scores are differentiated (${uniqueScores.size} unique values)\n`
      );
    }

    // Check that higher performers got higher scores (rough correlation)
    const highPerformerScore =
      scoredTrajectories.find((t) => Math.abs((t.finalPnL || 0) - 500) < 50)
        ?.aiJudgeReward || 0;

    const lowPerformerScore =
      scoredTrajectories.find((t) => Math.abs((t.finalPnL || 0) - -500) < 50)
        ?.aiJudgeReward || 1;

    if (highPerformerScore > lowPerformerScore) {
      console.log(
        `  ✅ High performer (${highPerformerScore.toFixed(3)}) scored higher than low performer (${lowPerformerScore.toFixed(3)})\n`
      );
    } else {
      console.log(
        `  ⚠️  WARNING: Score ordering doesn't match performance expectations\n`
      );
      console.log(
        `     High: ${highPerformerScore.toFixed(3)}, Low: ${lowPerformerScore.toFixed(3)}\n`
      );
    }
