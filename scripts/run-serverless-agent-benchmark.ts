...
async function main() {
  ...
  const apiUrl =
    args.find((a) => a.startsWith('--api-url='))?.split('=')[1] ||
    'http://localhost:3000';
  const authToken =
  ...
  if (!agentId) {
    ...
    console.log('\nUsage:');
    console.log(
      '  npx tsx scripts/run-serverless-agent-benchmark.ts --agent-id=<id> [options]'
    );
    console.log('\nOptions:');
    console.log(
      '  --benchmark=path/to/benchmark.json  Benchmark file (required)'
    );
    console.log(
      '  --runs=5                            Number of runs (default: 1)'
    );
    console.log(
      '  --api-url=http://localhost:3000     API base URL (default: localhost:3000)'
    );
    console.log(
      '  --token=<jwt>                       Auth token (or set AUTH_TOKEN env var)'
    );
    console.log(
      '  --output=path/to/results            Output directory (optional)'
    );
    process.exit(1);
  }
  ...
  if (!authToken) {
    console.error(
      'Error: Authentication token required (use --token or AUTH_TOKEN env var)'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (runs === 1) {
      ...
      console.log(
        `  Prediction Accuracy: ${((result.results.predictionAccuracy || 0) * 100).toFixed(1)}%`
      );
      console.log(
        `  Perp Win Rate: ${((result.results.perpWinRate || 0) * 100).toFixed(1)}%`
      );
      console.log(
        `  Optimality Score: ${(result.results.optimalityScore || 0).toFixed(1)}%`
      );
      console.log(`  Actions Executed: ${result.results.actionsExecuted || 0}`);
      console.log(
        `  Duration: ${(result.results.duration / 1000).toFixed(1)}s`
      );
    } else {
      ...
      console.log(
        `  Avg Accuracy: ${((result.results.avgAccuracy || 0) * 100).toFixed(1)}%`
      );
      console.log(
        `  Avg Optimality: ${(result.results.avgOptimality || 0).toFixed(1)}%`
      );
      console.log(
        `  Avg Actions: ${result.results.avgActions?.toFixed(1) || 0}`
      );
      console.log(
        `  Total Duration: ${(result.results.duration / 1000).toFixed(1)}s`
      );
    }
