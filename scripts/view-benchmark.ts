...
async function main() {
  ...
  if (!filePath) {
    console.error(
      'Usage: bun run scripts/view-benchmark.ts <benchmark-file> [--verbose] [--show-hidden]'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    // Verify security
    const securityCheck =
      BenchmarkDataViewer.verifyAgentCannotAccessHiddenFacts(
        JSON.parse(
          await import('fs').then((m) => m.promises.readFile(fullPath, 'utf-8'))
        )
      );

    console.log('\n🔒 Security Check:');
    console.log(
      `  Agents can access hidden facts: ${securityCheck.canAccess ? '❌ YES (SECURITY ISSUE!)' : '✅ NO'}`
    );
