...
async function main() {
  ...
  if (!agent) {
    console.error('❌ No test agent found. Run: bun run scripts/ensure-test-agents.ts');
    process.exit(1);
  }
  
  console.log(`✅ Agent: ${agent.displayName} (${agent.id.substring(0, 12)}...)\n`);
  ...
  for (let i = 0; i < totalTicks; i++) {
    ...
    process.stdout.write(`\r   Progress: [${i + 1}/${totalTicks}] ${progress}% | Actions: ${actionsCount} | Errors: ${errorsCount}`);
    ...
    try {
      ...
      const tickResult = await Promise.race([tickPromise, timeoutPromise]) as {
        success: boolean;
  ...
  console.log(`   Avg per tick: ${(parseFloat(duration) / totalTicks).toFixed(2)}s\n`);
