...
async function main() {
  const numGames = Number.parseInt(process.argv[2] || '3', 10);
  ...
  logger.info(
    `Estimated time: ${numGames * 2}-${numGames * 3} minutes`,
    undefined,
    'Validator'
  );
  ...
  for (let i = 0; i < numGames; i++) {
    ...
    logger.info(
      `[${i + 1}/${numGames}] Complete in ${duration}s`,
      undefined,
      'Validator'
    );
  }
  ...
  logger.info(
    'Goal: Early game unclear (<60%), late game clear (>75%)',
    undefined,
    'Validator'
  );
  logger.info('─'.repeat(60), undefined, 'Validator');
  ...
  for (const game of games) {
    for (const question of game.setup.questions) {
      ...
      const pass =
        gradient > 0.15 && earlyCertainty < 0.65 && lateCertainty > 0.7;
  ...
  const gradientPassRate =
    gradientResults.filter((r) => r.pass).length / gradientResults.length;
  logger.info('', undefined, 'Validator');
  logger.info(
    `Result: ${(gradientPassRate * 100).toFixed(0)}% of questions have information gradient`,
    undefined,
    'Validator'
  );
  logger.info(
    gradientPassRate > 0.7 ? '✅ PASS' : '❌ FAIL (need 70%+)',
    undefined,
    'Validator'
  );
  ...
  logger.info(
    'Goal: 65-85% accuracy (better than random, not trivial)',
    undefined,
    'Validator'
  );
  logger.info('─'.repeat(60), undefined, 'Validator');
  ...
  for (const game of games) {
    for (const question of game.setup.questions) {
      ...
      const yesVotes = strongClues.filter(
        (p) => p.pointsToward === true
      ).length;
      const noVotes = strongClues.filter(
        (p) => p.pointsToward === false
      ).length;
  ...
  const strategyPass = accuracy > 0.65 && accuracy < 0.9;

  logger.info('', undefined, 'Validator');
  logger.info(
    `Result: ${correctPredictions}/${totalPredictions} = ${(accuracy * 100).toFixed(0)}%`,
    undefined,
    'Validator'
  );
  logger.info(
    strategyPass ? '✅ PASS (65-90% target)' : '❌ FAIL',
    undefined,
    'Validator'
  );
  ...
  logger.info(
    'Goal: High reliability NPCs are more accurate than low',
    undefined,
    'Validator'
  );
  logger.info('─'.repeat(60), undefined, 'Validator');
  ...
  const highAccuracy =
    reliabilityBuckets.high.length > 0
      ? calculateGroupAccuracy(games[0]!, reliabilityBuckets.high)
      : 0;
  const mediumAccuracy =
    reliabilityBuckets.medium.length > 0
      ? calculateGroupAccuracy(games[0]!, reliabilityBuckets.medium)
      : 0;
  const lowAccuracy =
    reliabilityBuckets.low.length > 0
      ? calculateGroupAccuracy(games[0]!, reliabilityBuckets.low)
      : 0;

  logger.info(
    `High reliability NPCs (${reliabilityBuckets.high.length}): ${(highAccuracy * 100).toFixed(0)}% accurate`,
    undefined,
    'Validator'
  );
  logger.info(
    `Medium reliability NPCs (${reliabilityBuckets.medium.length}): ${(mediumAccuracy * 100).toFixed(0)}% accurate`,
    undefined,
    'Validator'
  );
  logger.info(
    `Low reliability NPCs (${reliabilityBuckets.low.length}): ${(lowAccuracy * 100).toFixed(0)}% accurate`,
    undefined,
    'Validator'
  );

  const correlationPass =
    highAccuracy > mediumAccuracy + 0.1 || mediumAccuracy > lowAccuracy + 0.1;
  logger.info('', undefined, 'Validator');
  logger.info(
    correlationPass
      ? '✅ PASS: Reliability correlates with accuracy'
      : '⚠️  PARTIAL: Weak correlation',
    undefined,
    'Validator'
  );
  ...
  logger.info('', undefined, 'Validator');
  logger.info(
    `OVERALL SCORE: ${passCount}/${checks.length} checks passed (${(score * 100).toFixed(0)}%)`,
    undefined,
    'Validator'
  );
  ...
  if (score === 1.0) {
    logger.info(
      '🎉 EXCELLENT: Game is fully learnable and ready for RL',
      undefined,
      'Validator'
    );
  } else if (score >= 0.67) {
    logger.info(
      '✅ GOOD: Game is learnable with minor improvements needed',
      undefined,
      'Validator'
    );
  } else {
    logger.info(
      '❌ NEEDS WORK: Game requires fixes before RL training',
      undefined,
      'Validator'
    );
  }
