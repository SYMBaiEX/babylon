...
async function main() {
  ...
  if (validation.contracts.identityRegistry) {
    ...
    if (registryContract && 'identityCount' in registryContract) {
      await registryContract
        ...
        .then((count: bigint) => {
          logger.info(
            `✅ Identity Registry functional (${count} identities)`,
            undefined,
            'Script'
          );
        })
        .catch((error: Error) => {
          logger.warn(
            '⚠️  Identity Registry smoke test failed',
            error,
            'Script'
          );
        });
    ...
    await $`bun run agent0:verify`
      ...
      .then(async () => {
        ...
        await $`bun run agent0:register`
          ...
          .catch(() => {
            logger.warn(
              '⚠️  Agent0 registration failed or already registered',
              undefined,
              'Script'
            );
          });
  ...
  if (validation.contracts.diamond) {
    ...
    logger.info(
      `  Diamond: ${validation.contracts.diamond}`,
      undefined,
      'Script'
    );
    logger.info(
      `  View: https://sepolia.basescan.org/address/${validation.contracts.diamond}`,
      undefined,
      'Script'
    );
    ...
    if (validation.contracts.identityRegistry) {
      logger.info(
        `  Identity Registry: ${validation.contracts.identityRegistry}`,
        undefined,
        'Script'
      );
    }
    ...
    if (validation.contracts.reputationSystem) {
      logger.info(
        `  Reputation System: ${validation.contracts.reputationSystem}`,
        undefined,
        'Script'
      );
    }
