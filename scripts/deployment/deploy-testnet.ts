...
async function main() {
  logger.info(
    'Deploying Babylon contracts to Base Sepolia testnet...',
    undefined,
    'Script'
  );
  ...
  if (!process.env.ETHERSCAN_API_KEY) {
    logger.warn(
      '⚠️  ETHERSCAN_API_KEY not set. Contract verification will be skipped.',
      undefined,
      'Script'
    );
    logger.info(
  ...
  logger.warn(
    '⚠️  You are about to deploy to Base Sepolia TESTNET',
    undefined,
    'Script'
  );
  ...
  try {
    ...
    if (addresses.chainlinkOracle) {
      logger.info(
        `  ChainlinkOracle (Mock): ${addresses.chainlinkOracle}`,
        undefined,
        'Script'
      );
    }
    if (addresses.umaOracle) {
      logger.info(
        `  UMAOracle (Mock): ${addresses.umaOracle}`,
        undefined,
        'Script'
      );
    }
    ...
    logger.info(
      `  Diamond: https://sepolia.basescan.org/address/${addresses.diamond}`,
      undefined,
      'Script'
    );
    logger.info(
      `  Identity Registry: https://sepolia.basescan.org/address/${addresses.identityRegistry}`,
      undefined,
      'Script'
    );
    logger.info('', undefined, 'Script');
    ...
    if (!process.env.AGENT0_ENABLED || process.env.AGENT0_ENABLED !== 'true') {
      logger.info(
        '  1. Configure Agent0: bun run agent0:configure',
        undefined,
        'Script'
      );
      logger.info(
        '  2. Register with Agent0: bun run agent0:register',
        undefined,
        'Script'
      );
      logger.info(
        '  3. Start dev server: bun run dev:testnet',
        undefined,
        'Script'
      );
    } else {
      logger.info(
        '  1. Verify contracts (if not auto-verified): bun run scripts/verify-contracts.sh',
        undefined,
        'Script'
      );
      logger.info(
        '  2. Start dev server: bun run dev:testnet',
        undefined,
        'Script'
      );
    }
...
async function attemptAgent0Registration(): Promise<void> {
  ...
  const hasPrivateKey = !!(
    process.env.BABYLON_GAME_PRIVATE_KEY || process.env.AGENT0_PRIVATE_KEY
  );
  ...
  if (!hasRpcUrl) {
    ...
    logger.info(
      '   Set in .env.testnet to enable Agent0 registration',
      undefined,
      'Script'
    );
    return;
  ...
  if (!hasSubgraph) {
    ...
    logger.info(
      '   Agent0 will work but discovery may be limited',
      undefined,
      'Script'
    );
  }
  ...
  await $`bun run agent0:register`
    .quiet()
    .then(() => {
      logger.info('✅ Agent0 registration successful!', undefined, 'Script');
      logger.info(
        '   Your game is now discoverable in the Agent0 network',
        undefined,
        'Script'
      );
    })
    .catch(() => {
      logger.warn(
        '⚠️  Agent0 registration failed (you can register manually later)',
        undefined,
        'Script'
      );
...
function parseDeploymentOutput(output: string): ContractAddresses {
  ...
  const patterns = {
    ...
    diamond:
      /Diamond.*Proxy.*:\s*(0x[a-fA-F0-9]{40})|^Diamond:\s*(0x[a-fA-F0-9]{40})/m,
