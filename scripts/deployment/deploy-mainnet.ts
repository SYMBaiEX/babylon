...
async function main() {
  ...
  if (process.env.USE_MAINNET !== 'true') {
    logger.error(
      '❌ USE_MAINNET must be set to "true" to deploy to mainnet',
      undefined,
      'Script'
    );
    logger.info(
  ...
  if (!process.env.ETHERSCAN_API_KEY) {
    logger.error(
      '❌ ETHERSCAN_API_KEY is required for mainnet deployment',
      undefined,
      'Script'
    );
    logger.info(
  ...
  logger.info('', undefined, 'Script');
  logger.warn(
    '⚠️  YOU ARE ABOUT TO DEPLOY TO BASE MAINNET',
    undefined,
    'Script'
  );
  logger.warn('   This will cost REAL ETH.', undefined, 'Script');
  logger.warn(
    `   Estimated cost: ${estimatedCostEth} ETH`,
    undefined,
    'Script'
  );
  ...
  try {
    ...
    logger.info(
      `  Diamond: https://basescan.org/address/${addresses.diamond}`,
      undefined,
      'Script'
    );
    logger.info(
      `  Identity Registry: https://basescan.org/address/${addresses.identityRegistry}`,
      undefined,
      'Script'
    );
    logger.info('', undefined, 'Script');
