...
async function validateA2AProtocol() {
  ...
  try {
    ...
    if (!response.ok) {
      addResult(
        'A2A',
        'AgentCard accessible',
        'fail',
        'Cannot fetch agent card',
        true
      );
      return;
    }
    ...
    if (card.protocolVersion === '0.3.0') {
      ...
      addResult(
        ...
        'Protocol version',
        'fail',
        `Got ${card.protocolVersion}, expected 0.3.0`,
        true
      );
    }
    ...
    if (!card.skills || card.skills.length === 0) {
      ...
      addResult(
        ...
        'Skills declaration',
        'pass',
        `${card.skills.length} skills defined`,
        true
      );
      ...
      if (invalidSkills > 0) {
        addResult(
          ...
          'Skill validation',
          'fail',
          `${invalidSkills} skills missing required fields`,
          true
        );
      } else {
        addResult(
    ...
    try {
      ...
      addResult(
        ...
        'pass',
        'Official A2AClient initialized',
        true
      );
        ...
        if ('kind' in sendResponse && sendResponse.kind === 'task') {
          ...
          try {
            ...
            addResult(
              ...
              'tasks/get method',
              'pass',
              `Task retrieval working (state=${taskState})`,
              true
            );
          } catch (error) {
            addResult(
  ...
  try {
    ...
    if (!configValue?.agentId) {
      addResult(
        ...
        'fail',
        'Babylon not registered on Agent0',
        true
      );
      addResult(
        'Agent0',
    ...
    addResult(
      ...
      'On-chain registration',
      'pass',
      `Registered as ${configValue.agentId}`,
      true
    );
    ...
    try {
      ...
      if (agent) {
        addResult(
          ...
          'Subgraph indexed',
          'pass',
          'Found in Agent0 subgraph',
          true
        );
        addResult(
          'Agent0',
          'Agent active',
          agent.active ? 'pass' : 'warning',
          `Active: ${agent.active}`
        );
        ...
        if (agent.a2a) {
          ...
          addResult(
            ...
            'A2A support',
            'fail',
            'A2A not enabled in registration',
            true
          );
        }
        ...
        if (agent.a2aSkills && agent.a2aSkills.length > 0) {
          ...
          addResult(
            ...
            'Skills indexed',
            'warning',
            'No skills indexed (may take time)'
          );
        }
      } else {
        addResult(
          ...
          'Subgraph indexed',
          'warning',
          'Not yet indexed (try again in a few minutes)'
        );
      }
    } catch (error) {
    ...
    try {
      ...
      const found = searchResults.items.find(
        (a) => a.agentId === configValue?.agentId
      );
      ...
      if (found) {
        addResult(
          ...
          'Discoverable',
          'pass',
          'Babylon is discoverable via search',
          true
        );
      } else {
        addResult(
          ...
          'Discoverable',
          'warning',
          'Not yet discoverable (indexing delay)'
        );
      }
    } catch (error) {
...
async function generateReport() {
  ...
  console.log('\n📈 Overall Compliance:');
  console.log(
    `  All Checks:       ${totalPassed}/${results.length} (${overallScore}%)`
  );
  console.log(
    `  Required Checks:  ${requiredPassed}/${totalRequired} (${requiredScore}%)`
  );
