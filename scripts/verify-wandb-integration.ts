  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const username =
          data.username || data.entity || process.env.WANDB_ENTITY || 'unknown';
      ...
      console.error(
        '❌ W&B API authentication failed, but continuing with env vars'
      );
      return null;
    } catch (error) {
      // Fall back to env var
...
async function listWandbRuns(project: string): Promise<WandbRun[]> {
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        `   If project doesn't exist, create it first or check entity/project name`
      );
    ...
    if (runs.length > 0) {
      ...
      console.log(
        '   No runs found yet. Training will create runs automatically.'
      );
    }
...
async function checkWandbModels(project: string): Promise<WandbModel[]> {
  try {
    ...
    const data = (await response.json()) as {
      artifacts?: Array<{ name: string; version: string; createdAt: string }>;
    };
    ...
    if (artifacts.length > 0) {
      ...
      console.log(
        '   No model artifacts found yet. Training will create them automatically.'
      );
    }
...
async function checkDatabaseModels(): Promise<void> {
  try {
    ...
    try {
      ...
      console.log(
        '⚠️  Database not available (this is okay for W&B verification)'
      );
      console.log(
...
async function main() {
  ...
  console.log(
    `✅ WANDB_API_KEY is set (${process.env.WANDB_API_KEY.length} chars)`
  );
