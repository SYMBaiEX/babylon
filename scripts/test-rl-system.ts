...
async function testBenchmarking() {
  ...
  try {
    ...
    if (modelsToTest.length > 0) {
      try {
        const comparison = await benchmarkService.compareModels(
          modelsToTest[0]!.modelId
        );
        addResult(
          'Benchmarking',
          'Comparison Logic',
...
async function testDatabase() {
  ...
  try {
    ...
    const modelStats = {
      ...
      benchmarked: await db.trainedModel.count({
    ...
    const batchStats = {
      ...
      completed: await db.trainingBatch.count({
        where: { status: 'completed' },
      }),
...
async function printSummary() {
  ...
  console.log(
    `Overall: ${totalPassed}/${totalTests} tests passed (${totalPercentage.toFixed(1)}%)\n`
  );
...
async function main() {
  ...
  console.log(
    '║           Complete RL Training System - Comprehensive Test                ║'
  );
  console.log(
    '╚════════════════════════════════════════════════════════════════════════════╝'
  );
