...
async function loadTable(db: postgres.Sql, table: string): Promise<number> {
  ...
  // Get current count
  const beforeCountResult = await db.unsafe(
    `SELECT COUNT(*)::int as c FROM "${table}"`
  );
  const beforeCount = (beforeCountResult[0]?.c as number) ?? 0;
  log(
    `${table}: ${beforeCount} existing rows in target, inserting ${rows.length} (with ON CONFLICT DO NOTHING)`
  );
  ...
  for (let i = 0; i < batches.length; i += WORKERS) {
      ...
      group.map(async (batch) => {
        ...
        const valueRows = processed.map((row) => {
          const placeholders = commonCols.map((col) => {
            values.push(row[col] as string | number | boolean | null | Date);
            return `$${idx++}`;
          });
          return `(${placeholders.join(',')})`;
        });

        // Use ON CONFLICT DO NOTHING without specifying column to handle ALL unique constraints (id, username, etc.)
        const sql = `INSERT INTO "${table}" (${colList}) VALUES ${valueRows.join(',')} ON CONFLICT DO NOTHING`;
        await db.unsafe(sql, values as postgres.ParameterOrJSON<never>[]);
      })
    );
    ...
    const rate = Math.round(
      group.reduce((s, b) => s + b.length, 0) / (groupMs / 1000)
    );
    log(
      `  ${table}: ${pct}% (${inserted}/${rows.length}) - ${groupMs}ms, ${rate}/s`
    );
  }

  // Get final count
  const afterCountResult = await db.unsafe(
    `SELECT COUNT(*)::int as c FROM "${table}"`
  );
  const afterCount = (afterCountResult[0]?.c as number) ?? 0;
  const newRows = afterCount - beforeCount;

  const totalMs = Date.now() - start;
  const rate = Math.round(inserted / (totalMs / 1000));
  log(
    `✓ ${table}: ${newRows} new rows inserted (${afterCount} total) in ${(totalMs / 1000).toFixed(1)}s (${rate}/s)`
  );
...
async function main(): Promise<void> {
  ...
  log('\n═══════════════════════════════════════════════════════════');
  log(
    `DONE: ${totalRows} new rows inserted in ${(totalMs / 1000).toFixed(1)}s`
  );
  log('═══════════════════════════════════════════════════════════');
}
