...
async function verifyDataCollection(
  ...
  for (const traj of trajectories) {
    ...
    // Check 2: LLM calls
    const llmCallsTotal = steps.reduce(
      (sum: number, s: TrajectoryStep) => sum + (s.llmCalls?.length || 0),
      0
    );
    if (llmCallsTotal === 0) {
      result.checks.push({
    ...
    const providerTotal = steps.reduce(
      (sum: number, s: TrajectoryStep) =>
        sum + (s.providerAccesses?.length || 0),
      0
    );
    if (providerTotal === 0) {
      result.checks.push({
        ...
        status: 'WARN',
        message:
          '⚠️  No provider accesses - agent may be operating without context',
      });
    ...
    // Check 4: Actions
    const actionsWithResults = steps.filter(
      (s: TrajectoryStep) => s.action?.result || s.action?.error
    ).length;
    ...
    const stepsWithEnv = steps.filter(
      (s: TrajectoryStep) => s.environmentState
    ).length;
...
async function generateReport() {
  ...
  if (result.overall === 'PASS') {
    ...
    console.log(
      '❌ Data collection has critical issues! Fix before training.\n'
    );
  }
  ...
  console.log(
    `  Avg prompt tokens: ${llmStats._avg?.promptTokens?.toFixed(0) || 0}`
  );
  console.log(
    `  Avg completion tokens: ${llmStats._avg?.completionTokens?.toFixed(0) || 0}`
  );
