...
async function collectTrajectories(count = 10) {
  ...
  if (process.env.RECORD_AGENT_TRAJECTORIES !== 'true') {
    ...
    console.log(
      '✅ RECORD_AGENT_TRAJECTORIES=true (trajectories will be recorded)\n'
    );
  }
  ...
  if (agents.length === 0) {
    ...
    console.log(
      '   Create agents with: bun run scripts/spawn-test-agents.ts\n'
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    console.log(
      `[${i + 1}/${count}] Running agent: ${agent.username || agent.id}`
    );
  ...
  if (newTrajectories > 0) {
    ...
    console.log(
      '   Set RECORD_AGENT_TRAJECTORIES=true to collect trajectories.\n'
    );
  }
}
