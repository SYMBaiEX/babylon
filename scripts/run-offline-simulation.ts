...
import {
  ...
  SimulationEngine,
} from '../src/lib/benchmark/SimulationEngine';
...
async function loadGameData(dataPath: string): Promise<BenchmarkGameSnapshot> {
  ...
  throw new Error(
    'Unknown data format. Expected game world or benchmark snapshot.'
  );
}
...
async function runOfflineSimulation(
  ...
  console.log(
    `Question: ${snapshot.initialState.predictionMarkets[0]?.question || 'Unknown'}`
  );
  console.log(
    `Duration: ${snapshot.ticks.length} ticks (${Math.floor((snapshot.ticks.length * snapshot.tickInterval) / 60)} minutes)`
  );
  console.log(
    `Fast-forward: ${(options.fastForward ?? true) ? 'YES (faster-than-real-time)' : 'NO (real-time)'}`
  );
  console.log('');
  ...
  console.log(
    `   Mode: ${options.fastForward !== false ? 'FAST-FORWARD (no delays)' : 'REAL-TIME'}\n`
  );
  ...
  while (!engine.isComplete() && ticksProcessed < maxTicks) {
    ...
    if (currentTick % 100 === 0 || currentTick < 5) {
      ...
      console.log(
        `   Tick ${currentTick}/${maxTicks} (${ticksPerSec.toFixed(1)} ticks/sec, ${elapsed}s elapsed)`
      );
    }
    ...
    if (options.fastForward === false) {
      await new Promise((resolve) =>
        setTimeout(resolve, snapshot.tickInterval * 1000)
      );
    }
  }
  ...
  console.log(
    `Speedup: ${(ticksPerSec / (1 / snapshot.tickInterval)).toFixed(1)}x faster than real-time`
  );
  console.log('');
  console.log('📊 Results:');
...
function parseArgs(): OfflineSimulationOptions {
  ...
  for (const arg of args) {
    if (arg.startsWith('--data=')) {
      ...
      options.maxTicks = Number.parseInt(arg.split('=')[1]!);
    }
  }
