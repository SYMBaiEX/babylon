...
async function listResults(): Promise<void> {
  ...
  const dirs = ['baselines', 'model-comparison', 'test-baselines', 'quick-test'];
...
async function viewResult(filePath: string): Promise<void> {
  ...
  try {
    ...
    const metrics = data.metrics ?? {
      ...
      predictionMetrics: data.predictionMetrics ?? { totalPositions: 0, correctPredictions: 0, incorrectPredictions: 0, accuracy: 0, avgPnlPerPosition: 0 },
      perpMetrics: data.perpMetrics ?? { totalTrades: 0, profitableTrades: 0, winRate: 0, avgPnlPerTrade: 0, maxDrawdown: 0 },
      socialMetrics: data.socialMetrics ?? { postsCreated: 0, groupsJoined: 0, messagesReceived: 0, reputationGained: 0 },
      timing: data.timing ?? { avgResponseTime: 0, maxResponseTime: 0, totalDuration: 0 },
      optimalityScore: data.optimalityScore ?? 0,
    };
    
    const modelName = data.model?.displayName ?? data.modelId ?? 'Unknown Model';
    ...
    console.log(`   Optimality Score: ${metrics.optimalityScore.toFixed(1)}%\n`);
    
    console.log('🎯 Prediction Markets:');
    console.log(`   Total Positions: ${metrics.predictionMetrics.totalPositions}`);
    console.log(`   Accuracy: ${(metrics.predictionMetrics.accuracy * 100).toFixed(1)}%`);
    console.log(`   Correct: ${metrics.predictionMetrics.correctPredictions}`);
    console.log(`   Incorrect: ${metrics.predictionMetrics.incorrectPredictions}\n`);
    
    console.log('📈 Perpetual Trades:');
    console.log(`   Total Trades: ${metrics.perpMetrics.totalTrades}`);
    console.log(`   Win Rate: ${(metrics.perpMetrics.winRate * 100).toFixed(1)}%`);
    console.log(`   Max Drawdown: $${metrics.perpMetrics.maxDrawdown.toFixed(2)}\n`);
    
    console.log('⏱️ Timing:');
    console.log(`   Total Duration: ${(metrics.timing.totalDuration / 1000).toFixed(1)}s`);
    console.log(`   Avg Response: ${metrics.timing.avgResponseTime.toFixed(0)}ms\n`);
...
async function generateReport(dirPath: string, outputPath?: string): Promise<void> {
  const fullPath = path.isAbsolute(dirPath) ? dirPath : path.join(process.cwd(), dirPath);
  ...
  try {
    ...
    const reportPath = await BenchmarkChartGenerator.generateReport(results, output, {
...
async function showHistory(): Promise<void> {
  ...
  // Print table
  console.log('┌────────────────────────┬─────────────────────┬────────────┬──────────┬───────────────────────┐');
  console.log('│ Directory              │ Model               │ P&L        │ Accuracy │ Run At                │');
  console.log('├────────────────────────┼─────────────────────┼────────────┼──────────┼───────────────────────┤');
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'view':
      if (!args[0]) {
        console.error('Error: Please provide a path to a benchmark result file');
        process.exit(1);
      }
