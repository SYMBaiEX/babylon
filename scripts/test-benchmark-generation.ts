...
async function testGeneration() {
  ...
  try {
    ...
    // Check posts
    const hasPosts = snapshot.ticks.some(
      (t) => t.state.posts && t.state.posts.length > 0
    );
    console.log(`   Posts: ${hasPosts ? '✅' : '❌'}`);

    // Check group chats
    const hasGroupChats = snapshot.ticks.some(
      (t) => t.state.groupChats && t.state.groupChats.length > 0
    );
    console.log(`   Group Chats: ${hasGroupChats ? '✅' : '❌'}`);

    // Check ground truth
    console.log(
      `   Market Outcomes: ${Object.keys(snapshot.groundTruth.marketOutcomes).length > 0 ? '✅' : '❌'}`
    );
    console.log(
      `   Hidden Facts: ${(snapshot.groundTruth.hiddenFacts?.length || 0) > 0 ? '✅' : '❌'}`
    );
    console.log(
      `   Hidden Events: ${(snapshot.groundTruth.hiddenEvents?.length || 0) > 0 ? '✅' : '❌'}`
    );
    console.log(
      `   True Facts: ${Object.keys(snapshot.groundTruth.trueFacts || {}).length > 0 ? '✅' : '❌'}`
    );
    ...
    console.log(
      `   Market Outcomes: ${outcomes.predictions.length} predictions, ${outcomes.stocks.length} stocks`
    );
    ...
    console.log('\n6. Security check...');
    const security =
      BenchmarkDataViewer.verifyAgentCannotAccessHiddenFacts(snapshot);
    ...
    if (jsonStr.includes('null') && jsonStr.match(/":\s*null/g)?.length) {
      ...
      console.log(
        `   ⚠️  Found ${nullMatches?.length} null values (may be expected)`
      );
    } else {
