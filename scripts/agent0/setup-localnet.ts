...
async function main() {
  ...
  logger.info(
    'Updating .env.test with Agent0 localnet configuration...',
    undefined,
    'Script'
  );
  ...
  logger.info('', undefined, 'Script');
  logger.info(
    'Note: Agent0 contracts may not be deployed on localnet.',
    undefined,
    'Script'
  );
  logger.info(
    'This is OK for testing - the SDK will work in read-only mode.',
    undefined,
    'Script'
  );
}
