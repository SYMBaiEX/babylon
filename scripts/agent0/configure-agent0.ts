...
async function main() {
  ...
  if (!currentConfig.rpcUrl) {
    logger.info(
      'Setting Ethereum Sepolia RPC URL for Agent0...',
      undefined,
      'Script'
    );
  ...
  if (!currentConfig.subgraphUrl) {
    ...
    logger.info(
      '   Update AGENT0_SUBGRAPH_URL in .env.testnet with your actual subgraph URL',
      undefined,
      'Script'
    );
  }
  ...
  if (Object.keys(updates).length > 0) {
    ...
    logger.info('', undefined, 'Script');
    logger.info(
      '✅ Configuration updated in .env.testnet',
      undefined,
      'Script'
    );
  }
  ...
  if (!currentConfig.privateKey) {
    logger.info(
      '1. Set BABYLON_GAME_PRIVATE_KEY in .env.testnet',
      undefined,
      'Script'
    );
    logger.info(
      '   This is the private key for your game agent (needs ETH for registration)',
      undefined,
      'Script'
    );
    logger.info('', undefined, 'Script');
  ...
  if (
    ...
    logger.info(
      '2. Update AGENT0_SUBGRAPH_URL in .env.testnet',
      undefined,
      'Script'
    );
    logger.info(
      '   Get the subgraph URL from The Graph Studio',
      undefined,
      'Script'
    );
    logger.info('', undefined, 'Script');
  ...
  if (!currentConfig.pinataJwt) {
    logger.info(
      '3. (Optional) Set PINATA_JWT for Pinata IPFS hosting',
      undefined,
      'Script'
    );
