...
import { GameSimulator } from '@babylon/engine';
...
async function runSingleGame(outcome: boolean, options: CLIOptions) {
  ...
  if (options.verbose && !options.json) {
    ...
    simulator.on('game:started', (event) => {
      ...
      logger.info(
        `Predetermined Outcome: ${outcome ? 'YES' : 'NO'}`,
        undefined,
        'CLI'
      );
    ...
    simulator.on('clue:distributed', (event) => {
      if (options.verbose && !options.fast) {
        logger.debug(
          `${event.agentId}: Received clue (${event.data.tier})`,
          undefined,
          'CLI'
        );
      }
    ...
    simulator.on('agent:bet', (event) => {
      logger.info(
        `${event.agentId}: Bet ${event.data.outcome ? 'YES' : 'NO'} (${event.data.amount} tokens)`,
        undefined,
        'CLI'
      );
    });
    ...
    simulator.on('market:updated', (event) => {
      if (options.verbose && event.day % 10 === 0) {
        logger.info(
          `Market: ${event.data.yesOdds}% YES / ${event.data.noOdds}% NO`,
          undefined,
          'CLI'
        );
      }
    ...
    simulator.on('game:ended', (event) => {
      logger.info(
        `Winners: ${event.data.winners.join(', ')}`,
        undefined,
        'CLI'
      );
    });
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!options.fast && !options.json) {
        process.stdout.write(
          `\r[${i + 1}/${count}] ${Math.round(((i + 1) / count) * 100)}%`
        );
      }
    }
