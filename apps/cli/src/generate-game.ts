 ...
 * @see {@link ../engine/GameGenerator.ts} for implementation details
 * @since v0.1.0
 */
...
const logger = {
  ...
  error: (msg: string, data?: LogData, _ctx?: string) => {
    if (data) {
  ...
  debug: (msg: string, data?: LogData, _ctx?: string) => {
  ...
  warn: (msg: string, data?: LogData, _ctx?: string) => {
    if (data) {
...
async function validateActorsData(): Promise<void> {
  ...
  if (errors.length > 0) {
    ...
    logger.error(
      'Please fix actor data in database before generating a game.',
      undefined,
      'CLI'
    );
    process.exit(1);
...
async function main() {
  ...
  if (existingGames.length === 0) {
    ...
    logger.info(
      `Total events: ${genesis.timeline.reduce((sum, day) => sum + day.events.length, 0)}`,
      undefined,
      'CLI'
    );
    logger.info(
      `Total posts: ${genesis.timeline.reduce((sum, day) => sum + day.feedPosts.length, 0)}`,
      undefined,
      'CLI'
    );
  } else {
    logger.info(
      `Found ${existingGames.length} existing game(s) in database`,
      undefined,
      'CLI'
    );
  }
  ...
  if (existingGames.length > 0) {
    logger.info(
      `Found ${existingGames.length} previous game(s):`,
      undefined,
      'CLI'
    );
    ...
    for (
      ...
      if (historyConfig && historyConfig.value) {
        ...
        const minimalHistory = await generateMinimalGameHistory(
          gameData.id,
          i + 1
        );
        history.push(minimalHistory);
        logger.info(
          `Generated minimal history for game ${gameData.id}`,
          undefined,
          'CLI'
        );
      }
    ...
    logger.info(
      `Next game will be #${gameNumber} starting ${nextStartDate}`,
      undefined,
      'CLI'
    );
  } else {
    logger.info(
      'No previous games found. This will be Game #1',
      undefined,
      'CLI'
    );
    ...
    logger.info(
      `Starting ${nextStartDate} (current month, first game)`,
      undefined,
      'CLI'
    );
  }
  ...
  logger.info(
    `Total events: ${game.timeline.reduce((sum, day) => sum + day.events.length, 0)}`,
    undefined,
    'CLI'
  );
  logger.info(
    `Total feed posts: ${game.timeline.reduce((sum, day) => sum + day.feedPosts.length, 0)}`,
    undefined,
    'CLI'
  );
  logger.info(
  ...
  game.setup.scenarios.forEach((scenario) => {
    ...
    const scenarioQuestions = game.setup.questions.filter(
      (q) => q.scenario === scenario.id
    );
    if (scenarioQuestions.length > 0) {
      ...
      scenarioQuestions.forEach((q) => {
        ...
        logger.info(
          `        Outcome: ${outcomeIcon} | Rank: ${q.rank}`,
          undefined,
          'CLI'
        );
      });
  ...
  if (options.verbose) {
    ...
    logger.info(
      ...
      game.setup.mainActors.map((a) => `${a.name} (${a.tier})`),
      'CLI'
    );
    logger.info(
      `Questions (${game.setup.questions.length}):`,
      game.setup.questions.map(
        (q) => `${q.id}. ${q.text} - Answer: ${q.outcome ? 'YES' : 'NO'}`
      ),
      'CLI'
    );
