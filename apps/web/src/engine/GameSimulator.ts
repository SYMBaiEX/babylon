...
interface TypedGameSimulatorEmitter {
  on<K extends keyof GameSimulatorEvents>(
    event: K,
    listener: (data: GameSimulatorEvents[K]) => void
  ): this;
  emit<K extends keyof GameSimulatorEvents>(
    event: K,
    data: GameSimulatorEvents[K]
  ): boolean;
  off<K extends keyof GameSimulatorEvents>(
    event: K,
    listener: (data: GameSimulatorEvents[K]) => void
  ): this;
}
...
export class GameSimulator
  extends EventEmitter
  implements TypedGameSimulatorEmitter
{
  ...
  private generateClueNetwork(): ClueData[] {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      clues.push({
        ...
        reliability: isTrueClue
          ? 0.6 + Math.random() * 0.15
          : 0.4 + Math.random() * 0.2, // 60-75% for true, 40-60% for noise
      });
    }
    ...
    for (let i = 0; i < 7; i++) {
      ...
      clues.push({
        ...
        reliability: isTrueClue
          ...
          : 0.3 + Math.random() * 0.2, // 75-90% for true, 30-50% for noise
      });
    }
   ...
   */
  private getCluesForDay(
    day: number,
    agents: InternalAgent[],
    clueNetwork: ClueData[]
  ): DistributedClue[] {
    ...
    dayClues.forEach((clue) => {
      ...
      if (clue.tier === 'early') {
        ...
        recipient = agents.find(
          (a) => a.isInsider && a.cluesReceived.length < 8
        );

        // If no insiders available, give to random agent with few clues
        if (!recipient) {
          const eligibleAgents = agents.filter(
            (a) => a.cluesReceived.length < 5
          );
          recipient =
        ...
        const eligibleAgents = agents.filter(
          (a) => a.cluesReceived.length > 0 && a.cluesReceived.length < 10
        );
        recipient =
  ...
  private processAgentBets(
    ...
    agents.forEach((agent) => {
      ...
      const shouldBet =
        agent.cluesReceived.length > 0 && (day % 5 === 0 || day > 20);

      if (shouldBet && Math.random() > 0.5) {
        // Determine outcome based on clues
        const yesClues = agent.cluesReceived.filter(
          (c) => c.pointsToward
        ).length;
        ...
        const shares =
          (amount / (betOnYes ? market.yesOdds || 50 : market.noOdds || 50)) *
          100;
