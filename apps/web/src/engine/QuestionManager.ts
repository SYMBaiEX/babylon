...
import { type Article, ArticleGenerator } from '@/engine/ArticleGenerator';
...
`
  ...
  async generateProofContent(
    ...
    try {
      ...
      logger.error(
        'Failed to generate proof article',
        { error },
        'QuestionManager'
      );
      return null;
    ...
    try {
      ...
      for (const questionData of questionsData.slice(0, count)) {
        ...
        try {
          ...
          logger.warn(
            'Failed to trigger NPC betting on new question (non-blocking)',
            {
