...
describe('PerpPriceChart - Data Processing', () => {
  ...
  describe('Price Formatting Logic', () => {
    it('should format billions correctly', () => {
      ...
      const formatted = `ƀ${(price / 1000000000).toFixed(2)}B`;

      expect(formatted).toBe('ƀ1.50B');
    });

    it('should format millions correctly', () => {
      const price = 1500000;
      const formatted = `ƀ${(price / 1000000).toFixed(2)}M`;

      expect(formatted).toBe('ƀ1.50M');
    });

    it('should format thousands correctly', () => {
      const price = 1500;
      const formatted = `ƀ${(price / 1000).toFixed(2)}K`;

      expect(formatted).toBe('ƀ1.50K');
    });

    it('should format regular prices correctly', () => {
      const price = 123.456;
      const formatted = `ƀ${price.toFixed(2)}`;

      expect(formatted).toBe('ƀ123.46');
    });

    it('should format small decimals correctly', () => {
      const price = 0.001234;
      const formatted = `ƀ${price.toFixed(6)}`;

      expect(formatted).toBe('ƀ0.001234');
    });

    it('should format very small decimals correctly', () => {
      const price = 0.00000123;
      const formatted = `ƀ${price.toFixed(8)}`;

      expect(formatted).toBe('ƀ0.00000123');
    });

    it('should handle zero price', () => {
      const price = 0;
      const formatted = price === 0 ? '' : `ƀ${price}`;
