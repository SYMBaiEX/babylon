...
export class BenchmarkRunner {
  ...
  static async runSingle(
    ...
    const { AutonomousCoordinator } = await import(
      '@/lib/agents/autonomous/AutonomousCoordinator'
    );
    ...
    while (!engine.isComplete()) {
      ...
      if (
        ...
        'trades' in tickResult.actionsExecuted &&
        (tickResult.actionsExecuted.trades > 0 ||
          tickResult.actionsExecuted.posts > 0)
      ) {
