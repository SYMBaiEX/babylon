...
import { asc, count, db, eq, perpPositions } from '@/db';
import {
  isHybridMode,
  isOnChainEnabled,
  PERP_CONFIG,
} from '@/lib/config/perp-modes';
...
export class PerpSettlementService {
  ...
  static async settleOpenPosition(
    ...
    return await PerpSettlementService.settleToContract('open', settlementPos);
  }
  ...
  static async settleClosePosition(
    ...
    return await PerpSettlementService.settleToContract('close', settlementPos);
  }
  ...
  private static async executeBatchSettlement(): Promise<void> {
    ...
    const positions = await PerpSettlementService.getUnsettledPositionsFromDb(
      PERP_CONFIG.hybridBatchSize
    );

    if (positions.length === 0) {
      logger.debug(
        'No unsettled positions to settle',
        undefined,
        'PerpSettlementService'
      );
      return;
    ...
    const results = await Promise.allSettled(
      positions.map((pos) =>
        PerpSettlementService.settleToContract(
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled' && result.value.success) {
        successCount++;
        PerpSettlementService.unsettledPositions.delete(position.id);
        PerpSettlementService.markPositionSettled(
          position.id,
    ...
    logger.info(
      'Batch settlement completed',
      {
        ...
        failed: failureCount,
        remainingInMemory: PerpSettlementService.unsettledPositions.size,
      },
      'PerpSettlementService'
    );
  }
  ...
  private static async settleToContract(
    ...
    if (!privateKey) {
      throw new Error(
        'BABYLON_SETTLEMENT_PRIVATE_KEY not configured for settlement'
      );
    }

    const rpcUrl =
      process.env.NEXT_PUBLIC_RPC_URL || process.env.BASE_SEPOLIA_RPC_URL;
    if (!rpcUrl) {
      throw new Error('RPC_URL not configured for blockchain settlement');
    }

    logger.info(
      'Settling position to blockchain',
      {
        action,
        positionId: position.id,
        ticker: position.ticker,
    ...
    const marketId =
      `0x${Buffer.from(position.ticker).toString('hex').padEnd(64, '0')}` as Hash;
    ...
    if (action === 'open') {
      ...
      const collateralWei = BigInt(
        Math.floor((position.size / position.leverage) * 1e18)
      );
   ...
   */
  private static async markPositionUnsettled(
    positionId: string
  ): Promise<void> {
    ...
    if (result.length === 0) {
      ...
      PerpSettlementService.unsettledPositions.delete(positionId);
    }
  }
   ...
   * Get unsettled positions from database
   */
  private static async getUnsettledPositionsFromDb(
    limit: number
  ...
  static async getSettlementStats(): Promise<{
    ...
    return {
      ...
      settlementRate:
        totalPositions > 0
          ? ((totalPositions - unsettledCount) / totalPositions) * 100
          : 100,
    };
  }
}
