...
export class GroupChatInvite {
  ...
  // Probabilities
  private static readonly BASE_INVITE_PROBABILITY = 0.1; // 10% base chance
  private static readonly MAX_INVITE_PROBABILITY = 0.6; // 60% max chance

  // Weighting for chat type
  private static readonly OWNED_CHAT_WEIGHT = 0.7; // 70% chance for owned chat
  private static readonly MEMBER_CHAT_WEIGHT = 0.3; // 30% chance for member chat

  // Use MEMBER_CHAT_WEIGHT in probability calculations
  private static calculateChatTypeWeight(isOwned: boolean): number {
    return isOwned
      ? GroupChatInvite.OWNED_CHAT_WEIGHT
      : GroupChatInvite.MEMBER_CHAT_WEIGHT;
  }
  ...
  private static calculateInviteProbability(
    ...
    const weight = GroupChatInvite.calculateChatTypeWeight(isOwned);
    return Math.min(baseProb * weight, GroupChatInvite.MAX_INVITE_PROBABILITY);
  }
