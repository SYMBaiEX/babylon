...
import { db } from '@/db';
import { agentPerformanceMetrics, feedbacks, users } from '@/db/schema';
 ...
 */
export async function submitFeedbackToAgent0(
  feedbackId: string,
  ...
  if (submitToBlockchain && recipientUser.nftTokenId) {
    logger.info(
      'Submitting feedback to blockchain would require wallet client',
      {
        feedbackId,
