  ...
  try {
    ...
    if (questionsToResolve.length > 0) {
      ...
      const mappedEvents: WorldEvent[] = recentDbEvents
        .filter(
          (e) =>
            isValidEventType(e.eventType) && isValidVisibility(e.visibility)
        )
        .map((e) => ({
          id: e.id,
          day: e.dayNumber || 0,
    ...
    const { batchSyncReputationsToERC8004 } = await import(
      './reputation/erc8004-reputation-sync'
    );
    try {
    ...
    const { AlphaGroupInviteService } = await import(
      './services/alpha-group-invite-service'
    );
    ...
    const { NPCGroupDynamicsService } = await import(
      './services/npc-group-dynamics-service'
    );
    ...
    if (result.marketsUpdated === 0 && hadNPCTrading) {
      validationWarnings.push(
        'NPC trading executed but no markets were updated'
      );
    }
      ...
      const resolvedQuestions = await db
        ...
        .where(
          and(
            ...
            gte(
              questionsSchema.updatedAt,
              new Date(timestamp.getTime() - 60000)
      ...
      const articlePromise = (async () => {
        try {
          ...
          const org: Organization = {
            ...
            type:
              (orgData.type as 'company' | 'media' | 'government') || 'media',
          ...
          const transformedSummary =
            await characterMappingService.transformText(article.summary || '');
          const transformedContent =
            await characterMappingService.transformText(article.content || '');
          const transformedTitle = await characterMappingService.transformText(
            article.title || 'Untitled'
          );
          ...
          logger.warn(
            'Failed to generate article for question',
            {
              error: error instanceof Error ? error.message : String(error),
              questionId: question.id,
              orgId: orgData.id,
...
async function updateMarketPricesFromTrades(
  ...
  if (priceUpdatesForChain.length > 0) {
    const { PriceUpdateService } = await import(
      '@/lib/services/price-update-service'
    );
...
async function forceTrendingCalculation(): Promise<boolean> {
  ...
  const { calculateTrendingTags } = await import(
    './services/trending-calculation-service'
  );
  await calculateTrendingTags();
