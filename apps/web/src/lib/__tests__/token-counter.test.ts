...
describe('TokenCounter', () => {
  describe('countTokensSync', () => {
    ...
    test('should count tokens for longer text', () => {
      const text =
        'This is a much longer piece of text that contains multiple sentences. It should result in more tokens being counted. The approximation is 1 token per 4 characters.';
      const tokens = countTokensSync(text);
  ...
  describe('budgetTokens', () => {
    test('should distribute tokens by priority', () => {
      ...
      const total =
        (budget.high ?? 0) + (budget.medium ?? 0) + (budget.low ?? 0);
