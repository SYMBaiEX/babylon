...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  await PointsService.awardPrivateGroupCreate(user.userId, groupId).catch(
    (error) => {
      // Log error but don't fail group creation if points award fails
      console.error(
        'Failed to award points for private group creation:',
        error
      );
    }
  );
  ...
  if (validatedData.memberIds.length > 0) {
    const filteredIds = validatedData.memberIds.filter(
      (id) => id !== user.userId
    );
