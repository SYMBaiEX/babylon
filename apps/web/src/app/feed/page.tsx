...
function FeedPageContent() {
  ...
  useEffect(() => {
    ...
    fetchFollowingPosts();
  }, [tab, authenticated, user, getAccessToken]);

  // Compute timeline-visible posts from game (mirrors viewer FeedView)
  const timelinePosts = useMemo(() => {
    if (!startTime || !currentDate || allGames.length === 0)
      return [] as FeedPost[];
              ...
              {tab === 'trades' ? (
                ...
                <div className="w-full p-4 text-center sm:p-8">
                  <div className="py-8 text-muted-foreground sm:py-12">
                    ...
                    <p className="mb-4 text-sm sm:text-base">
                      Game tick runs every 60 seconds. Content will appear here
                      as it&apos;s generated.
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full space-y-0">
                  {basePosts.map((post, i: number) => {
                    ...
                    const postData = {
                      id: post.id,
                      type:
                        ('type' in post ? post.type : undefined) || undefined,
                      content: post.content,
                      articleTitle:
                      ...
                      authorName,
                      authorUsername:
                        ('authorUsername' in post
                          ? post.authorUsername
                          : null) || null,
                      authorProfileImageUrl:
                        'authorProfileImageUrl' in post
                          ? post.authorProfileImageUrl
                          : null,
                      timestamp: post.timestamp,
                      likeCount:
                        ('likeCount' in post
                          ? (post.likeCount as number)
                          : 0) || 0,
                      commentCount:
                        ('commentCount' in post
                          ? (post.commentCount as number)
                          : 0) || 0,
                      shareCount:
                        ('shareCount' in post
                          ? (post.shareCount as number)
                          : 0) || 0,
                      isLiked:
                        ('isLiked' in post
                      ...
                      isShared:
                        ('isShared' in post
                          ? (post.isShared as boolean)
                          : false) || false,
                      // Repost metadata
                      isRepost:
                        ('isRepost' in post
                          ? (post.isRepost as boolean)
                          : false) || false,
                      isQuote:
                        ('isQuote' in post
                          ? (post.isQuote as boolean)
                          : false) || false,
                      quoteComment:
                        ...
                        {showBannerAfterThisPost && (
                          <InviteFriendsBanner
                            onDismiss={() => {
                              ...
                              bannerInterval.current =
                                calculateBannerInterval();
                            }}
                          />
                        )}
                      </div>
