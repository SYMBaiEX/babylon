...
function filterPageMap(
  ...
  const isApiReferencePage = API_REFERENCE_PAGES.some(
    (page) => currentPath === `/${page}` || currentPath.startsWith(`/${page}/`)
  );

  const isDocumentationPage = DOCUMENTATION_PAGES.some(
    (page) => currentPath === `/${page}` || currentPath.startsWith(`/${page}/`)
  );
  ...
  if (isApiReferencePage) {
    return pageMap.filter((item) => {
      ...
      return (
        API_REFERENCE_PAGES.includes(
          routeKey as (typeof API_REFERENCE_PAGES)[number]
  ...
  if (isDocumentationPage) {
    return pageMap.filter((item) => {
      ...
      const routeKey = route.replace('/', '');
      return (
        DOCUMENTATION_PAGES.includes(
          routeKey as (typeof DOCUMENTATION_PAGES)[number]
