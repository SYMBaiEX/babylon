...
// Connection pooling is configured in packages/db/src/index.ts
...
# Drizzle logs queries when NODE_ENV=development
bun run dev

# Check for missing indexes in schema
# Add indexes to frequently queried fields in packages/db/src/schema/
```
...
# Agents run automatically via the autonomous coordinator
 ```
...
# Verify deployment
cat packages/contracts/deployments/base-sepolia/index.json
```
...
ls apps/web/src/app/api/path/to/route.ts
...
# Enable query logging in development
bun run dev
# Look for slow queries in console output
...
# Drizzle logs queries in development
bun run dev
...
await db.insert(users)
  .values({ username: 'trader', displayName: 'Trader' })
  .onConflictDoUpdate({
