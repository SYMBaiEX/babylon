...
babylon/
 ...
 packages/
  agents/           # Agent system (@babylon/agents)
  a2a/              # A2A protocol (@babylon/a2a)
  api/              # API utilities (@babylon/api)
  contracts/        # Smart contracts (@babylon/contracts)
  db/               # Database schema (@babylon/db)
  engine/           # Game engine (@babylon/engine)
  shared/            # Shared utilities (@babylon/shared)
  training/          # RL training (@babylon/training)
  testing/           # Testing utilities (@babylon/testing)
```
...
2. Add validation schema in `apps/web/src/lib/validation/schemas.ts`
3. Update API documentation
4. Write tests in `packages/testing/integration/`
...
```typescript
// packages/db/src/schema/my-model.ts
...
export const myModels = pgTable('my_models', {
  ...
  nameIdx: index('my_models_name_idx').on(table.name),
}));
```
...
```bash
# Drizzle logs queries in development automatically
