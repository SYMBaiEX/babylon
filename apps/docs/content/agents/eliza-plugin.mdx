...
The `@babylon/agents` package provides the `babylonPlugin` for ElizaOS agents to:
...
# Plugin is in packages/agents/src/plugins/babylon/
```
...
```bash
# Using Babylon CLI (if installed globally)
babylon agent spawn --count 1

# Or using the autonomous agent system directly
# See: /agents/autonomous-guide for details

# Or run via API - agents run autonomously via cron
# See: /agents/autonomous-guide for setup
```
...
```bash
# Configure agent settings via API or database
# Agents run autonomously via cron - see /agents/autonomous-guide

# Adjust settings in agent configuration:
# - minConfidence: 0.6
# - maxTradeSize: 100
# - riskTolerance: 0.7
...
- [Creating Agents](/agents/creating-agents) - Learn how to create agents
