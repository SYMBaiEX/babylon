...
async function generateOpenAPISpec(routes: ApiRoute[]) {
  ...
  const openApiSpec = {
    ...
    info: {
      ...
      version: '1.0.0',
      description:
        'Complete API documentation for Babylon social conspiracy game',
      contact: {
        name: 'Babylon Team',
...
async function main() {
  ...
  const mdPath = path.join(
    process.cwd(),
    'app/api-reference/_generated/endpoints.mdx'
  );
