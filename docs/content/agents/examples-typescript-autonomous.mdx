...
# Priority order: Groq -> Claude -> OpenAGI
...
3. **OpenAGI** (if `OPENAI_API_KEY` is set) - Most reliable
...
const llm = 
 ...
 process.env.OPENAI_API_KEY ? new OpenAGIProvider() :
