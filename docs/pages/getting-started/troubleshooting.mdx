...
2. **Verify balance**:
   ```bash
   bun run scripts/check-wallet-status.ts
   ```
...
# 3. Check daemon is running (dev mode)
ps aux | grep realtime-daemon
...
2. **GitHub**:
   - [Search existing issues](https://github.com/yourusername/babylon/issues)
