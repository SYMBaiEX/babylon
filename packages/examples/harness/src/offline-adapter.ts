...
export class SimulationAdapter implements A2AClientInterface {
  ...
  async getPortfolio(): Promise<{ balance: number; positions: Position[]; pnl: number }> {
  ...
  async buyShares(marketId: string, outcome: 'YES' | 'NO', amount: number): Promise<Trade> {
    const result = this.store.buyPredictionShares(this.userId, marketId, outcome, amount);
  ...
  async sellShares(marketId: string, outcome: 'YES' | 'NO', shares: number): Promise<Trade> {
    const result = this.store.sellPredictionShares(this.userId, marketId, outcome, shares);
  ...
  async likePost(postId: string): Promise<{ success: boolean; likesCount: number }> {
  ...
  getProgress(): { tick: number; day: number; hour: number; totalTicks: number } {
  ...
  async openPerpPosition(
    ...
    const result = this.store.openPerpPosition(this.userId, ticker, side, size, leverage);
