...
async function runAllTests() {
  ...
  await test('register', async () => {
    // Register may fail with "already registered" if auto-registration happened
    // This is expected behavior - we try to register, and it works either way
    try {
      ...
      if (error.message.includes('already registered')) {
        // This is fine - agent was auto-registered
        return { success: true, note: 'Agent already auto-registered' };
      }
      throw e;
    }
  });
