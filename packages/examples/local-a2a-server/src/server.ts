...
// Initialize services (blockchain optional - will work without anvil)
let blockchain: LocalBlockchain;
try {
  const provider = new ethers.JsonRpcProvider(RPC_URL, undefined, {
    staticNetwork: true,
    batchMaxCount: 1,
  });
  blockchain = new LocalBlockchain(provider);
} catch {
  // Create a mock provider that won't spam errors
