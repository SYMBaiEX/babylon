...
describe('NPC Voice Diversity Integration', () => {
  ...
  it('should load actors from static registry', async () => {
    ...
    expect(actorIds.some((id) => id.includes('kanye') || id.includes('kanyai'))).toBe(true);
  });

  it.skipIf(!hasApiKey)('should generate a post for KanyAI with uppercase style', async () => {
    const actors = StaticDataRegistry.getAllActors();
...
${templates.slice(0, 3).map((t) => `"${t}"`).join('\n')}
...
<response>
  ...
  it.skipIf(!hasApiKey)('should generate a post for Dairiio with safety focus', async () => {
    const actors = StaticDataRegistry.getAllActors();
...
<response>
  ...
  it.skipIf(!hasApiKey)('should generate distinct posts for different NPCs on same topic', async () => {
