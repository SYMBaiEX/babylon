...
export class GameSimulator extends EventEmitter {
  ...
  private async distributeClues(day: number): Promise<void> {
    ...
    for (const agent of recipients) {
      ...
      const cluePool = this.config.outcome ? CLUE_TEMPLATES.positive : CLUE_TEMPLATES.negative;
  ...
  private executeBet(agent: SimulatedAgent, side: 'YES' | 'NO', amount: number): void {
    // Calculate shares purchased using current price
    const shares = amount / (side === 'YES' ? this.market.yesOdds / 100 : this.market.noOdds / 100);
    ...
    this.market.yesOdds = Math.round((this.market.yesShares / totalShares) * 100);
  ...
  private async revealOutcome(): Promise<void> {
    ...
    for (const agent of this.agents) {
      ...
      const correctBets = agent.betsPlaced > 0 ? Math.floor(agent.betsPlaced * (agent.strategy === 'informed' && agent.cluesReceived > 0 ? 0.8 : 0.5)) : 0;
      agent.winningBets = correctBets;
      const pnl = (correctBets - (agent.betsPlaced - correctBets)) * (this.config.startingBalance * 0.1);
      agent.totalPnl = pnl;
      agent.balance += pnl;
    }
  }
