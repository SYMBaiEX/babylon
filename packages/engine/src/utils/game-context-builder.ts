...
import type { Actor, WorldEvent, FeedPost, Question, GroupChatMessage } from '../types/shared';
import { db, worldEvents, posts, questions as questionsTable, desc, gte, eq, and } from '@babylon/db';
import { worldFactsService } from '../world-facts-service';
import { CONTEXT_LIMITS, truncateText, truncateArray } from './context-limits';
import { shuffleArray } from './randomization';
import { extractDayFromTimestamp, extractDayFromEvent, extractDayFromPost } from './date-utils';
import { getPhaseNarrativeGuidance, getPhaseForDay, type GamePhase } from './shared-utils';
...
export async function buildRichGameContext(
  ...
  const [
    ...
    includeEventHistory
      ? db
          ...
          .from(worldEvents)
          .where(
            gameId ? eq(worldEvents.gameId, gameId) : undefined
          )
          ...
          .then(events => events.map(e => {
            ...
            return {
              ...
              actors: truncateArray((e.actors as string[]) || [], CONTEXT_LIMITS.MAX_ACTORS_PER_EVENT),
              description: truncateText(e.description || '', CONTEXT_LIMITS.MAX_EVENT_DESCRIPTION_LENGTH),
      ...
      ? db
          ...
          .where(
            and(
              ...
              gte(posts.createdAt, new Date(Date.now() - maxDays * 24 * 60 * 60 * 1000)),
            )
          )
          ...
          .then(posts => posts.map(p => {
            ...
            return {
              ...
              author: p.authorId,
              authorName: truncateText((p as { authorName?: string }).authorName || 'Unknown', 50),
              sentiment: p.sentiment ?? null,
              clueStrength: (p as { clueStrength?: number }).clueStrength ?? null,
              pointsToward: (p as { pointsToward?: boolean | null }).pointsToward ?? null,
              relatedEvent: (p as { relatedEvent?: string | null }).relatedEvent ?? null,
            } as FeedPost;
          }))
      : Promise.resolve([]),
  ...
  const eventTimeline: Array<{ day: number; events: WorldEvent[]; summary?: string }> = [];
  ...
  const activeQuestions = shuffleArray(allQuestions.filter(q => q.status === 'active'));
  const resolvedQuestions = shuffleArray(allQuestions.filter(q => q.status === 'resolved'));
  ...
  return {
    ...
    eventTimeline: truncateArray(eventTimeline, CONTEXT_LIMITS.MAX_EVENT_TIMELINE_DAYS),
    recentFeedPosts: truncateArray(shuffleArray(allPosts), CONTEXT_LIMITS.MAX_POSTS_RECENT),
    feedActivityByActor,
    trendingTopics: [],
    recentGroupMessages: [],
    activeQuestions: truncateArray(activeQuestions, CONTEXT_LIMITS.MAX_QUESTIONS_ACTIVE),
    resolvedQuestions: truncateArray(resolvedQuestions, CONTEXT_LIMITS.MAX_QUESTIONS_RESOLVED),
    ongoingNarratives: truncateArray(shuffledNarratives, CONTEXT_LIMITS.MAX_NARRATIVE_THREADS),
    characterEventHistories,
    characterPostHistories,
    worldFacts: truncateText(worldFacts.general || '', CONTEXT_LIMITS.MAX_SECTION_LENGTH),
    phase,
  };
}
...
export function formatRichGameContext(
  ...
  if (includeEventTimeline && context.eventTimeline.length > 0) {
    const timelineDays = truncateArray(context.eventTimeline, CONTEXT_LIMITS.MAX_EVENT_TIMELINE_DAYS);
    const timelineText = timelineDays
      .map(({ day, events }) => {
        ...
        const eventsText = dayEvents
          .map(e => {
            const actors = e.actors.length > 0 
            ...
            const description = truncateText(e.description, CONTEXT_LIMITS.MAX_EVENT_DESCRIPTION_LENGTH);
    ...
    const sectionText = `=== COMPLETE EVENT TIMELINE ===\n` +
  ...
  if (includeResolvedQuestions && context.resolvedQuestions.length > 0) {
    const resolvedText = truncateArray(context.resolvedQuestions, CONTEXT_LIMITS.MAX_QUESTIONS_RESOLVED)
      .map(q => {
        const text = truncateText(q.text, CONTEXT_LIMITS.MAX_QUESTION_TEXT_LENGTH);
        return `  - "${text}" → ${q.resolvedOutcome ? 'YES' : 'NO'}`;
      })
      .join('\n');
    
    const sectionText = `=== RESOLVED QUESTIONS ===\n` +
  ...
  if (includeNarrativeThreads && context.ongoingNarratives.length > 0) {
    const narrativesText = truncateArray(context.ongoingNarratives, CONTEXT_LIMITS.MAX_NARRATIVE_THREADS)
      .map(n => {
        // Shuffle actors within narrative for variety
        const actors = n.involvedActors.length > 0 
          ? ` (Actors: ${truncateArray(shuffleArray(n.involvedActors), 5).join(', ')})`
          : '';
        const questions = n.relatedQuestions.length > 0
    ...
    const sectionText = `=== ONGOING NARRATIVE THREADS ===\n` +
  ...
  if (includeFeedHistory && context.recentFeedPosts.length > 0) {
    const feedSummary = truncateArray(context.recentFeedPosts, CONTEXT_LIMITS.MAX_POSTS_RECENT)
      .map(p => {
        const day = p.day || extractDayFromTimestamp(p.timestamp);
        const content = truncateText(p.content, CONTEXT_LIMITS.MAX_POST_CONTENT_LENGTH);
        const authorName = truncateText(p.authorName, 50);
        return `  Day ${day}: @${authorName}: "${content}"`;
      })
      .join('\n');
    
    const sectionText = `=== RECENT FEED ACTIVITY ===\n` +
  ...
  if (context.worldFacts) {
    const factsText = truncateText(context.worldFacts, CONTEXT_LIMITS.MAX_SECTION_LENGTH);
    const sectionText = `=== ACCUMULATED WORLD FACTS ===\n` +
      `${factsText}\n` +
...
export function formatCharacterGameContext(
  ...
  if (includePersonalEvents) {
    ...
    if (personalEvents.length > 0) {
      ...
      const sectionText = `=== ${actor.name.toUpperCase()}'S EVENT HISTORY ===\n` +
  ...
  if (includePersonalPosts) {
    ...
    if (personalPosts.length > 0) {
      ...
      const sectionText = `=== ${actor.name.toUpperCase()}'S POST HISTORY ===\n` +
...
function extractNarrativeThreads(
  ...
  const threadsByType = new Map<string, {
    events: WorldEvent[];
    actors: Set<string>;
  ...
  return Array.from(threadsByType.entries()).map(([theme, data]) => ({
    ...
    relatedQuestions: truncateArray(Array.from(data.questions), CONTEXT_LIMITS.MAX_QUESTIONS_PER_NARRATIVE),
  }));
}
...
export function extractNarrativeThreadsWithLoopDetection(
  ...
  for (const event of events) {
    ...
    const normalizedDesc = event.description.toLowerCase().replace(/[^a-z0-9\s]/g, '').trim();
  ...
  const potentialLoops: Array<{ pattern: string; occurrences: number; warning: string }> = [];
  ...
  const narratives = Array.from(threadsByType.entries()).map(([theme, data]) => {
    ...
    return {
      ...
      involvedActors: truncateArray(Array.from(data.actors), CONTEXT_LIMITS.MAX_ACTORS_PER_NARRATIVE),
      relatedQuestions: truncateArray(Array.from(data.questions), CONTEXT_LIMITS.MAX_QUESTIONS_PER_NARRATIVE),
      recentActivity,
    };
  });
  ...
  if (potentialLoops.length > 0) {
    suggestions.push('LOOP PREVENTION: Varied content needed for these patterns:');
  ...
  const coveredQuestionIds = new Set(narratives.flatMap(n => n.relatedQuestions.map(q => String(q))));
  const uncoveredQuestions = questions.filter(q => 
    q.status === 'active' && !coveredQuestionIds.has(String(q.id))
  );
  if (uncoveredQuestions.length > 0) {
    suggestions.push(`COVERAGE GAP: ${uncoveredQuestions.length} active questions have no related events`);
  }
...
export function formatRichGameContextWithEntropy(
  ...
  if (includeEventTimeline && context.eventTimeline.length > 0) {
    ...
    sections.push({ priority: 2, content: truncateText(sectionText, CONTEXT_LIMITS.MAX_SECTION_LENGTH) });
  }
  ...
  if (includeResolvedQuestions && context.resolvedQuestions.length > 0) {
    ...
    const sectionText = `=== RESOLVED QUESTIONS (CANON OUTCOMES) ===\n` +
    ...
    sections.push({ priority: 3, content: truncateText(sectionText, CONTEXT_LIMITS.MAX_SECTION_LENGTH) });
  }
  
  // 4. ACTIVE QUESTIONS (medium priority)
  if (includeActiveQuestions && context.activeQuestions.length > 0) {
    const activeText = truncateArray(context.activeQuestions, CONTEXT_LIMITS.MAX_QUESTIONS_ACTIVE)
      .map(q => {
        const text = truncateText(q.text, CONTEXT_LIMITS.MAX_QUESTION_TEXT_LENGTH);
    ...
    const sectionText = `=== ACTIVE QUESTIONS (IN PLAY) ===\n` +
    ...
    sections.push({ priority: 4, content: truncateText(sectionText, CONTEXT_LIMITS.MAX_SECTION_LENGTH) });
  }
  ...
  if (includeNarrativeThreads && context.ongoingNarratives.length > 0) {
    ...
    sections.push({ priority: 5, content: truncateText(sectionText, CONTEXT_LIMITS.MAX_SECTION_LENGTH) });
  }
  ...
  if (includeFeedHistory && context.recentFeedPosts.length > 0) {
    ...
    sections.push({ priority: 6, content: truncateText(sectionText, CONTEXT_LIMITS.MAX_SECTION_LENGTH) });
  }
  ...
  if (includeWorldFacts && context.worldFacts) {
    ...
    sections.push({ priority: 7, content: truncateText(sectionText, CONTEXT_LIMITS.MAX_SECTION_LENGTH) });
  }
  ...
  if (randomizeSectionOrder) {
    ...
    const priorityGroups = new Map<number, Array<{ priority: number; content: string }>>();
...
export function formatDaySummaries(
  ...
  const summaries = truncateArray(context.eventTimeline, maxDays)
    .map(({ day, events }) => {
      const eventSummary = events.length > 3
        ? `${events.slice(0, 3).map(e => truncateText(e.description, 50)).join('; ')}; and ${events.length - 3} more`
...
export function buildCharacterRoster(
  ...
  const rosterEntries: CharacterRosterEntry[] = allActors.map(actor => {
    ...
    const actorRelationships = relationships
      ...
      .map(r => {
        ...
        return {
          ...
          sentiment: (r.sentiment > 0.3 ? 'ally' : r.sentiment < -0.3 ? 'rival' : 'neutral') as 'ally' | 'rival' | 'neutral',
        };
      });
    ...
    return {
      id: actor.id,
      name: actor.name,
      briefDescription: truncateText(actor.description || `${actor.name} - ${actor.domain?.join(', ') || 'notable figure'}`, 100),
      ...
      isMentionedInEvents: events.some(e => 
        e.description.toLowerCase().includes(nameLower) || 
        e.actors?.some(a => a.toLowerCase() === nameLower)
      ),
      isMentionedInQuestions: questions.some(q => 
        q.text.toLowerCase().includes(nameLower)
  ...
  const mentionedActors = rosterEntries.filter(e => 
    e.isMentionedInEvents || e.isMentionedInQuestions || e.isMentionedInTrending
  );
  const otherActors = rosterEntries.filter(e => 
    !e.isMentionedInEvents && !e.isMentionedInQuestions && !e.isMentionedInTrending
  );
...
${briefRosterLines.join('\n')}`;
  ...
  const detailedProfileLines = shuffledMentioned.map(entry => {
    ...
    const relationshipsSummary = entry.relationships && entry.relationships.length > 0
      ? `\nRELATIONSHIPS:\n${shuffleArray(entry.relationships).slice(0, 5).map(r => 
...
${relationshipsSummary}`;
  ...
  const detailedProfiles = shuffledMentioned.length > 0
...
${detailedProfileLines.join('\n---\n')}`
  ...
  return {
    briefRoster: truncateText(briefRoster, CONTEXT_LIMITS.MAX_SECTION_LENGTH * 2),
    detailedProfiles: truncateText(detailedProfiles, CONTEXT_LIMITS.MAX_SECTION_LENGTH * 2),
...
function buildFullProfile(actor: Actor): string {
  ...
  if (actor.persona) {
    ...
    if (actor.persona.reliability !== undefined) {
      personaDetails.push(`Reliability: ${(actor.persona.reliability * 100).toFixed(0)}%`);
    }
    ...
    if (actor.persona.willingToLie !== undefined) {
      personaDetails.push(`Will lie: ${actor.persona.willingToLie ? 'yes' : 'no'}`);
    }
...
export function formatCharacterAndOrgRoster(
  ...
  if (organizations && organizations.length > 0) {
    ...
    const orgSections = Array.from(orgsByType.entries()).map(([type, orgs]) => {
      ...
      const orgLines = shuffledOrgs.map(o => 
    ...
    const shuffledSections = shuffleAll ? shuffleArray(orgSections) : orgSections;
...
${shuffledSections.join('\n\n')}`;
  ...
  const allSections = [briefRoster, detailedProfiles, orgRoster].filter(Boolean);
