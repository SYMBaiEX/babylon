...
const PREDICTION_TEMPLATES = [
  { q: 'Will {company} stock reach ${target} by end of month?', desc: 'Price target prediction' },
  { q: 'Will {company} announce earnings beat this quarter?', desc: 'Earnings prediction' },
  { q: 'Will {sector} sector outperform market this week?', desc: 'Sector performance' },
  { q: 'Will {company} announce new product launch?', desc: 'Product announcement' },
];
...
const NPC_NAMES = [
  'Marcus Chen', 'Sarah Williams', 'Alex Rivera', 'Jordan Lee', 'Emma Thompson',
  'David Kim', 'Lisa Patel', 'Chris Morgan', 'Rachel Santos', 'James Wilson',
];
...
export class InMemoryStateStore implements GameStateStore {
  ...
  async getActiveQuestions(): Promise<ActiveQuestion[]> {
    return Array.from(this.questions.values()).filter((q) => q.status === 'active');
  }

  async getQuestionsToResolve(beforeTime: Date): Promise<ActiveQuestion[]> {
    return Array.from(this.questions.values()).filter(
      (q) => q.status === 'active' && q.resolutionDate && q.resolutionDate <= beforeTime
    );
  }
  ...
  async updateMarketPrice(marketId: string, yesPrice: number, noPrice: number): Promise<void> {
  ...
  async executeTrade(trade: TradeInput): Promise<TradeResult> {
    return this.buyPredictionShares(trade.actorId, trade.marketId, trade.side, trade.amount);
  }
  ...
  buyPredictionShares(userId: string, marketId: string, outcome: 'YES' | 'NO', amount: number): TradeResult {
  ...
  sellPredictionShares(userId: string, marketId: string, outcome: 'YES' | 'NO', shares: number): TradeResult {
    const user = this.users.get(userId);
    if (!user) return { success: false, error: 'User not found' };
    
    const posIndex = user.predictionPositions.findIndex(
      (p) => p.marketId === marketId && p.outcome === outcome && p.shares >= shares
    );
  ...
  openPerpPosition(userId: string, ticker: string, side: 'LONG' | 'SHORT', size: number, leverage: number): TradeResult & { positionId?: string } {
    const user = this.getOrCreateUser(userId);
    const market = this.perpMarkets.get(ticker);
    if (!market) return { success: false, error: 'Market not found' };

    const margin = size / leverage;
    if (user.balance < margin) return { success: false, error: 'Insufficient balance' };
  ...
  private updatePerpPrices(): void {
    for (const market of this.perpMarkets.values()) {
      ...
      market.priceChange24h = ((market.price - market.previousPrice) / market.previousPrice) * 100;
    }
  }
