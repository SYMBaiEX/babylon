...
export class LiquidityHealthService {
   ...
   */
  static async assessPredictionMarket(
    marketId: string
  ): Promise<PredictionMarketHealth | null> {
    ...
    const { spreadBps, effectiveBid, effectiveAsk } =
      LiquidityHealthService.calculateSpread(yesShares, noShares);

    // Calculate imbalance
    const imbalanceRatio = Math.abs(yesShares - noShares) / totalLiquidity;
    const imbalanceDirection =
      yesShares > noShares * 1.1
        ? 'yes'
        : noShares > yesShares * 1.1
          ? 'no'
          : 'balanced';

    // Calculate price impacts for various trade sizes
    const priceImpact50 = LiquidityHealthService.calculatePriceImpact(
      yesShares,
      noShares,
      50
    );
    const priceImpact100 = LiquidityHealthService.calculatePriceImpact(
      yesShares,
      noShares,
      100
    );
    const priceImpact500 = LiquidityHealthService.calculatePriceImpact(
      yesShares,
      noShares,
      500
    );
    ...
    // Assess health
    const { healthScore, warnings, recommendations } =
      LiquidityHealthService.assessPredictionHealth({
        totalLiquidity,
        imbalanceRatio,
        spreadBps,
        priceImpact100,
    ...
    const healthTier = LiquidityHealthService.getHealthTier(healthScore);
  ...
  static async assessPerpMarket(
    ...
    const imbalancePercent =
      totalOI > 0 ? (Math.abs(longOI - shortOI) / totalOI) * 100 : 0;

    // Calculate 24h volume (positions opened in last 24h)
    const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const recentPositions = positions.filter(
      (p) => new Date(p.openedAt) > dayAgo
    );
    const volume24h = recentPositions.reduce(
      (sum, p) => sum + Number(p.size),
      0
    );

    // Assess health
    const { healthScore, warnings, recommendations } =
      LiquidityHealthService.assessPerpHealth({
        totalOI,
        imbalancePercent,
        fundingRate,
  ...
  static async generateHealthReport(): Promise<MarketHealthReport> {
    ...
    for (const market of predictionMarkets) {
      const health = await LiquidityHealthService.assessPredictionMarket(
        market.id
      );
      if (health) {
    ...
    for (const company of companies) {
      const health = await LiquidityHealthService.assessPerpMarket(company.id);
      if (health) {
    ...
    const predictionSummary = {
      excellent: predictionHealths.filter((h) => h.healthTier === 'excellent')
        .length,
      ...
      critical: predictionHealths.filter((h) => h.healthTier === 'critical')
        .length,
    };

    const predictionTotalLiquidity = predictionHealths.reduce(
      (sum, h) => sum + h.totalLiquidity,
      0
    );
    const predictionAvgHealth =
      predictionHealths.length > 0
        ? predictionHealths.reduce((sum, h) => sum + h.healthScore, 0) /
          predictionHealths.length
        : 100;
    ...
    const perpTotalOI = perpHealths.reduce(
      (sum, h) => sum + h.totalOpenInterest,
      0
    );
    const perpAvgHealth =
      perpHealths.length > 0
        ? perpHealths.reduce((sum, h) => sum + h.healthScore, 0) /
          perpHealths.length
        : 100;
    ...
    for (const market of predictionAttention.filter(
      (h) => h.healthTier === 'critical'
    )) {
      criticalIssues.push(
        `Prediction "${market.question.substring(0, 50)}..." has critical liquidity issues`
      );
    }
    for (const market of perpAttention.filter(
      (h) => h.healthTier === 'critical'
    )) {
      criticalIssues.push(
        `Perp market ${market.ticker} has severe imbalance (${market.imbalancePercent.toFixed(0)}%)`
      );
    }
  ...
  private static calculateSpread(
    ...
    const buyCalc = PredictionPricing.calculateBuy(
      yesShares,
      noShares,
      'yes',
      SPREAD_CALC_SIZE
    ...
    if (yesShares > sellShares) {
      const sellCalc = PredictionPricing.calculateSell(
        yesShares,
        noShares,
        'yes',
        sellShares
      ...
      effectiveBid =
        PredictionPricing.getCurrentPrice(yesShares, noShares, 'yes') * 0.95;
    }
  ...
  private static assessPredictionHealth(params: {
    ...
    if (params.totalLiquidity < 2000) {
      healthScore -= 40;
      warnings.push(
        `Very low liquidity: ${params.totalLiquidity.toFixed(0)} points`
      );
      recommendations.push('Add more liquidity to improve trading experience');
    } else if (params.totalLiquidity < 5000) {
      healthScore -= 20;
      warnings.push(
        `Low liquidity: ${params.totalLiquidity.toFixed(0)} points`
      );
    ...
    if (params.imbalanceRatio > 0.8) {
      healthScore -= 30;
      warnings.push(
        `Extreme imbalance: ${(params.imbalanceRatio * 100).toFixed(0)}%`
      );
      recommendations.push('Market heavily skewed to one side');
    } else if (params.imbalanceRatio > 0.6) {
      healthScore -= 15;
      warnings.push(
        `High imbalance: ${(params.imbalanceRatio * 100).toFixed(0)}%`
      );
    ...
    if (params.priceImpact100 > 10) {
      healthScore -= 15;
      warnings.push(
        `High price impact for $100 trade: ${params.priceImpact100.toFixed(1)}%`
      );
  ...
  private static assessPerpHealth(params: {
    ...
    if (params.imbalancePercent > 80) {
      healthScore -= 35;
      warnings.push(
        `Extreme imbalance: ${params.imbalancePercent.toFixed(0)}%`
      );
      recommendations.push(
        'Consider positions on the minority side for funding income'
      );
    ...
    if (params.fundingRate.isSeverelyImbalanced) {
      healthScore -= 15;
      warnings.push(
        `High funding rate: ${(params.fundingRate.annualRate * 100).toFixed(1)}% APR`
      );
      const direction =
        params.fundingRate.paymentDirection === 'longs_pay'
          ? 'longs'
          : 'shorts';
