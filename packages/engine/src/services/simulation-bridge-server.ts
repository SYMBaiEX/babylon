...
class SimulationState {
  ...
  async getScenario(npcId: string): Promise<ScenarioResponse> {
    ...
    return this.createSyntheticScenario(npcId, archetype, npc);
  }
  ...
  async executeAction(request: ExecuteRequest): Promise<ExecuteResponse> {
    ...
    // Check if NPC is a bridge-generated NPC (not in registry)
    const isBridgeNpc = this.npcs.some((n) => n.id === npcId);

    if (isBridgeNpc) {
      // Use synthetic execution for bridge NPCs
      return this.executeSyntheticAction(npcId, action);
    }

    // Fall back to real execution for registry NPCs
  ...
  private executeSyntheticAction(
    ...
    return {
      ...
      events: success ? [`${action.type} executed for ${npcId}`] : [],
    };
  }
