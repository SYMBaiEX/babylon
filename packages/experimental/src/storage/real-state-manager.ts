 ...
 * This is the production version - all data is permanently stored.
 */
...
export interface RealStateManagerConfig {
  /** Enable verbose logging */
  verbose?: boolean;
}

/**
 * Manages encrypted game state and public training data using real permanent storage
 */
export class RealStateManager {
  private enclave: TEEEnclave;
  private storage: Storage;
  private checkpoints: StateCheckpoint[] = [];
  private trainingDatasets: TrainingDataset[] = [];
  private currentEpoch = 0;
  private config: RealStateManagerConfig;

  constructor(
    enclave: TEEEnclave,
    storage: Storage,
    config: RealStateManagerConfig = {}
  ) {
    ...
    if (config.verbose) {
      console.log('[RealStateManager] Initialized with permanent storage');
    }
  }
  ...
  async saveState(state: object): Promise<StateCheckpoint> {
    ...
    if (this.config.verbose) {
      console.log(
        `[RealStateManager] Uploading encrypted state (${sealedJson.length} bytes)...`
      );
    }
    ...
    if (this.config.verbose) {
      console.log(
        `[RealStateManager] ✓ Saved checkpoint v${checkpoint.version}: ${result.url}`
      );
    }
  ...
  async loadState<T = object>(id: string, keyVersion?: number): Promise<T> {
    if (this.config.verbose) {
      console.log(
        `[RealStateManager] Loading state from ${this.storage.getUrl(id)}...`
      );
    }
    ...
    if (this.config.verbose) {
      console.log(`[RealStateManager] ✓ Loaded and decrypted state from ${id}`);
    }
  ...
  async rotateKey(): Promise<StateCheckpoint> {
    if (this.config.verbose) {
      console.log('\n[RealStateManager] === INITIATING KEY ROTATION ===');
    }
    ...
    if (this.config.verbose) {
      console.log(
        `[RealStateManager] ✓ Key rotated to v${newVersion}, checkpoint: ${result.url}`
      );
    }
  ...
  async saveTrainingData(
    ...
    if (this.config.verbose) {
      console.log(
        `[RealStateManager] Uploading training data epoch ${this.currentEpoch} (${data.length} samples)...`
      );
    }
    ...
    if (this.config.verbose) {
      console.log(`[RealStateManager] ✓ Saved training data: ${result.url}`);
    }
