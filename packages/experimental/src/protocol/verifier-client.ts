...
export class VerifierClient {
  ...
  async generateReport(
    ...
    let revealData: Uint8Array | null = null;
    let hashMatches = false;

    try {
      revealData = await this.storage.download(revealStorageId);
      hashMatches = verifyDataHash(revealData, commitment.dataHash);
    } catch (error) {
      checks.push({
        name: 'Reveal Accessible',
        passed: false,
        details: `Failed to fetch reveal: ${(error as Error).message}`,
        severity: 'critical',
      });
    }
    ...
    if (revealData) {
      ...
      try {
        ...
        // If it doesn't parse as JSON, check if it looks random (encrypted)
        // High entropy data is likely encrypted
        isEncrypted = this.hasHighEntropy(revealData);
      }
   ...
   */
  private hasHighEntropy(data: Uint8Array): boolean {
    ...
    // High entropy (> 5.5 bits per byte) suggests encryption or base64
    // Pure random is ~8, base64 of encrypted data is ~6
    return entropy > 5.5;
  }
