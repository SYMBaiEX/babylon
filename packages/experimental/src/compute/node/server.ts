...
export class ComputeNodeServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/v1/chat/completions', async (c) => {
      ...
      const userAddress = c.req.header('x-babylon-address');
      const settlementNonceStr = c.req.header('x-babylon-settlement-nonce');
  ...
  private verifyAuth(c: Context): { valid: boolean; reason?: string } {
    const address = c.req.header('x-babylon-address');
    const nonce = c.req.header('x-babylon-nonce');
    const signature = c.req.header('x-babylon-signature');
    const timestamp = c.req.header('x-babylon-timestamp');
