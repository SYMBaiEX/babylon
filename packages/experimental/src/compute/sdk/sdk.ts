 ...
 * Babylon Compute SDK
 *
 * Client library for interacting with the Babylon Compute Marketplace
 */
 ...
 * Babylon Compute SDK
 */
export class BabylonComputeSDK {
  ...
  async generateAuthHeaders(
    provider: string
  ): Promise<AuthHeaders & { 'x-babylon-settlement-nonce': string }> {
    const signer = this.requireSigner();
    ...
    return {
      'x-babylon-address': signer.address,
      'x-babylon-nonce': nonce,
      'x-babylon-signature': signature,
      'x-babylon-timestamp': timestamp,
      'x-babylon-settlement-nonce': settlementNonce.toString(),
    };
  }
