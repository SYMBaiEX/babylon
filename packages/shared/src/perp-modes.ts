...
export const PERP_CONFIG: PerpModeConfig = {
  ...
  hybridBatchInterval: Number.parseInt(
    process.env.NEXT_PUBLIC_HYBRID_BATCH_INTERVAL || '3600000'
  ),
  hybridBatchSize: Number.parseInt(
    process.env.NEXT_PUBLIC_HYBRID_BATCH_SIZE || '100'
  ),
  ...
  settlementRetries: Number.parseInt(
    process.env.NEXT_PUBLIC_SETTLEMENT_RETRIES || '3'
  ),
};
...
export function validatePerpConfig(): void {
  ...
  if (
    (PERP_CONFIG.settlementMode === 'onchain' ||
      PERP_CONFIG.settlementMode === 'hybrid') &&
