...
export interface TradingPort {
  ...
  createPool(pool: Omit<PoolRecord, 'openedAt' | 'updatedAt'>): Promise<PoolRecord>;
  ...
  createPosition(position: Omit<PoolPositionRecord, 'updatedAt'>): Promise<PoolPositionRecord>;
  updatePosition(id: string, updates: Partial<PoolPositionRecord>): Promise<PoolPositionRecord>;
  ...
  createNpcTrade(trade: Omit<NpcTradeRecord, 'id' | 'createdAt'>): Promise<NpcTradeRecord>;
