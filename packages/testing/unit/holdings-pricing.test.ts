...
describe('Holdings-Based Spot Pricing', () => {
  ...
  describe('realistic scenarios', () => {
    test('10 NPCs deploy $10k each (all long)', () => {
      ...
      expect((newPrice - initialPrice) / initialPrice).toBeCloseTo(0.1, 2); // 10% increase
    });
    ...
    test('whale dominates market', () => {
      ...
      expect((newPrice - initialPrice) / initialPrice).toBeCloseTo(1.1, 2); // 110% increase
    });
  });
  ...
  describe('game scenarios', () => {
    test('all NPCs bullish on FACEHOOK', () => {
      ...
      expect(((newPrice - initialPrice) / initialPrice) * 100).toBeCloseTo(
        12,
        1
      ); // 12% gain
    });
