...
describe('Name Replacement System', () => {
  ...
  describe('Mixed Content', () => {
    it('should replace multiple names in one sentence', () => {
      ...
      expect(result).toBe(
        'AIlon Musk and Sam AIltman discussed OpenAGI at TeslAI'
      );
    });
  ...
  describe('Data Integrity', () => {
    it('should have originalFirstName for all actors', () => {
      const missingFirst = actorsData.actors.filter(
        (a: ActorData) => !a.originalFirstName
      );
    ...
    it('should have originalHandle for all actors', () => {
      const missingHandle = actorsData.actors.filter(
    ...
    it('should have originalName for all organizations', () => {
      const missingName = actorsData.organizations.filter(
  ...
  describe('No Original Names in AI Names', () => {
    it('should have AI variations in actor names', () => {
      ...
      for (const actor of actorsData.actors) {
        const fullOriginal =
          `${actor.originalFirstName} ${actor.originalLastName}`.trim();
    ...
    it('should have modified usernames from original handles', () => {
      ...
      for (const actor of actorsData.actors) {
        if (
          actor.username.toLowerCase() !== actor.originalHandle.toLowerCase()
        ) {
          modifiedCount++;
        }
      }
      ...
      it(`${file} should not contain "OpenAI"`, () => {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          const contentNoComments = content
            ...
            .replace(/\/\*[\s\S]*?\*\//g, '');
          const hasOpenAI =
            contentNoComments.includes('OpenAI') &&
            !contentNoComments.includes('OpenAGI');
