...
export class ArchetypeMatchupBenchmark {
  ...
  private buildDecisionPrompt(agent: MatchupAgent, tick: Tick): string {
    const state = tick.state as unknown as Record<string, unknown>;
    return `
Current game state:
- Timestamp: ${tick.timestamp}
- Your balance: ${state.agentBalance || 1000}
- Market prices: ${JSON.stringify(state.marketPrices || {})}
- Recent news: ${JSON.stringify(state.recentNews || [])}
...
`;
  ...
  private getMarketTrend(tick: Tick): number {
    const state = tick.state as unknown as Record<string, unknown>;
    const prices = state.marketPrices as Record<string, number> | undefined;
    if (!prices) return 0;
