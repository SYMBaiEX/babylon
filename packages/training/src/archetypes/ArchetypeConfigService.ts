...
export class ArchetypeConfigService {
   ...
   */
  static applyToAgentParams<T extends Record<string, unknown>>(
    archetypeId: string,
    baseParams: T
  ): T & {
    name: string;
    ...
    const baseMetadata =
      (baseParams as { metadata?: Record<string, unknown> }).metadata || {};
    ...
    return {
      ...
      // Store archetype in metadata
      metadata: {
        ...baseMetadata,
        archetype: archetypeId,
        archetypeTraits: config.traits,
        archetypeWeights: config.actionWeights,
