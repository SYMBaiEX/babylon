...
async function main() {
  ...
  console.log(`  Agents with performance metrics: ${result.counts.agentsWithPerformanceMetrics}`);
  ...
  const archetypeMap = new Map<string, { count: number; totalPnL: number; totalTrades: number }>();
  ...
  for (const agent of allAgents) {
    ...
    if (archetype === 'unknown') {
      ...
      const archetypes = ['trader', 'degen', 'scammer', 'social-butterfly', 'researcher', 
        'information-trader', 'goody-twoshoes', 'ass-kisser', 'perps-trader', 
        'super-predictor', 'infosec', 'liar'];
    ...
    const existing = archetypeMap.get(archetype) || { count: 0, totalPnL: 0, totalTrades: 0 };
    existing.count++;
  ...
  for (const arch of result.archetypeBreakdown) {
    console.log(`  ${arch.archetype.padEnd(20)}: ${arch.count} agents, avg PnL: $${arch.avgPnL.toFixed(2)}`);
  }
  ...
  if (result.counts.totalTrajectories > 0) {
    const trajectoryStats = await db
      .select({
        ...
        withLlmCalls: sql`COUNT(CASE WHEN ${trajectories.aiJudgeReward} IS NOT NULL THEN 1 END)`.mapWith(Number),
      })
      .from(trajectories);
    
    result.dataQuality.averageReward = trajectoryStats[0]?.avgReward ?? 0;
    result.dataQuality.averageStepsPerTrajectory = trajectoryStats[0]?.avgSteps ?? 0;
    result.dataQuality.trajectoriesWithRewards = trajectoryStats[0]?.withLlmCalls ?? 0;
  }
  ...
  if (result.counts.totalTrajectories === 0) {
    result.summary.issues.push('No trajectories recorded - agents may not have recordTrajectories enabled');
  }
  if (result.counts.scoredTrajectories === 0 && result.counts.totalTrajectories > 0) {
    result.summary.issues.push('Trajectories exist but none are scored - run "babylon train score"');
  }
  ...
  if (result.counts.totalAgents === 0) {
    result.summary.recommendations.push('Create agents: babylon agent spawn --count 5');
    result.summary.recommendations.push('Enable agents: babylon agent enable --all');
  }
  if (result.counts.totalTrajectories === 0 && result.counts.totalAgents > 0) {
    result.summary.recommendations.push('Run agents with trajectory recording: babylon train parallel -a trader -n 3 -t 50');
  }
  if (result.counts.scoredTrajectories === 0 && result.counts.totalTrajectories > 0) {
    result.summary.recommendations.push('Score trajectories: babylon train score');
  }
  if (result.counts.trainingBatches === 0 && result.counts.scoredTrajectories > 0) {
    result.summary.recommendations.push('Export and train: babylon train archetype -a trader');
  }
