...
import { and, count, db, desc, eq, isNotNull, trajectories } from '@babylon/db';
...
// Test 2: Check for Real Data (not synthetic)
async function testRealDataExists(): Promise<{
  ...
  if (recent.length === 0) {
    return {
      passed: false,
      message: 'No trajectories found in database',
    };
  }
  ...
  for (const traj of recent) {
    const isSynthetic =
      traj.agentId.match(/agent-[a-z]+-\d+/) !== null ||
      (traj.scenarioId && traj.scenarioId.startsWith('multi-archetype'));
  ...
  if (realCount === 0) {
    return {
      passed: false,
      message: `All ${recent.length} trajectories appear to be synthetic. Run "babylon train parallel" to generate real data.`,
    };
  }

  if (syntheticCount > realCount) {
    return {
      passed: false,
      message: `Found ${syntheticCount} synthetic vs ${realCount} real trajectories. Clean synthetic data with "bun run packages/training/scripts/clean-synthetic-data.ts"`,
    };
  }

  return {
    passed: true,
    message: `Found ${realCount} real trajectories (${syntheticCount} synthetic)`,
  };
}

// Test 3: Check Trajectory Quality
async function testTrajectoryQuality(): Promise<{
  ...
  return {
    passed: true,
    message: `${validStepsCount}/${recent.length} have valid steps, ${hasActionsCount}/${recent.length} have multiple actions`,
  };
}
