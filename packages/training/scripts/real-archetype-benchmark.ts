...
async function main() {
  ...
  for (const agent of agents) {
    ...
    const openPositionsList = await db
      ...
      .where(
        and(
          eq(poolPositions.userId, agent.id),
    ...
    for (const a of ARCHETYPES) {
      if (lowerName.includes(a.replace('-', '').toLowerCase()) || 
  ...
  console.log('Archetype            | Agents | Avg PnL      | Win Rate | Reputation');
  console.log('─'.repeat(70));
  
  const benchmarkResults: Array<{archetype: string; count: number; avgPnL: number; avgWinRate: number; avgReputation: number}> = [];
  
  for (const [archetype, agentsList] of grouped) {
    const count = agentsList.length;
    const avgPnL = count > 0 ? agentsList.reduce((sum, a) => sum + a.lifetimePnL, 0) / count : 0;
    const avgWinRate = count > 0 ? agentsList.reduce((sum, a) => sum + a.winRate, 0) / count : 0;
    const avgReputation = count > 0 ? agentsList.reduce((sum, a) => sum + a.reputationPoints, 0) / count : 0;

    benchmarkResults.push({ archetype, count, avgPnL, avgWinRate, avgReputation });
  }
