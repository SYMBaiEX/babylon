...
class FastRolloutGenerator:
    ...
    def _calculate_tick_reward(
        ...
        if tick_data.action and tick_data.action.success:
            reward += 0.1
        
        # Reward for P&L improvement
        pnl_delta = tick_data.feedback.get("pnl_delta", 0.0)
        reward += pnl_delta / 1000.0  # Normalize
        
        # Penalty for errors
        if tick_data.action and tick_data.action.error:
            reward -= 0.1
...
class RolloutQualityValidator:
    ...
    def validate_rollout(result: RolloutResult) -> tuple[bool, list[str]]:
        ...
        if result.total_llm_calls < result.ticks_completed:
            issues.append(f"Low LLM call rate: {result.total_llm_calls} calls for {result.ticks_completed} ticks")
