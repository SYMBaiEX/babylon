...
class BabylonEnvConfig(BaseEnvConfig):
    ...
    judge_model: str = Field(
        default="gpt-4o-mini",
        description="Model to use for LLM judge scoring"
    )
...
class BabylonRLAIFEnv(BaseEnv):
    ...
    async def collect_trajectories(self, item: Tuple) -> Tuple[Optional[ScoredDataGroup], List]:
        """
        Collect and score trajectories using RLAIF (LLM-as-judge)
        
        This is the core RLAIF implementation:
        ...
        async with self.server.managed_server(tokenizer=self.tokenizer) as managed:
            for traj in trajectory_group:
                ...
                if len(self.tokenizer.apply_chat_template(messages)) > self.config.max_token_length - 512:
                    ...
                    messages = [messages[0]] + messages[-(self.config.max_steps_per_trajectory * 2):]
...
You receive market updates and must analyze, reason, and then act."""
        ...
        for step_idx, step in enumerate(steps):
            ...
            if llm_calls:
                ...
                # Less rich, but still captures the decision
                env_state = step.get('environmentState', step.get('environment_state', {}))
                balance = env_state.get('agentBalance', env_state.get('agent_balance', 0))
                pnl = env_state.get('agentPnL', env_state.get('agent_pnl', 0))
                positions = env_state.get('openPositions', env_state.get('open_positions', 0))
    ...
    def _trajectory_to_multi_prompt_samples(self, traj: Dict) -> List[Dict]:
        """
        Extract ALL individual prompt samples from a trajectory.
        
        Each LLM call becomes a separate training sample with:
        - The prompt context
        - The response
        - The trajectory-level score as reward signal
        
        This allows training on each prompt type (reasoning, action, etc.)
        """
        samples = []
        steps = traj.get('steps', [])
        ...
        for step_idx, step in enumerate(steps):
            ...
            for call_idx, llm_call in enumerate(llm_calls):
                sample = {
                    ...
                    "temperature": llm_call.get("temperature", 0.7),
                    "environment_state": step.get("environmentState", step.get("environment_state", {})),
                    "action": step.get("action", {}),
                    "step_reward": step.get("reward", 0.0),
                }
                
                # Only include samples with actual content
                if sample["user_prompt"] and sample["response"]:
    ...
    async def _score_with_judge(self, rollout_data: List[Dict]) -> Optional[ScoredDataGroup]:
        """
        Score rollouts using LLM-as-judge (RLAIF)
        
        The judge compares trajectories and assigns relative scores.
        """
        # Build judge prompt
        judge_prompt = self._build_judge_prompt(rollout_data)
        
        # Call judge model
        judge_response = await self._call_judge(judge_prompt)
        
        if not judge_response:
            logger.error("Judge returned no response")
            return None
            
        # Parse scores from judge response
        scores = self._parse_judge_scores(judge_response, len(rollout_data))
        
        if not scores or len(scores) != len(rollout_data):
            logger.error(f"Score count mismatch: got {len(scores) if scores else 0}, expected {len(rollout_data)}")
            # Fallback to heuristic scoring based on P&L
            scores = self._heuristic_scores(rollout_data)
        ...
        for i, rollout in enumerate(rollout_data):
            ...
            scored_group["scores"].append(scores[i])
            scored_group["inference_logprobs"].append(rollout["logprobs"])
            
        # Store sample for wandb logging
        if len(rollout_data) >= 2:
            self.judgement_samples.append((
                str(rollout_data[0]["trajectory"].get("final_pnl", 0)),
                str(rollout_data[1]["trajectory"].get("final_pnl", 0)),
                judge_response[:500] if judge_response else "No response"
            ))
            
        return scored_group
    
    def _build_judge_prompt(self, rollout_data: List[Dict]) -> str:
        """Build prompt for LLM judge"""
        prompt_parts = [
            "# Trading Agent Evaluation Task\n",
        ...
        for i, rollout in enumerate(rollout_data):
            ...
            prompt_parts.append(f"- Episode Length: {traj.get('episode_length', 0)} steps")
            prompt_parts.append(f"- Total Reward: {traj.get('total_reward', 0):.2f}")
            
            # Summarize key actions
            steps = traj.get('steps', [])
            actions = [s.get('action', {}).get('actionType', 'unknown') for s in steps if isinstance(s, dict)]
            action_summary = {}
            for a in actions:
                action_summary[a] = action_summary.get(a, 0) + 1
            prompt_parts.append(f"- Actions: {dict(action_summary)}")
            
        prompt_parts.append("\n## Output Format:")
        prompt_parts.append("Respond with ONLY a valid JSON object:")
        prompt_parts.append("""{
  "reasoning": "Brief explanation of your scoring rationale",
  "scores": [
    {"trajectory_id": 1, "score": 0.85},
    {"trajectory_id": 2, "score": 0.65}
  ]
}""")
    ...
    async def _call_judge(self, prompt: str) -> Optional[str]:
        """Call the LLM judge to score trajectories using OpenAI API"""
        messages = [
            {
                "role": "system",
                "content": "You are an expert evaluator of trading agent performance. Respond ONLY with valid JSON."
            },
            {
                "role": "user", 
                "content": prompt
    ...
    def _parse_judge_scores(self, response: str, expected_count: int) -> Optional[List[float]]:
        """Parse scores from judge response"""
        try:
            # Clean response
            ...
            json_match = None
            for start_char in ['{', '[']:
                if start_char in clean_response:
                    start = clean_response.find(start_char)
                    end = clean_response.rfind('}' if start_char == '{' else ']') + 1
                    if end > start:
                        json_match = clean_response[start:end]
                        break
                        
            if not json_match:
                logger.error("No JSON found in judge response")
                return None
                
            parsed = json.loads(json_match)
            
            # Handle different response formats
            scores_data = parsed.get("scores", parsed) if isinstance(parsed, dict) else parsed
            ...
            if isinstance(scores_data, list):
                ...
                for item in scores_data:
                    if isinstance(item, (int, float)):
                        ...
                        score = item.get("score", item.get("value", 0.5))
    ...
    def _heuristic_scores(self, rollout_data: List[Dict]) -> List[float]:
        """Fallback heuristic scoring based on P&L"""
        pnls = [r["trajectory"].get("final_pnl", 0) for r in rollout_data]
        
        # Normalize P&L to 0-1 range
        min_pnl = min(pnls)
        max_pnl = max(pnls)
    ...
    async def evaluate(self, *args, **kwargs):
        ...
        if eval_results:
            ...
            logger.info(f"Evaluation complete: {len(eval_results)} groups, avg P&L: ${overall_pnl:.2f}")
