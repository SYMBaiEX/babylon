...
// ONLY import plugins that ACTUALLY have dist/index.js (verified with ls)
import { autonomyPlugin } from '../../plugin-autonomy/dist/index.js';
import { bootstrapPlugin } from '../../plugin-bootstrap/dist/index.js';
import { experiencePlugin } from '../../plugin-experience/dist/index.js';
import PersonalityPlugin from '../../plugin-personality/dist/index.js';
import shellPlugin from '../../plugin-shell/dist/index.js';
import { plugin as sqlPlugin } from '../../plugin-sql/dist/index.js';  // CRITICAL - database adapter
import { trajectoryLoggerPlugin } from '../../plugin-trajectory-logger/dist/index.js';
import TodoPlugin from '../../plugin-todo/dist/index.js';
// import evmPlugin from '@elizaos/plugin-evm';
import predictionMarketsPlugin from '../../../../../plugin-babylon/dist/index.js';
...
import { erc8004Plugin, autoRegisterToRegistry } from './erc8004';
...
const allAvailablePlugins: Record<string, Plugin> = {
  ...
  erc8004: erc8004Plugin,
  // evm: evmPlugin,
  trajectoryLogger: trajectoryLoggerPlugin,
  babylon: predictionMarketsPlugin,
};

// Create initial plugin list with only successfully-built plugins
function createInitialPlugins(): Plugin[] {
  return [
    sqlPlugin,        // MUST be first - database adapter
    trajectoryLoggerPlugin, // Must be early to capture all events
    bootstrapPlugin,
    gameAPIPlugin,
    predictionMarketsPlugin,  // Babylon game integration (Agent0 + A2A)
    autonomyPlugin,
    PersonalityPlugin,
    experiencePlugin,
    shellPlugin,
    TodoPlugin,       // Task management
    erc8004Plugin,
    // evmPlugin,        // Blockchain interactions
  ].filter(Boolean);
}
...
export async function startAgent(character: Character): Promise<IAgentRuntime> {
  ...
  // Create the runtime using ElizaAgentRuntime with initial plugins AND database config
  // CRITICAL: Must pass serverUrl for PostgreSQL, otherwise defaults to PGLite!
  const serverUrl = process.env.DATABASE_URL || process.env.POSTGRES_URL || 
    'postgresql://eliza:eliza_secure_pass@localhost:5433/eliza';
    
  const runtime = new ElizaAgentRuntime({
    agentId,
    character: cleanCharacter,
    plugins: initialPlugins,
    serverUrl,  // THIS IS CRITICAL - forces PostgreSQL instead of PGLite
  });
...
export async function startServer() {
  ...
  const postgresHost = isContainer ? 'eliza-postgres:5432' : 'localhost:5433';  // Using :5433 for local
  ...
  // Load and start ALL Crucible agents (hacker, scammer, citizen, guardian, player)
  console.log('[BACKEND] 🔥 Loading Crucible multi-agent team...');
  ...
  for (const charFile of characterFiles) {
    try {
      ...
      console.log(`[BACKEND] Loading ${charFile} from ${charPath}...`);
      
      if (fs.existsSync(charPath)) {
        const characterData = JSON.parse(fs.readFileSync(charPath, 'utf-8'));
        const runtime = await startAgent(characterData);
        await server.registerAgent(runtime);
        runtimes.push(runtime);
        console.log(`[BACKEND] ✅ ${characterData.name} (${charFile}) started successfully`);
      } else {
        console.warn(`[BACKEND] ⚠️  Character file not found: ${charPath}`);
      }
    } catch (error) {
      console.error(`[BACKEND] ❌ Failed to start agent from ${charFile}:`, error);
      // Continue with other agents even if one fails
    }
  }
  
  console.log(`[BACKEND] ✅ ${runtimes.length}/5 Crucible agents started and registered successfully`);

  // Test the shell service to ensure it's working properly (using first agent)
  console.log('[BACKEND] Testing shell service...');
  try {
    const testRuntime = runtimes[0]; // Use first agent for testing
    if (!testRuntime) {
      console.error('[BACKEND] ❌ No agents available for shell service test');
      return server;
    }
    const shellService = testRuntime.getService('SHELL');
    if (!shellService) {
