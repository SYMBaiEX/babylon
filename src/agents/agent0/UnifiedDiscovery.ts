...
import type { IUnifiedDiscoveryService, DiscoveryFilters, IReputationBridge } from './types'
...
export class UnifiedDiscoveryService implements IUnifiedDiscoveryService {
  ...
  async discoverAgents(filters: DiscoveryFilters): Promise<AgentProfile[]> {
    ...
    try {
      // 1. Search local registry (existing Babylon agents)
      ...
      // 2. Search Agent0 network (if enabled and requested)
      if (filters.includeExternal && process.env.AGENT0_ENABLED === 'true') {
        try {
          ...
          // Transform Agent0 results to AgentProfile format
          for (const agent0Data of externalAgents) {
            try {
              ...
              logger.warn(
                `Failed to transform Agent0 agent ${agent0Data.tokenId}:`,
                error,
                'UnifiedDiscovery'
              )
            }
          ...
          logger.error('Failed to search Agent0 network:', error, 'UnifiedDiscovery')
          // Continue with local results only
        }
      }
      
      // 3. Deduplicate and sort by reputation
      return this.deduplicateAndSort(results)
    } catch (error) {
      logger.error('Failed to discover agents:', error, 'UnifiedDiscovery')
      return []
    }
  }
  ...
  private async transformAgent0Profile(
    ...
    try {
      ...
      // Get aggregated reputation if ReputationBridge available
      let reputation
      if (reputationBridge) {
        try {
          ...
          // Fallback to basic reputation if aggregation fails
        ...
        // No reputation bridge - use basic reputation
        reputation = agent0Data.reputation ? {
          totalBets: agent0Data.reputation.totalBets || 0,
          winningBets: agent0Data.reputation.winningBets || 0,
          accuracyScore: (agent0Data.reputation.accuracyScore || 0) / 100,
      ...
      return {
        agentId: `agent0-${agent0Data.tokenId}`,  // Mark as external agent
      ...
      logger.error('Failed to transform Agent0 profile:', error, 'UnifiedDiscovery')
      return null
    }
  }
...
export function getUnifiedDiscoveryService(): UnifiedDiscoveryService {
  if (!unifiedDiscoveryInstance) {
    ...
    // Initialize ReputationBridge if enabled
    let reputationBridge: ReputationBridge | null = null
    if (process.env.AGENT0_ENABLED === 'true') {
      try {
        // RegistryClient may not be available, pass null for graceful handling
        reputationBridge = new ReputationBridge(null)
      } catch (error) {
        logger.warn('Failed to initialize ReputationBridge', { error }, 'UnifiedDiscovery')
      }
    }
