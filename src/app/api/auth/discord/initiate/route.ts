...
export async function GET(request: NextRequest) {
  ...
  authUrl.searchParams.set('redirect_uri', `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/discord/callback`)
  authUrl.searchParams.set('scope', 'identify guilds')
  authUrl.searchParams.set('state', state)

  logger.info('Initiating Discord OAuth', { userId, state: state.substring(0, 20) + '...' }, 'DiscordInitiate')
