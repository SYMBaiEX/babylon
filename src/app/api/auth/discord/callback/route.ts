...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  const parsed = DiscordCallbackQuerySchema.safeParse(Object.fromEntries(searchParams));
  ...
  if (!parsed.success) {
    return NextResponse.redirect(
      new URL(`${OAUTH_REDIRECT_PATH}?error=${encodeURIComponent('Invalid parameters received from Discord')}`, baseUrl)
    )
  }
  ...
  if (oauthError) {
    ...
    return NextResponse.redirect(
      new URL(`${OAUTH_REDIRECT_PATH}?error=${encodeURIComponent('Discord authentication failed')}`, baseUrl)
    )
  }

  if (!code || !state) {
    logger.warn('Discord callback missing code or state', { hasCode: !!code, hasState: !!state }, 'DiscordCallback')
    return NextResponse.redirect(
  ...
  if (stateParts.length < 2) {
    logger.warn('Discord callback invalid state format', { state }, 'DiscordCallback')
    return NextResponse.redirect(
  ...
  if (!userId || !timestampStr) {
    logger.warn('Discord callback missing userId or timestamp in state', { state }, 'DiscordCallback')
    return NextResponse.redirect(
      new URL(`${OAUTH_REDIRECT_PATH}?error=invalid_state`, baseUrl)
    )
  }
  ...
  if (isNaN(stateTimestamp)) {
    logger.warn('Discord callback invalid timestamp in state', { state, timestampStr }, 'DiscordCallback')
    return NextResponse.redirect(
      new URL(`${OAUTH_REDIRECT_PATH}?error=invalid_state`, baseUrl)
    )
  }
  ...
  if (now - stateTimestamp > 10 * 60 * 1000) {
    logger.warn('Discord callback state expired', { stateTimestamp, now, ageMs: now - stateTimestamp }, 'DiscordCallback')
    return NextResponse.redirect(
  ...
  if (!oauthState) {
    logger.warn('Discord callback missing or expired state', { 
      state, 
      userId, 
    }, 'DiscordCallback')
  ...
  await db.oAuthState.delete({
    ...
    logger.warn('Failed to delete OAuth state', { error, stateId: oauthState.id }, 'DiscordCallback')
  })

  if (!tokenResponse.ok) {
    const errorData = await tokenResponse.text()
    logger.error('Failed to exchange Discord code', { errorData }, 'DiscordCallback')
    return NextResponse.redirect(
  ...
  if (!userData.id || !userData.username) {
    logger.error('Invalid Discord user data received', { userData }, 'DiscordCallback')
    return NextResponse.redirect(
  ...
  const pointsResult = await PointsService.awardDiscordLink(userId, discordUsername)
