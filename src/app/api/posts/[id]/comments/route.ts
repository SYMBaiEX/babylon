...
import { db, posts, comments, users, reactions, eq, and, asc, count, inArray } from '@/db';
...
export const GET = withErrorHandling(async (
  ...
  const likeCountMap = new Map(likeCounts.map(l => [l.commentId, Number(l.count)]));

  // Get user's likes if authenticated
  const userLikes = new Set<string>();
  if (canonicalUserId && commentIds.length > 0) {
    const userLikesResult = await db.select({ commentId: reactions.commentId })
      .from(reactions)
      .where(and(
        inArray(reactions.commentId, commentIds),
  ...
  const commentsWithData: CommentWithUser[] = commentsResult.map(comment => ({
    id: comment.id,
    content: comment.content,
...
export const POST = withErrorHandling(async (
  ...
  if (!post) {
    ...
    const [existingPost] = await db.select({ id: posts.id, deletedAt: posts.deletedAt })
      .from(posts)
      .where(eq(posts.id, postId))
      .limit(1);
  ...
  if (parentCommentId) {
    const [parentComment] = await db.select({ id: comments.id, postId: comments.postId })
      .from(comments)
  ...
  const [postRecord] = await db.select({ id: posts.id, authorId: posts.authorId })
    .from(posts)
    .where(eq(posts.id, postId))
    .limit(1);
  ...
  if (!newComment) {
    throw new BusinessLogicError('Failed to create comment', 'COMMENT_CREATION_FAILED');
  }
  ...
  if (parentCommentId) {
    ...
    const [parentComment] = await db.select({ authorId: comments.authorId })
      .from(comments)
      .where(eq(comments.id, parentCommentId))
      .limit(1);
    ...
    if (
      ...
      const [postAuthorUser] = await db.select({ id: users.id })
        .from(users)
  ...
  logger.info('Comment created successfully', { postId, userId: canonicalUserId, commentId: newComment.id, parentCommentId }, 'POST /api/posts/[id]/comments');
