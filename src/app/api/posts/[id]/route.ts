...
import { db, posts, users, actors, organizations, reactions, comments, shares, eq, and, lte, count, isNull } from '@/db';
...
export const GET = withErrorHandling(async (
  ...
  if (!post) {
    ...
    if (gamePost) {
      ...
      const [[likeCountResult], [commentCountResult], [shareCountResult]] = await Promise.all([
      ...
      // Get author info
      const [actor] = await db.select({ name: actors.name })
        .from(actors)
        .where(eq(actors.id, gamePost.authorId))
        .limit(1);

      const [userRecord] = actor ? [] : await db.select({ 
      ...
      if (originalPostIdFromGame) {
        const [originalPost] = await db.select({ authorId: posts.authorId, content: posts.content })
          .from(posts)
        ...
        if (originalPost) {
          ...
          const [originalActor] = !originalUser ? await db.select({
            id: actors.id,
            name: actors.name,
          ...
          const [originalOrg] = !originalUser && !originalActor ? await db.select({
            id: organizations.id,
      ...
      if (parts.length >= 3 && parts[1]) {
        ...
        if (!isNaN(timestampNum) && timestampNum > 1000000000000) {
          ...
          if (parts.length >= 4 && parts[2] && !parts[2].includes('.')) {
            ...
            const [actorRecord] = await db.select({ id: actors.id })
              .from(actors)
    ...
    if (actorRecord) {
      ...
      authorProfileImageUrl = actorRecord.profileImageUrl || `/images/actors/${actorRecord.id}.jpg`;
    } else {
      ...
      if (userRecord) {
        authorName = userRecord.displayName || userRecord.username || createdPost.authorId;
    ...
    return successResponse({
      data: {
        ...
        biasScore: createdPost.biasScore !== undefined ? createdPost.biasScore : null,
        ...
        isActorPost: true,
        timestamp: createdPost.timestamp ? createdPost.timestamp.toISOString() : createdPost.createdAt.toISOString(),
  ...
  if (post.originalPostId) {
    const [originalPost] = await db.select()
      .from(posts)
      .where(and(
        eq(posts.id, post.originalPostId),
