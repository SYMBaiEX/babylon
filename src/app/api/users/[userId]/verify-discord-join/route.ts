...
export const POST = withErrorHandling(async (
  ...
  if (!authUser.dbUserId) {
    throw new AuthorizationError('User profile not found. Please complete onboarding first.', 'discord-join-verification', 'create');
  }
  ...
  if (authUser.dbUserId !== canonicalUserId) {
    throw new AuthorizationError('You can only verify your own Discord membership', 'discord-join-verification', 'create');
  }
  ...
  if (!user?.discordUsername || !user?.discordId) {
    throw new BusinessLogicError('Please link your Discord account first to verify join.', 'DISCORD_NOT_LINKED');
  }

  if (!user.discordAccessToken) {
    throw new BusinessLogicError('Discord access token not found. Please re-link your Discord account.', 'DISCORD_TOKEN_MISSING');
  }
  ...
  if (!BABYLON_DISCORD_GUILD_ID) {
    throw new BusinessLogicError('Discord verification is not configured. Please contact support.', 'DISCORD_GUILD_NOT_CONFIGURED');
  }
  ...
  try {
    logger.info(
      ...
      { userId: canonicalUserId, discordId: user.discordId, guildId: BABYLON_DISCORD_GUILD_ID },
      'POST /api/users/[userId]/verify-discord-join'
    );
    ...
    if (discordResponse.ok) {
      const guilds: Array<{ id: string; name: string }> = await discordResponse.json();
      ...
      const babylonGuild = guilds.find(guild => guild.id === BABYLON_DISCORD_GUILD_ID);
      ...
      if (babylonGuild) {
        ...
        verificationError = 'You are not a member of the Babylon Discord server. Please join first.';
        logger.warn(
          `User is not a member of Babylon Discord`,
          { userId: canonicalUserId, discordId: user.discordId, guildsChecked: guilds.length },
          'POST /api/users/[userId]/verify-discord-join'
        );
      }
    } else if (discordResponse.status === 401) {
      const errorBody = await discordResponse.text().catch(() => '');
      verificationError = 'Discord authentication failed. Please re-link your Discord account.';
      logger.warn(
      ...
      logger.error(
        `Discord API error`,
        { userId: canonicalUserId, discordId: user.discordId, status: discordResponse.status, error: errorText },
        'POST /api/users/[userId]/verify-discord-join'
      );
    }
  } catch (error) {
    verificationError = 'Failed to verify with Discord API. Please try again later.';
    logger.error(
      `Discord API verification exception`,
      { userId: canonicalUserId, discordId: user.discordId, error: error instanceof Error ? error.message : String(error) },
      'POST /api/users/[userId]/verify-discord-join'
    );
  }
  ...
  if (!isMember) {
    return successResponse({
      verified: false,
      message: verificationError || 'Could not verify membership. Please ensure you have joined the Babylon Discord server.',
      points: {
        awarded: 0,
        newTotal: 0,
      },
  ...
  if (!alreadyAwarded) {
    ...
    const pointsResult = await PointsService.awardDiscordJoin(canonicalUserId, user.discordUsername);
