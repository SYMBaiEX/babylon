...
export const POST = withErrorHandling(async (
  ...
  if (!authUser.dbUserId) {
    throw new AuthorizationError('User profile not found. Please complete onboarding first.', 'farcaster-follow-verification', 'create');
  }
  ...
  if (authUser.dbUserId !== canonicalUserId) {
    throw new AuthorizationError('You can only verify your own Farcaster follow', 'farcaster-follow-verification', 'create');
  }
  ...
  if (!user?.farcasterUsername && !user?.farcasterFid) {
    throw new BusinessLogicError('Please link your Farcaster account first to verify follow.', 'FARCASTER_NOT_LINKED');
  }
  ...
  if (!process.env.NEYNAR_API_KEY) {
    throw new BusinessLogicError('Farcaster verification is not configured. Please contact support.', 'NEYNAR_NOT_CONFIGURED');
  }
  ...
  try {
    ...
    if (neynarResponse.ok) {
      ...
      if (neynarData.users && neynarData.users.length > 0) {
        ...
        if (viewerContext && viewerContext.followed_by) {
          ...
          verificationError = 'You are not following @playbabylon on Farcaster. Please follow first.';
          logger.warn(
        ...
        verificationError = 'User not found on Farcaster. Please re-link your account.';
        logger.warn(
      ...
      verificationError = 'User not found on Farcaster. Please check your account.';
      logger.warn(
      ...
      logger.error(
        `Neynar API error`,
        { userId: canonicalUserId, userFid, status: neynarResponse.status, error: errorText },
        'POST /api/users/[userId]/verify-farcaster-follow'
      );
    }
  } catch (error) {
    ...
    logger.error(
      ...
      { userId: canonicalUserId, userFid, error: error instanceof Error ? error.message : String(error) },
      'POST /api/users/[userId]/verify-farcaster-follow'
    );
  }
  ...
  if (!isFollowing) {
    return successResponse({
      verified: false,
      message: verificationError || 'Could not verify follow. Please ensure you are following @playbabylon on Farcaster.',
      points: {
        awarded: 0,
        newTotal: 0,
      },
  ...
  if (!alreadyAwarded) {
    ...
    const pointsResult = await PointsService.awardFarcasterFollow(canonicalUserId);
