...
import { chats, chatParticipants, messages, users, actors, userGroups, eq, inArray, desc, asc } from '@/db';
...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  const { chatsList, allUsers, allActors, allUserGroups } = await asSystem(async (database) => {
    // Get all group chats
    const chatsList = await database.select()
      .from(chats)
      .where(eq(chats.isGroup, true))
      .orderBy(sortOrder === 'asc' ? asc(chats.createdAt) : desc(chats.createdAt));

    // Get all chat IDs
    const chatIds = chatsList.map(c => c.id);

    // Get all participants for these chats
    const participantsList = chatIds.length > 0
    ...
    const messagesList = chatIds.length > 0
      ? await database.select({
          id: messages.id,
    ...
    const allActors = allUserIds.length > 0
      ? await database.select({
          id: actors.id,
          name: actors.name,
    ...
    // Get all user groups
    const allUserGroups = await database.select({
    ...
    messagesList.forEach(m => {
      ...
      if (list.length < 10) { // Keep only last 10 messages per chat
        list.push(m);
      }
    ...
    return { chatsList: chatsWithRelations, allUsers, allActors, allUserGroups };
  }, 'admin-groups');
  ...
  const enrichedChats = chatsList.map((chat) => {
    ...
    const usersInChat = participantIds.map(id => usersMap.get(id)).filter((u): u is NonNullable<typeof u> => u !== undefined);
    const actorsInChat = participantIds.map(id => actorsMap.get(id)).filter((a): a is NonNullable<typeof a> => a !== undefined);
    ...
    if (hasNPCs && !hasUsers) {
      ...
      const creator = actorsInChat.find(a => chat.name?.includes(a.name)) || actorsInChat[0];
      if (creator) {
