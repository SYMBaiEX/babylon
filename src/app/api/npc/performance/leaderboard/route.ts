...
import { db, pools, actors, poolPositions, gte, desc, and, isNull, inArray } from '@/db'
...
export async function GET(request: Request) {
  ...
  const poolsList = await db
    ...
    .where(and(
      ...
      gte(pools.totalValue, String(minValue))
    ))
  ...
  const positionsList = poolIds.length > 0
    ? await db
        .select({
  ...
  const leaderboard = poolsList.map((pool, index) => {
    ...
    const initialValue = parseFloat(pool.totalDeposits?.toString() || '0')
