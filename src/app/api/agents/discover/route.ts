 ...
 *     description: Find agents by type, status, skills, domains, and capabilities
 *     parameters:
 *       - in: query
 *         name: types
 *         schema:
...
export async function GET(req: NextRequest) {
  try {
    ...
    const agentCards = agents.map(agent => ({
      ...
      endpoints: {
        a2a: agent.capabilities.a2aEndpoint || `${baseUrl}/api/agents/${agent.agentId}/a2a`,
        mcp: agent.capabilities.mcpEndpoint || `${baseUrl}/api/agents/${agent.agentId}/mcp`,
    ...
    logger.info(`Discovered ${agentCards.length} agents`, {
