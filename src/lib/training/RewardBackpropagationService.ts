...
import { db, trajectories, marketOutcomes, eq, and } from '@/db';
...
export class RewardBackpropagationService {
  ...
  async updateRewardsForWindow(windowId: string): Promise<number> {
    ...
    const trajectoriesResult = await db.select({
      id: trajectories.id,
  ...
  async processPendingWindows(): Promise<number> {
    ...
    const windowsWithOutcomes = await db.selectDistinct({ windowId: marketOutcomes.windowId })
      .from(marketOutcomes);
