...
import { db, trainedModels, users, eq } from '@/db';
...
export class ModelDeployer {
  ...
  private async getTargetAgents(options: DeploymentOptions) {
    const agents = await db.select({ id: users.id, displayName: users.displayName })
      .from(users)
    ...
    switch (options.strategy) {
      ...
      case 'test':
        if (options.testAgentIds) {
          return agents.filter((a: typeof agents[number]) => options.testAgentIds!.includes(a.id));
        }
