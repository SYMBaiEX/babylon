...
export class ContentValidator {
  ...
  static validatePostContent(content: unknown, context?: string): asserts content is string {
    ...
    if (content.length > this.MAX_POST_LENGTH) {
      logger.warn(`${ctx}: content exceeds max length`, {
        length: content.length,
        max: this.MAX_POST_LENGTH,
      }, 'ContentValidator');
      throw new Error(`${ctx}: content exceeds maximum length (${this.MAX_POST_LENGTH} chars)`);
    }
  }
   ...
   */
  static validateEventDescription(description: unknown, context?: string): asserts description is string {
    ...
    if (description.length > this.MAX_EVENT_DESCRIPTION) {
      logger.warn(`${ctx}: description too long, will truncate`, {
        length: description.length,
        max: this.MAX_EVENT_DESCRIPTION,
      }, 'ContentValidator');
  ...
  static validateQuestionText(text: unknown, context?: string): asserts text is string {
    ...
    if (text.length > this.MAX_QUESTION_TEXT) {
      throw new Error(`${ctx}: text exceeds maximum length (${this.MAX_QUESTION_TEXT} chars)`);
    }
  }
  ...
  static truncateContent(content: string, maxLength: number): string {
    ...
    logger.warn('Truncating content', {
