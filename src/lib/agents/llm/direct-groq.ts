...
export async function callGroqDirect(params: {
  ...
  if (params.runtime) {
    ...
    const wandbApiKey = params.runtime.getSetting('WANDB_API_KEY') as string | undefined
    const wandbModel = params.runtime.getSetting('WANDB_MODEL') as string | undefined
    ...
    if (wandbEnabled && wandbApiKey && wandbModel) {
      ...
      logger.info('Using trained W&B model for agent decision', {
        model: wandbModel,
