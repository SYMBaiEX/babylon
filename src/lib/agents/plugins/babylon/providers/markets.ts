  ...
  get: async (runtime: IAgentRuntime, _message: Memory, _state: State) => {
    try {
      ...
      if (!babylonRuntime.a2aClient?.isConnected()) {
        ...
        return 'ERROR: A2A client not connected. Cannot fetch markets data. Please ensure A2A server is running.'
      }
      ...
      const perpetualsData = perpetuals as { tickers?: any[] }
...
${perpetualsData.tickers?.slice(0, 10).map((p: any, i: number) => `${i + 1}. ${p.name} (${p.ticker}) @ $${p.currentPrice}`).join('\n') || 'None'}`
      ...
      logger.error('Failed to fetch markets via A2A', error, 'BabylonPlugin')
      return 'ERROR: Failed to fetch markets via A2A protocol'
    }
  }
}
