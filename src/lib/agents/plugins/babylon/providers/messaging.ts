  ...
  get: async (runtime: IAgentRuntime, _message: Memory, _state: State) => {
    try {
      ...
      if (!babylonRuntime.a2aClient?.isConnected()) {
        ...
        return 'ERROR: A2A client not connected. Cannot fetch messages. Please ensure A2A server is running.'
      }
      ...
      const chatsData = chats as { chats?: any[] }
...
${chatsData.chats?.slice(0, 10).map((c: any) => `- ${c.name || (c.isGroup ? 'Group Chat' : 'Direct Message')} (${c.participants} participants)
  ...
  ${c.updatedAt}`).join('\n') || 'No chats'}`
      ...
      logger.error('Failed to fetch messages via A2A', error, 'BabylonPlugin')
      return 'ERROR: Failed to fetch messages via A2A protocol'
    }
  }
}
  ...
  get: async (runtime: IAgentRuntime, _message: Memory, _state: State) => {
    try {
      ...
      if (!babylonRuntime.a2aClient?.isConnected()) {
        ...
        return 'ERROR: A2A client not connected. Cannot fetch notifications. Please ensure A2A server is running.'
      }
      ...
      const notifData = notifications as { notifications?: any[]; unreadCount?: number }
...
${notifData.notifications?.map((n: any) => `${n.read ? '✓' : '•'} ${n.type}: ${n.message}
  ${n.createdAt}`).join('\n') || 'No notifications'}`
      ...
      logger.error('Failed to fetch notifications via A2A', error, 'BabylonPlugin')
      return 'ERROR: Failed to fetch notifications via A2A protocol'
    }
  }
}
