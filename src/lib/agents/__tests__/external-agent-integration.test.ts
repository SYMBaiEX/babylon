...
describe('CommunicationHub - Streaming and Context Support', () => {
  ...
  it('should support contextId for conversation continuity', async () => {
    ...
    const subscription = hub.subscribeToMessages('recipient-agent', async (message) => {
  ...
  it('should broadcast with contextId to multiple agents', async () => {
    ...
    const subscription1 = hub.subscribeToMessages('agent-1', async (message) => {
      expect(message.contextId).toBe(contextId)
    })

    const subscription2 = hub.subscribeToMessages('agent-2', async (message) => {
      expect(message.contextId).toBe(contextId)
    })

    const subscription3 = hub.subscribeToMessages('agent-3', async (message) => {
      expect(message.contextId).toBe(contextId)
    })
