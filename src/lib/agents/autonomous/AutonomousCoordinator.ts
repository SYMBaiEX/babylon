...
export class AutonomousCoordinator {
  ...
  async executeAutonomousTick(
    ...
    recordTrajectories: boolean = false
    ...
    if (recordTrajectories) {
      ...
      if (trajectoryLogger) {
        ...
        (runtime as { currentTrajectoryId?: string }).currentTrajectoryId = trajId
      }
    }
    ...
    if (agent.autonomousTrading) {
      ...
      let initialState: { agentBalance: number; agentPnL: number; openPositions: number; activeMarkets: number; timestamp: number } | undefined
