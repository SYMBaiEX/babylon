...
export class AutonomousTradingService {
   ...
   */
  async executeTrades(agentUserId: string, runtime: IAgentRuntime): Promise<number> {
    try {
...
${predictionMarkets.slice(0, 5).map(m => `- ${m.question} (YES: ${m.yesShares}, NO: ${m.noShares})`).join('\n')}

Available Perp Markets:
${perpMarkets.slice(0, 5).map(o => `- ${o.name} @ $${o.currentPrice}`).join('\n')}
...
Only trade if you have strong conviction and sufficient balance.`
      ...
      const decision = await runtime.useModel(modelType, {
        prompt,
        temperature: 0.7,
        maxTokens: 300
      })
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          // If no valid JSON, assume hold
          return 0
        }
      } catch {
        logger.warn(`Failed to parse trade decision for agent ${agentUserId}`, undefined, 'AutonomousTrading')
        return 0
      }
      ...
      logger.error(`Autonomous trading failed for ${agentUserId}`, error, 'AutonomousTrading')
      return 0
    }
  }
}
