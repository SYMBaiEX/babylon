...
export class AgentWalletService {
  ...
  async registerAgentOnChain(agentUserId: string): Promise<{
    ...
    try {
      ...
      logger.error(`Failed to register agent ${agentUserId} on-chain`, error, 'AgentWalletService')
      
      // Log error but don't throw - agent can still work without on-chain registration
      await prisma.agentLog.create({
        data: {
          id: uuidv4(),
          agentUserId,
          type: 'error',
          level: 'error',
          message: `On-chain registration failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  async setupAgentIdentity(agentUserId: string): Promise<{
    ...
    try {
      ...
      logger.error(`Failed to setup agent identity for ${agentUserId}`, error, 'AgentWalletService')
      throw error
    }
  }
  ...
  async signTransaction(agentUserId: string, transactionData: {
    ...
    try {
      ...
      logger.error(`Failed to sign transaction for agent ${agentUserId}`, error, 'AgentWalletService')
      throw error
    }
  }
