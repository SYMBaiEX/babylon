...
import { countTokensSync, truncateToTokenLimitSync, getModelTokenLimit } from '@/lib/token-counter';
...
export function buildSafePrompt(
  ...
  model: string = 'OpenPipe/Qwen3-14B-Instruct',
  safetyMargin: number = 2000
): { prompt: string; truncated: boolean; originalTokens: number; finalTokens: number } {
  ...
  logger.warn(`Prompt exceeds limit: ${estimatedTokens} > ${maxPromptTokens}, truncating`, {
    model,
    estimatedTokens,
  ...
  for (const section of sortedSections) {
    ...
    if (currentTokens + sectionTokens <= maxPromptTokens) {
      ...
      const truncated = truncateToTokenLimitSync(section.content, available, { ellipsis: true });
...
export function buildPrompt(
  ...
  if (result.truncated) {
    logger.warn(`Prompt was truncated from ${result.originalTokens} to ${result.finalTokens} tokens`);
  }
