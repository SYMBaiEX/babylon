  ...
  if (agent.isAgent) {
    try {
      ...
      if (!existingReg) {
        ...
        logger.info('Registered and activated test agent in Unified Agent Registry', { agentId: agent.id });
      } else {
        // Ensure status is ACTIVE
        if (existingReg.status !== AgentStatus.ACTIVE) {
          await agentRegistry.updateAgentStatus(agent.id, AgentStatus.ACTIVE);
          logger.info('Updated test agent status to ACTIVE in Unified Agent Registry', { agentId: agent.id });
        }
      }
    } catch (error) {
