...
export class ConcentratedLiquidityPool {
  ...
  constructor(config: PoolConfig) {
    ...
    this.concentrationMultiplier = config.concentrationMultiplier ?? DEFAULT_CONCENTRATION;
  }
   ...
   */
  removeLiquidityPosition(positionId: string, ownerId: string): RemovePositionResult {
  ...
  private getEffectiveLiquidityAtCurrentPrice(): {
    ...
    const currentPrice = PredictionPricing.getCurrentPrice(yesShares, noShares, 'yes');
  ...
  private distributeFees(feeAmount: number, activePositionIds: string[]): void {
    ...
    let totalActiveLiquidity = this.state.baseYesShares + this.state.baseNoShares;
  ...
  getPoolStats(): {
    ...
    const activePositions = Array.from(this.positions.values()).filter(p => p.isActive).length;
