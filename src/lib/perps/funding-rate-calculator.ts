...
export function calculateDynamicFundingRate(
  ...
  const scaleFactor = (maxFundingRate - baseFundingRate) / Math.pow(1.0, imbalanceExponent);
  
  // Calculate rate multiplier using exponential function
  const rateMultiplier = Math.pow(absImbalance, imbalanceExponent) * scaleFactor;
  ...
  if (absImbalance < 0.01) {
    ...
    const signedRate = (baseFundingRate + rateMultiplier) * Math.sign(imbalance);
  ...
  const isSeverelyImbalanced = absImbalance > FUNDING_DEFAULTS.SEVERE_IMBALANCE_THRESHOLD;
