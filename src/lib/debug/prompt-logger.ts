...
export async function logPrompt(entry: PromptLogEntry): Promise<void> {
  ...
  try {
    ...
    if (entry.metadata) {
      lines.push(`## Metadata`, ``);
      if (entry.metadata.provider) lines.push(`- **Provider:** ${entry.metadata.provider}`);
      if (entry.metadata.model) lines.push(`- **Model:** ${entry.metadata.model}`);
      if (entry.metadata.temperature !== undefined) lines.push(`- **Temperature:** ${entry.metadata.temperature}`);
      if (entry.metadata.maxTokens) lines.push(`- **Max Tokens:** ${entry.metadata.maxTokens}`);
      if (entry.metadata.format) lines.push(`- **Format:** ${entry.metadata.format}`);
      lines.push(``);
    }
    ...
    logger.debug(`Logged prompt to ${filename}`, { 
      promptType: entry.promptType,
      filepath 
    }, 'PromptLogger');
  } catch (error) {
    logger.error('Failed to log prompt debug file', { 
 ...
 */
export async function cleanOldDebugLogs(maxAgeDays: number = 7): Promise<number> {
  if (!isPromptLoggingEnabled()) {
    return 0;
  }
  ...
  try {
    ...
    if (deleted > 0) {
      logger.info(`Cleaned up ${deleted} old debug logs`, { maxAgeDays }, 'PromptLogger');
    }
