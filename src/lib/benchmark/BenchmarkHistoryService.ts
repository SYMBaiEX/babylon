...
export class BenchmarkHistoryService {
   ...
   */
  static async saveResult(input: BenchmarkResultInput): Promise<BenchmarkResult> {
    ...
    const [result] = await db.insert(benchmarkResults).values(record).returning();
   ...
   */
  static async getResults(query: BenchmarkHistoryQuery): Promise<BenchmarkResult[]> {
   ...
   */
  static async getLatestResult(modelId: string): Promise<BenchmarkResult | null> {
   ...
   */
  static async getTrendData(modelId: string, limit = 20): Promise<BenchmarkTrendData> {
    const results = await db
      .select()
      .from(benchmarkResults)
      .where(eq(benchmarkResults.modelId, modelId))
   ...
   */
  static async getModelComparison(modelIds: string[], benchmarkId?: string): Promise<Map<string, BenchmarkResult[]>> {
   ...
   */
  static async getModelSummary(): Promise<Array<{
    modelId: string;
    runCount: number;
    avgPnl: number;
    avgAccuracy: number;
