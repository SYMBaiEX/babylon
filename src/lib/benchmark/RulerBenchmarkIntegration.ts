...
import type { BenchmarkGameSnapshot, GroundTruth } from './BenchmarkDataGenerator';
...
export function wasDecisionOptimal(
  ...
  const relevantActions = optimalActions.filter(a => 
 ...
 */
export function getTrueFacts(snapshot: BenchmarkGameSnapshot): GroundTruth['trueFacts'] {
...
export function scoreActionAgainstGroundTruth(
  ...
  const relevantFacts = hiddenFacts.filter(f => 
    f.value && typeof f.value === 'object' && 'marketId' in f.value &&
