...
<body>
  ...
  private static generateSummaryStats(results: ModelComparisonData[]): string {
    ...
    const bestAccuracy = results.reduce((best, curr) => 
      curr.metrics.predictionMetrics.accuracy > best.metrics.predictionMetrics.accuracy ? curr : best
    );
   ...
   */
  private static generateComparisonTable(results: ModelComparisonData[]): string {
   ...
   */
  private static generateHistorySection(history: BenchmarkHistoryEntry[]): string {
  ...
  static generateTerminalSummary(results: ModelComparisonData[]): string {
    ...
    // P&L Chart
    lines.push(this.generateTerminalChart(
      '💰 Total P&L',
    ...
    lines.push(this.generateTerminalChart(
      '🎯 Prediction Accuracy',
      sorted.map(r => ({ label: r.modelName, value: r.metrics.predictionMetrics.accuracy * 100 })),
