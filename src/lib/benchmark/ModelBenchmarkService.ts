...
export class ModelBenchmarkService {
   ...
   */
  static async benchmarkModel(options: ModelBenchmarkOptions): Promise<ModelBenchmarkResult[]> {
    ...
    for (const benchmarkPath of options.benchmarkPaths) {
      ...
      try {
        ...
        // Run benchmark
        const simulationResult: SimulationResult = await BenchmarkRunner.runSingle({
          benchmarkPath,
