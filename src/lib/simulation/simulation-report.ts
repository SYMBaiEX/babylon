...
function generateSummary(result: SimulationResult): ReportSummary {
  const healthTrend = result.finalHealthScore > result.initialHealthScore + 5
    ? 'improving'
  ...
  return {
    ...
    predictionMarket: {
      finalLiquidity: formatCurrency(result.finalPredictionState.totalLiquidity),
...
function createEmptyAnalysis(): TimeSeriesAnalysis {
  return {
    ...
    healthScore: { min: 0, max: 0, avg: 0, volatility: 0, timesBelowThreshold: { critical: 0, poor: 0, fair: 0 } },
  };
}
...
export function compareScenarios(
  ...
  lines.push('Metric'.padEnd(30) + scenarios.map(s => s.padStart(15)).join(''));
  ...
  const metrics = [
    ...
    {
      name: 'Perp Imbalance %',
      getter: (r: SimulationResult) => r.finalPerpState.imbalancePercent.toFixed(1),
    },
    {
      name: 'Avg Funding APR %',
      getter: (r: SimulationResult) => r.finalPerpState.avgFundingRateAPR.toFixed(2),
    },
    {
      name: 'Pred Liquidity',
      getter: (r: SimulationResult) => (r.finalPredictionState.totalLiquidity / 1000).toFixed(1) + 'k',
    },
    {
      name: 'NPC P&L',
      getter: (r: SimulationResult) => formatCurrencySigned(r.finalPredictionState.npcNetPnL),
    },
    {
      name: 'User P&L',
      getter: (r: SimulationResult) => formatCurrencySigned(r.finalPredictionState.userNetPnL),
    },
  ];
