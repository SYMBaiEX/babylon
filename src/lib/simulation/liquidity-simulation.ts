...
export const SCENARIOS = {
  ...
  spotCrash: {
    ...
    events: [
      { tick: 30, type: 'npc_spot_shock', magnitude: -0.20 },
    ],
  },
  ...
  massExit: {
    ...
    events: [
      { tick: 50, type: 'mass_exit', magnitude: 0.50 },
    ],
  },
...
export class LiquiditySimulator {
  ...
  async run(): Promise<SimulationResult> {
    ...
    for (this.currentTick = 0; this.currentTick < this.config.durationTicks; this.currentTick++) {
      // Check for events
      this.processEvents();
    ...
    const result: SimulationResult = {
      config: this.config,
      durationTicks: this.config.durationTicks,
      simulatedTimeSeconds: this.config.durationTicks * this.config.tickIntervalSeconds,
      ...
      finalPredictionState: {
        totalLiquidity: this.predictionMarkets.reduce((sum, m) => sum + m.yesShares + m.noShares, 0),
        avgSpread: this.calculateAvgPredictionSpread(),
        totalVolume: this.predictionMarkets.reduce((sum, m) => sum + m.volume, 0),
        npcNetPnL: this.predictionMarkets.reduce((sum, m) => sum + m.npcPnL, 0),
        userNetPnL: this.predictionMarkets.reduce((sum, m) => sum + m.userPnL, 0),
      },
  ...
  private initializePredictionMarkets(): SimPredictionMarket[] {
    ...
    for (let i = 0; i < this.config.predictions.marketCount; i++) {
      const init = PredictionPricing.initializeMarket(this.config.predictions.initialLiquidity);
      markets.push({
        id: `market-${i}`,
  ...
  private processEvents(): void {
    const events = this.config.events?.filter(e => e.tick === this.currentTick) || [];
  ...
  private simulatePerpTrading(): void {
    ...
    if (isOpen) {
      ...
      const idx = Math.floor(this.rng.next() * this.perpMarket.positions.length);
  ...
  private processFunding(): void {
    ...
    for (const pos of this.perpMarket.positions) {
      const payment = calculatePositionFunding(pos.size, fundingResult.periodRate, pos.side);
   ...
   */
  private executePredictionTrade(market: SimPredictionMarket, trader: 'npc' | 'user'): void {
    const amount = this.config.predictions.avgTradeSize * (0.5 + this.rng.next());
    ...
    try {
      ...
      const slippageCost = Math.abs(calc.avgPrice - midPrice) * calc.sharesBought * 0.1;
      
      // Simulate market resolution for this trade
      // Use market probability as win chance (efficient market hypothesis)
      const marketProb = PredictionPricing.getCurrentPrice(calc.newYesShares, calc.newNoShares, side);
  ...
  private executeMassExit(ratio: number): void {
    ...
    for (let i = 0; i < exitCount && this.perpMarket.positions.length > 0; i++) {
      const idx = Math.floor(this.rng.next() * this.perpMarket.positions.length);
      const pos = this.perpMarket.positions[idx];
  ...
  private collectTickMetrics(): TickMetrics {
    ...
    const fundingRate = this.perpMarket.fundingHistory[this.perpMarket.fundingHistory.length - 1];
    ...
    return {
      ...
      predictionTotalLiquidity: this.predictionMarkets.reduce(
        (sum, m) => sum + m.yesShares + m.noShares, 0
      ),
      predictionAvgSpread: this.calculateAvgPredictionSpread(),
      predictionVolume: this.predictionMarkets.reduce((sum, m) => sum + m.volume, 0),
  ...
  private calculateAvgFundingRate(): number {
    ...
    const sum = this.perpMarket.fundingHistory.reduce((s, f) => s + Math.abs(f.annualRate), 0);
  ...
  private calculateHealthScore(): number {
    ...
    const avgLiquidity = this.predictionMarkets.reduce(
      (sum, m) => sum + m.yesShares + m.noShares, 0
  ...
  private analyzeResults(): { findings: string[]; recommendations: string[] } {
    ...
    if (perpImbalance > 0.7) {
      findings.push(`Perp market ended with ${(perpImbalance * 100).toFixed(0)}% imbalance`);
      recommendations.push('Funding rate mechanism may need more aggressive scaling');
    }
    
    const avgFunding = this.calculateAvgFundingRate();
    if (avgFunding > 0.20) {
      findings.push(`Average funding rate was ${(avgFunding * 100).toFixed(1)}% APR (high)`);
    }
    ...
    const initialLiquidity = this.config.predictions.initialLiquidity * this.config.predictions.marketCount;
    const liquidityChange = (finalLiquidity - initialLiquidity) / initialLiquidity * 100;
    
    if (Math.abs(liquidityChange) > 30) {
      findings.push(`Prediction liquidity changed by ${liquidityChange.toFixed(0)}%`);
    }
    
    const npcPnL = this.predictionMarkets.reduce((sum, m) => sum + m.npcPnL, 0);
    const userPnL = this.predictionMarkets.reduce((sum, m) => sum + m.userPnL, 0);
    
    if (npcPnL < -1000) {
      findings.push(`NPCs lost ${Math.abs(npcPnL).toFixed(0)} points to users (user P&L: +${userPnL.toFixed(0)})`);
    } else if (npcPnL > 1000) {
      findings.push(`NPCs gained ${npcPnL.toFixed(0)} points from users (user P&L: ${userPnL.toFixed(0)})`);
      recommendations.push('Consider reviewing NPC trading strategies');
    } else if (Math.abs(userPnL) > 1000) {
      findings.push(`Users P&L: ${userPnL > 0 ? '+' : ''}${userPnL.toFixed(0)} points`);
    }
    ...
    // Health analysis
    const lowestHealth = Math.min(...this.tickMetrics.map(m => m.overallHealthScore));
    if (lowestHealth < 40) {
      findings.push(`Health score dropped to ${lowestHealth} during simulation`);
      recommendations.push('Consider implementing circuit breakers for extreme conditions');
    }
