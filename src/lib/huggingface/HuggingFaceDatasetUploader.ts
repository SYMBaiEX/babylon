...
export class HuggingFaceDatasetUploader {
   ...
   */
  private async collectBenchmarkData(benchmarkDir: string): Promise<BenchmarkRecord[]> {
      ...
      for (const file of files) {
        if (file.endsWith('.json') && file.startsWith('baseline-')) {
          ...
          records.push({
            ...
            modelName: data.model?.displayName || data.model?.name || file.replace('.json', ''),
...
`;
   ...
   */
  private async ensureRepository(datasetName: string, isPrivate: boolean): Promise<void> {
    if (!this.huggingFaceToken) {
