 ...
 */
import { db, users, actors, referrals, pointsTransactions, eq, and, gt, gte, ne, desc, asc, count, sql, isNull } from '@/db';
...
export class PointsService {
  ...
  static async awardReferralSignup(
    ...
    const [unqualifiedCountResult] = await db.select({ count: count() })
      .from(referrals)
      .where(and(
    ...
    if (referralRecord) {
      // Build update object
      const updateData: { signupPointsAwarded?: boolean; suspiciousReferralFlags?: JsonValue } = {};
   ...
   */
  static async awardPendingReferralSignupPoints(referrerId: string): Promise<AwardPointsResult | null> {
    ...
    const pendingReferralResult = await db.select()
      .from(referrals)
      .where(and(
        eq(referrals.referrerId, referrerId),
        eq(referrals.status, 'completed'),
