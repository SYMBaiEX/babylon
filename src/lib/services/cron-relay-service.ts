...
export async function relayCronToStaging(
  ...
  const stagingBaseUrl = process.env.CRON_STAGING_URL || 'https://staging.babylon.market'
  ...
  if (!cronSecret) {
    logger.warn('Cannot relay cron - CRON_SECRET missing', { routeName }, 'CronRelay')
    return { forwarded: false }
  }
  ...
  try {
    ...
    logger.info('Relayed cron execution to staging', {
      routeName,
      targetUrl,
      status: res.status,
    }, 'CronRelay')
    ...
    logger.error('Failed to relay cron to staging', {
      routeName,
      targetUrl,
