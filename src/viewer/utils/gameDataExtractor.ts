...
export function extractGameRecapData(
  ...
  return {
    ...
    startDate,
    endDate,
    worldSummary: generateWorldSummary(game),
 ...
 */
function generateWorldSummary(game: GeneratedGame): string {
  ...
  return `The world is focused on ${scenarioThemes}. Key players include ${actorNames}. ` +
...
function generatePreviousContext(previousGames: GameHistory[]): string {
  ...
  // Add key outcomes
  if (lastGame.keyOutcomes.length > 0) {
  ...
  // Add highlights
  if (lastGame.highlights.length > 0) {
