...
import {
  ...
  broadcastToChannel,
} from '@/lib/sse/event-broadcaster';
...
import { type A2AGameConfig, A2AGameIntegration } from './A2AGameIntegration';
...
export class GameEngine extends EventEmitter {
  ...
  constructor(config?: GameConfig) {
    ...
    this.marketDecisionEngine = new MarketDecisionEngine(
      this.llm,
      this.marketContextService
    );
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.info(
        'Successfully loaded actors from HTTP',
        undefined,
        'GameEngine'
      );
    } catch (httpError) {
      ...
      logger.warn(
        `Failed to load actors from HTTP (${errorMessage}), falling back to file system`,
        { error: httpError },
        'GameEngine'
      );

      const actorsPath = join(process.cwd(), 'public/data/actors.json');
      actorsData = JSON.parse(
        readFileSync(actorsPath, 'utf-8')
      ) as ActorsDatabase;
      logger.info(
        'Loaded actors from file system (fallback)',
        undefined,
        'GameEngine'
      );
    }
    ...
    logger.info(
      `Loaded ${this.actors.length} actors`,
      { actorCount: this.actors.length },
      'GameEngine'
    );

    logger.info(
      'Initializing organizations and prices...',
      undefined,
      'GameEngine'
    );
    ...
    logger.info(
      `Initialized ${companies.length} company prices`,
    ...
    logger.info(
      `Generated ${this.scenarios.length} scenarios`,
      { scenarioCount: this.scenarios.length },
      'GameEngine'
    );

    logger.info(
      'Loading actor connections from relationships...',
      undefined,
      'GameEngine'
    );
    this.connections = await this.loadConnectionsFromRelationships();
    logger.info(
      `Loaded ${this.connections.length} relationships`,
    ...
    logger.info(
      `Created ${this.groupChats.length} group chats`,
      { groupChatCount: this.groupChats.length },
      'GameEngine'
    );
  ...
  start(): void {
    ...
    this.dailySnapshotIntervalId = setInterval(() => {
      this.checkDailySnapshot().catch((error) => {
        ...
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger.error(
          `Daily snapshot error: ${errorMessage}`,
          { error: errorMessage, stack: errorStack },
          'GameEngine'
        );
      });
  ...
  async stop(): Promise<void> {
    ...
    try {
      await db.updateGameState({
        lastTickAt: new Date(),
        activeQuestions: this.questions.filter((q) => q.status === 'active')
          .length,
      });
    } catch (error) {
      const errorMessage =
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.warn(
        `Failed to update game state on stop: ${errorMessage}`,
        { error: errorMessage, stack: errorStack },
        'GameEngine'
      );
    }
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      const toResolve = this.questionManager.getQuestionsToResolve(
        activeQuestions,
        currentDate
      );
      ...
      if (toResolve.length > 0) {
        logger.info(
          `Resolving ${toResolve.length} questions`,
          { count: toResolve.length },
          'GameEngine'
        );
        for (const question of toResolve) {
          // Generate definitive resolution event
          const resolutionEvent: WorldEvent =
            await this.generateResolutionEvent(question);
          resolutionEvents.push(resolutionEvent);

          const resolved = this.questionManager.resolveQuestion(
            question,
            question.outcome
          );
          ...
          try {
            ...
            const errorStack = error instanceof Error ? error.stack : undefined;
            logger.error(
              `Failed to update question status in database: ${errorMessage}`,
              {
                error: errorMessage,
                stack: errorStack,
          ...
          logger.info(
            `Updating on-chain reputation for market ${question.id}`,
            { questionId: question.id },
            'GameEngine'
          );
          try {
            ...
            const winners = reputationUpdates.filter(
              (u) => u.change > 0
            ).length;
            ...
            logger.info(
              `Reputation updated: ${winners} winners (+10), ${losers} losers (-5)${errors > 0 ? `, ${errors} errors` : ''}`,
              {
                winners,
                losers,
                errors,
            ...
            const errorStack = error instanceof Error ? error.stack : undefined;
            logger.error(
              `Failed to update reputation for market ${question.id}: ${errorMessage}`,
              {
                error: errorMessage,
                stack: errorStack,
                questionId: question.id,
          ...
          logger.info(
            `Generating auto-feedback for resolved market ${question.id}`,
            { questionId: question.id },
            'GameEngine'
          );
          try {
            ...
            for (const position of positions) {
              try {
                ...
                logger.warn(
                  `Failed to generate feedback for position ${position.id}: ${errorMessage}`,
                  {
            ...
            logger.info(
              `Auto-feedback generated: ${successCount} success, ${failCount} failed (${positions.length} total positions)`,
              {
                successCount,
                failCount,
            ...
            const errorStack = error instanceof Error ? error.stack : undefined;
            logger.error(
              `Failed to generate auto-feedback for market ${question.id}: ${errorMessage}`,
              {
                error: errorMessage,
                stack: errorStack,
                questionId: question.id,
      ...
      const currentActive = this.questions.filter(
        (q) => q.status === 'active'
      ).length;
      ...
      if (currentActive < 15) {
        ...
        logger.info(
          `Created ${questionsCreated} new questions`,
        ...
        if (questionsCreated > 0) {
          try {
            ...
            logger.debug(
              'Could not broadcast new questions:',
              error,
              'GameEngine'
            );
          }
      ...
      const events = await this.generateQuestionDrivenEvents(
        activeQuestions,
        resolutionEvents
      );
      ...
      logger.info(
        `Generated ${posts.length} LLM posts`,
        { count: posts.length },
        'GameEngine'
      );

      // Step 6: Generate group chat messages about questions
      const groupChatMessages = await this.generateGroupChatDiscussions(
        activeQuestions,
        events
      );
      ...
      logger.info(
        `Executed ${executionResult.successfulTrades} trades`,
        {
      ...
      const tradeBasedPriceUpdates =
        await this.updatePricesFromTrades(executionResult);
      logger.info(
        `Trade-based price updates: ${tradeBasedPriceUpdates.length}`,
      ...
      if (allPriceUpdates.length > 0) {
        const formattedUpdates: PriceUpdateInput[] = [
        ...
        await PriceUpdateService.applyUpdates(formattedUpdates);
      }
      ...
      await this.persistTickData(tick).catch((error) => {
        ...
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger.error(
          `Failed to persist tick data: ${errorMessage}`,
          {
            error: errorMessage,
            stack: errorStack,
          },
      ...
      if (posts.length > 0) {
        try {
          ...
          logger.debug(
            `Broadcasted ${posts.length} posts to feed channel`,
            { count: posts.length },
            'GameEngine'
          );
        } catch (error) {
      ...
      if (Object.keys(groupChatMessages).length > 0) {
        try {
          ...
          logger.debug(
            `Broadcasted ${Object.values(groupChatMessages).flat().length} group chat messages`,
            undefined,
            'GameEngine'
          );
        } catch (error) {
          logger.debug(
            'Could not broadcast group chat messages:',
            error,
            'GameEngine'
          );
        }
      ...
      if (this.recentTicks.length % 5 === 0) {
        try {
          ...
          const errorStack = error instanceof Error ? error.stack : undefined;
          logger.warn(
            'Failed to process social actions',
            {
              error: errorMessage,
              stack: errorStack,
            },
      ...
      if (currentDate !== this.lastChatSweep) {
        logger.info(
          'Running daily group chat sweep...',
          { date: currentDate },
          'GameEngine'
        );
        try {
          ...
          logger.info(
            `Group chat sweep complete: ${sweepResult.totalRemoved} users removed from ${sweepResult.chatsChecked} chats`,
            sweepResult,
            'GameEngine'
          );
          ...
          const errorStack = error instanceof Error ? error.stack : undefined;
          logger.error(
            `Failed to run group chat sweep: ${errorMessage}`,
            {
              error: errorMessage,
              stack: errorStack,
            },
  ...
  private async generateEventForQuestion(
    ...
    let involvedActors = scenario
      ? this.actors
          .filter((a) => scenario.mainActors.includes(a.id))
          .slice(0, 3)
    ...
    const shouldPointToOutcome =
      urgency === 'critical' || (urgency === 'high' && Math.random() > 0.3);
  ...
  "type": "announcement|scandal|deal|conflict|revelation"
    ...
    try {
      ...
      return {
        ...
        pointsToward: shouldPointToOutcome
          ? question.outcome
            ? 'YES'
            : 'NO'
          : null,
      ...
      const errorMessage =
        eventGenerationError instanceof Error
          ? eventGenerationError.message
          : 'Failed to generate event';
      const errorStack =
        eventGenerationError instanceof Error
          ? eventGenerationError.stack
          : undefined;
      logger.error(
        `Failed to generate event: ${errorMessage}`,
        {
          error: errorMessage,
          stack: errorStack,
          questionId: question.id,
...
OUTPUT JSON:
{
    ...
    try {
      ...
      const errorMessage =
        resolutionError instanceof Error
          ...
          : 'Failed to generate resolution event';
      const errorStack =
        resolutionError instanceof Error ? resolutionError.stack : undefined;
      logger.error(
        `Failed to generate resolution event: ${errorMessage}`,
        {
          error: errorMessage,
          stack: errorStack,
          questionId: question.id,
  ...
  private async generateLLMPosts(
    ...
    const newsOrgs = this.organizations
    ...
    for (const event of events.slice(0, Math.ceil(numPosts / 4))) {
      ...
      for (const creator of selectedCreators) {
        try {
          ...
          const errorMessage =
            postGenerationError instanceof Error
              ? postGenerationError.message
              : 'Failed to generate post';
          logger.warn(
            'Failed to generate post, skipping',
            { error: errorMessage, creatorId: creator.id, eventId: event.id },
            'GameEngine'
          );
          continue;
        }
    ...
    while (posts.length < numPosts) {
      ...
      try {
        if (creator.type === 'actor') {
          const post = await this.generateAmbientPostAboutQuestion(
            creator.data as SelectedActor,
            question
          );
          if (post) {
        ...
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        logger.warn(
          'Failed to generate ambient post',
          {
            error: errorMessage,
    ...
    logger.info(
      `Generated ${posts.length} mixed posts (NPCs + orgs)`,
      { count: posts.length },
      'GameEngine'
    );
    return posts;
  ...
  private async generateActorPostAboutEvent(
    ...
    const relationshipContext =
      await RelationshipManager.getRelationshipContext(actor.id, event.actors);
...
OUTPUT JSON:
{
  ...
  "pointsToward": true/false/null
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.error(
        `Failed to generate LLM post: ${errorMessage}`,
        {
          error: errorMessage,
          stack: errorStack,
          actorId: actor.id,
    ...
    try {
      ...
      const errorMessage =
        llmError instanceof Error
          ? llmError.message
          : 'Failed to generate LLM post';
      logger.warn(
        'Failed to generate LLM post, returning null',
        { error: errorMessage, actorId: actor.id },
        'GameEngine'
      );
      return null;
    }
...
OUTPUT JSON:
{
  ...
  "summary": "brief summary"
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.warn(
        'Failed to generate org article',
        {
          error: errorMessage,
          stack: errorStack,
          orgId: org.id,
...
OUTPUT JSON:
{
  ...
  "summary": "brief analysis"
    ...
    try {
      ...
      logger.debug(
        'LLM failed to generate ambient org article',
        { error },
        'GameEngine'
      );
      return null;
  ...
  private async generateGroupChatDiscussions(
    ...
    for (const groupChat of this.groupChats.slice(0, 10)) {
      // Limit to first 10 chats
      const chatMessages: ChatMessage[] = [];

      // Pick 1-2 relevant questions
      const relevantQuestions = shuffleArray(activeQuestions).slice(
        0,
      ...
      for (const question of relevantQuestions) {
        ...
        const memberRelationships = await db.prisma.actorRelationship.findMany({
          where: {
            OR: [
              {
                actor1Id: { in: groupChat.members },
                actor2Id: { in: groupChat.members },
              },
            ],
        ...
        if (memberRelationships.length > 0) {
          ...
          if (!rivalPair) {
            respondingMembers = shuffleArray(
              groupChat.members.filter((m) => m !== groupChat.admin)
            ).slice(0, 2);
          } else {
            respondingMembers = [rivalPair.actor1Id, rivalPair.actor2Id];
            logger.debug(
              `Group chat drama: ${rivalPair.actor1Id} vs ${rivalPair.actor2Id}`,
              undefined,
              'GameEngine'
            );
          }
        ...
        for (const memberId of respondingMembers) {
          ...
          try {
            ...
            logger.warn(
              'Failed to generate chat response, skipping',
              {
                error: errorMessage,
...
OUTPUT JSON:
{
  ...
  private async updatePrices(events: WorldEvent[]): Promise<PriceUpdate[]> {
    ...
    for (const event of events) {
      ...
      for (const company of affectedCompanies) {
        ...
        const update = this.applyDeterministicEventImpact(
          company,
          event,
          direction,
          magnitude
  ...
  private applyDeterministicEventImpact(
    ...
    const companyIndex = this.organizations.findIndex(
      (o) => o.id === company.id
    );
    if (companyIndex >= 0) {
  ...
  private async generateQuestions(_count: number): Promise<Question[]> {
    ...
    const nextId =
      Math.max(0, ...this.questions.map((q) => toQuestionIdNumber(q.id))) + 1;
    ...
    for (const question of newQuestions) {
      try {
        ...
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger.error(
          `Failed to persist question ${question.id}: ${errorMessage}`,
          {
            error: errorMessage,
            stack: errorStack,
            questionId: question.id,
  ...
  private buildRecentDaysContext(
    recentEvents: WorldEvent[]
  ): Array<{ day: number; events: WorldEvent[] }> {
  ...
  private async checkDailySnapshot(): Promise<void> {
    ...
    if (currentDate !== this.lastDailySnapshot) {
      ...
      try {
        ...
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger.warn(
          `Failed to update lastSnapshotAt: ${errorMessage}`,
          {
            error: errorMessage,
            stack: errorStack,
          },
  ...
  private async syncDatabaseState(): Promise<void> {
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.warn(
        `Failed to ensure game state exists: ${errorMessage}`,
        {
          error: errorMessage,
          stack: errorStack,
        },
        ...
        try {
          ...
          const errorMessage =
            syncError instanceof Error
              ? syncError.message
              : 'Failed to sync actor';
          const errorStack =
            syncError instanceof Error ? syncError.stack : undefined;
          logger.warn(
            `Failed to sync actor ${actor.id}: ${errorMessage}`,
            {
              error: errorMessage,
              stack: errorStack,
              actorId: actor.id,
    ...
    await Promise.allSettled(
      this.organizations.map(async (org) => {
        try {
          ...
          const errorStack = error instanceof Error ? error.stack : undefined;
          logger.warn(
            `Failed to sync organization ${org.id}: ${errorMessage}`,
            {
              error: errorMessage,
              stack: errorStack,
              orgId: org.id,
  ...
  private async syncGroupChatsToDatabase(): Promise<void> {
    ...
    if (existingCount === 0) {
      logger.info(
        'No group chats in database - creating initial set...',
        undefined,
        'GameEngine'
      );
    }
    ...
    for (const chat of this.groupChats) {
      try {
        ...
        if (admin) {
          const initialMessage = await this.generateGroupChatInitialMessage(
            admin,
            chat
          );
        ...
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger.warn(
          `Failed to sync group chat ${chat.id}: ${errorMessage}`,
          {
            error: errorMessage,
            stack: errorStack,
            chatId: chat.id,
    ...
    logger.info(
      `✅ Synced ${this.groupChats.length} group chats to database`,
      { count: this.groupChats.length },
      'GameEngine'
    );
  }
  ...
  private async persistTickData(tick: Tick): Promise<void> {
    ...
    if (Object.keys(tick.groupChatMessages).length > 0) {
      for (const [chatId, messages] of Object.entries(tick.groupChatMessages)) {
        for (const msg of messages) {
          try {
            ...
            const errorStack = error instanceof Error ? error.stack : undefined;
            logger.warn(
              `Failed to persist chat message: ${errorMessage}`,
              {
                error: errorMessage,
                stack: errorStack,
                chatId,
      ...
      logger.debug(
        `Persisted ${Object.values(tick.groupChatMessages).flat().length} group chat messages`,
        undefined,
        'GameEngine'
      );
    }
    ...
    // Price updates already persisted via PriceUpdateService earlier in the tick.
    ...
    try {
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.warn(
        `Failed to update game state metadata: ${errorMessage}`,
        {
          error: errorMessage,
          stack: errorStack,
        },
  ...
  private async loadHistory(): Promise<void> {
    try {
      ...
      if (gameState?.lastSnapshotAt) {
        this.lastDailySnapshot = gameState.lastSnapshotAt
          .toISOString()
          .split('T')[0]!;
        logger.info(
          'Loaded lastDailySnapshot from database',
          { date: this.lastDailySnapshot },
          'GameEngine'
        );
      } else {
        ...
        logger.info(
          'No lastDailySnapshot in database, using current date',
          { date: this.lastDailySnapshot },
          'GameEngine'
        );
      }
      ...
      const errorStack = error instanceof Error ? error.stack : undefined;
      logger.error(
        `Failed to load history from database: ${errorMessage}`,
        {
          error: errorMessage,
          stack: errorStack,
        },
    ...
    return [
      {
        id: 1,
        title: 'Tech Drama',
        description: 'Tech companies and feuds',
        mainActors: [],
      ...
      },
      {
        ...
        description: 'Political scandals',
        mainActors: [],
        involvedOrganizations: [],
        theme: 'politics',
      },
      {
        id: 3,
        title: 'Market Mayhem',
        description: 'Stock and crypto chaos',
        mainActors: [],
        involvedOrganizations: [],
        theme: 'finance',
      },
  ...
  private async initializeActorRelationships(): Promise<void> {
    ...
    if (existingCount === 0) {
      ...
      logger.warn(
        'For enhanced gameplay, generate relationships:',
        undefined,
        'GameEngine'
      );
      logger.warn(
        '  1. Run: npx tsx scripts/init-actor-relationships.ts',
        undefined,
        'GameEngine'
      );
      logger.warn(
        '  2. Or add data/actor-relationships.json to repo and re-seed',
        undefined,
        'GameEngine'
      );
      logger.info(
        'Continuing with basic relationship generation...',
        undefined,
        'GameEngine'
      );
      return;
    ...
    logger.info(
      `Found ${existingCount} existing relationships`,
      { count: existingCount },
      'GameEngine'
    );
    ...
    if (followCount === 0) {
      logger.info(
        'No follow relationships found, initializing from relationships...',
        undefined,
        'GameEngine'
      );
      ...
      logger.info(
        `Found ${followCount} existing follow relationships`,
        { count: followCount },
        'GameEngine'
      );
    }
  ...
  private async createGroupChats(): Promise<GroupChat[]> {
    ...
    const allRelationships = await db.prisma.actorRelationship.findMany({
      where: {
        OR: [
          {
            actor1Id: { in: mainActorIds },
            relationshipType: { in: positiveTypes },
            sentiment: { gte: 0.3 },
          },
          {
            actor2Id: { in: mainActorIds },
            relationshipType: { in: positiveTypes },
            sentiment: { gte: 0.3 },
          },
        ],
    ...
    logger.info(
      'Creating group chats with relationship-based membership...',
      undefined,
      'GameEngine'
    );
    ...
    for (const main of mainActors) {
      ...
      logger.debug(
        `Created "${groupName}" with ${members.length} members (admin: ${main.name})`,
        undefined,
        'GameEngine'
      );
    ...
    logger.info(
      `Created ${chats.length} group chats`,
      { count: chats.length },
      'GameEngine'
    );
    return chats;
...
OUTPUT JSON:
{
  "message": "your initial message here"
    ...
    try {
      ...
      return (
        response.message ||
        `Welcome to ${chat.name}. Let's discuss what's happening in ${chat.theme}.`
      );
    } catch (error) {
      logger.warn(
        'Failed to generate initial chat message, using fallback',
  ...
  private async generateGroupChatName(
    ...
    const memberDescriptions = members
      ...
      .map(
        (m) =>
          `- ${m.name}: ${m.description || 'actor'}${m.affiliations?.length ? ` [${m.affiliations.join(', ')}]` : ''}`
      )
      .join('\n');
...
Return ONLY this JSON:
{
  "name": "the group chat name here"
    ...
    try {
      ...
      logger.warn(
        'Failed to generate group chat name, using fallback',
        { error, adminId: admin.id },
        'GameEngine'
      );
  ...
  getRecentPosts(minutes: number = 60): FeedPost[] {
    return this.recentTicks.slice(-minutes).flatMap((t) => t.posts);
  }
  ...
  private async updatePricesFromTrades(
    ...
    logger.debug(
      `updatePricesFromTrades called with ${executionResult.executedTrades.length} trades`,
      {},
      'GameEngine'
    );

    // Get trade impacts by ticker/market
    const impacts = await this.tradeExecutionService.getTradeImpacts(
      executionResult.executedTrades
    );

    logger.debug(
      `Calculated impacts for ${impacts.size} tickers`,
      {},
      'GameEngine'
    );
    ...
    for (const [key, impact] of impacts) {
      ...
      if (key.match(/^[A-Z]+$/)) {
        ...
        if (!company) {
          logger.warn(
            `No company found for ticker ${ticker}`,
            {},
            'GameEngine'
          );
          continue;
        ...
        if (!company.currentPrice) {
          logger.warn(
            `Company ${company.id} has no currentPrice`,
            {},
            'GameEngine'
          );
          continue;
        ...
        logger.debug(
          `Price update for ${ticker}: ${oldPrice.toFixed(2)} -> ${newPrice.toFixed(2)} (${changePercent.toFixed(2)}%)`,
          {
            ticker,
            oldPrice,
            newPrice,
    ...
    if (updates.length > 0) {
      logger.info(
        `Updated ${updates.length} prices based on NPC trades`,
