...
export class RelationshipEvolutionEngine {
  ...
  async generateInitialRelationships(
    ...
    logger.info('Generating initial NPC relationships...', undefined, 'RelationshipEvolutionEngine');
    ...
    for (let i = 0; i < actors.length; i++) {
      ...
      for (let j = i + 1; j < actors.length && relationshipCount < targetCount; j++) {
        ...
        const hasSharedContext = sharedOrgs.length > 0 || sharedDomains.length > 0;
    ...
    logger.info(`Created ${created} initial relationships`, { count: created }, 'RelationshipEvolutionEngine');
    return created;
  }
...
Return JSON: { "description": "...", "type": "...", "sentiment": 0.0 }`;
   ...
   */
  async trackInteraction(interaction: Omit<Interaction, 'timestamp'>): Promise<void> {
  ...
  async analyzeAndUpdateRelationships(): Promise<number> {
    if (!this.llm) {
      logger.warn('No LLM client available, skipping relationship analysis', undefined, 'RelationshipEvolutionEngine');
      return 0;
    }
    ...
    logger.info(`Analyzing ${pairInteractions.size} actor pairs for relationship changes`, {
    ...
    for (const [pairKey, interactions] of pairInteractions) {
      ...
      const interactionSummary = interactions
        .slice(0, 5)
        .map(i => `- ${i.interactionType}: ${i.context} (sentiment: ${i.sentiment > 0 ? '+' : ''}${i.sentiment.toFixed(2)})`)
        .join('\n');
...
Return JSON: { "description": "...", "type": "...", "sentiment": 0.0 }`;
      ...
      if (existing) {
        ...
        await db
          ...
          .set({
            ...
            strength: Math.min(1.0, (existing.strength || 0.5) + interactions.length * 0.05),
      ...
      updated++;
      logger.info(`Updated relationship`, {
        actor1: actor1.name,
        actor2: actor2.name,
    ...
    logger.info(`Updated ${updated} relationships`, { count: updated }, 'RelationshipEvolutionEngine');
    return updated;
  }
