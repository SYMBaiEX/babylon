...
import type { SQLValue, DrizzleClient } from './client';
 ...
 */
export async function $queryRaw<T extends Record<string, SQLValue> = Record<string, SQLValue>>(
  db: Database,
...
export async function withRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const dbError: DatabaseErrorType = error instanceof Error 
        ? error 
