...
export const agentPerformanceMetrics = pgTable(
  'AgentPerformanceMetrics',
  {
    ...
    averageIntelScore: doublePrecision('averageIntelScore').notNull().default(50),
  ...
  (table) => [
    ...
    index('AgentPerformanceMetrics_reputationScore_idx').on(table.reputationScore),
...
export const agentGoals = pgTable(
  ...
  (table) => [
    index('AgentGoal_agentUserId_status_idx').on(table.agentUserId, table.status),
...
export const agentRegistries = pgTable(
  'AgentRegistry',
  {
    ...
    discoveryAuthRequired: boolean('discoveryAuthRequired').notNull().default(false),
    discoveryAuthMethods: text('discoveryAuthMethods').array().notNull().default([]),
...
export const externalAgentConnections = pgTable(
  ...
  (table) => [
    index('ExternalAgentConnection_agentRegistryId_idx').on(table.agentRegistryId),
...
export const agentGoalActionsRelations = relations(agentGoalActions, ({ one }) => ({
  goal: one(agentGoals, {
...
export type AgentPointsTransaction = typeof agentPointsTransactions.$inferSelect;
export type NewAgentPointsTransaction = typeof agentPointsTransactions.$inferInsert;
...
export type ExternalAgentConnection = typeof externalAgentConnections.$inferSelect;
export type NewExternalAgentConnection = typeof externalAgentConnections.$inferInsert;
