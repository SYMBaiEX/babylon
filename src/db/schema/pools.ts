...
export const pools = pgTable(
  'Pool',
  {
    ...
    performanceFeeRate: doublePrecision('performanceFeeRate')
      .notNull()
      .default(0.05),
    totalFeesCollected: decimal('totalFeesCollected', { precision: 18, scale: 2 })
      .notNull()
      .default('0'),
...
export const poolDeposits = pgTable(
  'PoolDeposit',
  {
    ...
    currentValue: decimal('currentValue', { precision: 18, scale: 2 }).notNull(),
    unrealizedPnL: decimal('unrealizedPnL', { precision: 18, scale: 2 }).notNull(),
    depositedAt: timestamp('depositedAt', { mode: 'date' }).notNull().defaultNow(),
  ...
  (table) => [
    ...
    index('PoolDeposit_poolId_withdrawnAt_idx').on(table.poolId, table.withdrawnAt),
    index('PoolDeposit_userId_depositedAt_idx').on(table.userId, table.depositedAt),
  ]
);
...
export const poolPositions = pgTable(
  ...
  (table) => [
    ...
    index('PoolPosition_marketType_ticker_idx').on(table.marketType, table.ticker),
