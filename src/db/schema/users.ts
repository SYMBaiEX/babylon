...
export const users = pgTable(
  'User',
  {
    ...
    appealStakeAmount: decimal('appealStakeAmount', { precision: 18, scale: 2 }),
    ...
    autonomousCommenting: boolean('autonomousCommenting').notNull().default(false),
    autonomousDMs: boolean('autonomousDMs').notNull().default(false),
    autonomousGroupChats: boolean('autonomousGroupChats').notNull().default(false),
    ...
    agentMaxActionsPerTick: integer('agentMaxActionsPerTick').notNull().default(3),
  },
  (table) => [
...
export const profileUpdateLogs = pgTable(
  ...
  (table) => [
    index('ProfileUpdateLog_userId_createdAt_idx').on(table.userId, table.createdAt),
  ]
);
...
export const userInteractions = pgTable(
  ...
  (table) => [
    index('UserInteraction_npcId_timestamp_idx').on(table.npcId, table.timestamp),
    index('UserInteraction_userId_npcId_timestamp_idx').on(
      table.userId,
      table.npcId,
      table.timestamp
    ),
    index('UserInteraction_userId_timestamp_idx').on(table.userId, table.timestamp),
  ]
);
...
export const usersRelations = relations(users, ({ many, one }) => ({
  ...
  targetFavorites: many(favorites, { relationName: 'Favorite_targetUserIdToUser' }),
  userFavorites: many(favorites, { relationName: 'Favorite_userIdToUser' }),
  blockerBlocks: many(userBlocks, { relationName: 'UserBlock_blockerIdToUser' }),
  blockedBlocks: many(userBlocks, { relationName: 'UserBlock_blockedIdToUser' }),
  ...
  referrerReferrals: many(referrals, { relationName: 'Referral_referrerIdToUser' }),
...
export const userActorFollowsRelations = relations(userActorFollows, ({ one }) => ({
  user: one(users, {
