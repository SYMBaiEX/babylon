...
export const trajectories = pgTable(
  ...
  (table) => [
    index('trajectories_agentId_startTime_idx').on(table.agentId, table.startTime),
    ...
    index('trajectories_windowId_agentId_idx').on(table.windowId, table.agentId),
...
export const trainingBatches = pgTable(
  ...
  (table) => [
    ...
    index('training_batches_status_createdAt_idx').on(table.status, table.createdAt),
  ]
);
...
export const rewardJudgmentsRelations = relations(rewardJudgments, ({ one }) => ({
