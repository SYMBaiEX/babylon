...
export class Decimal {
  ...
  static add(a: Decimal | string | number, b: Decimal | string | number): Decimal {
    const aNum = typeof a === 'number' ? a : parseFloat(a.toString());
    const bNum = typeof b === 'number' ? b : parseFloat(b.toString());
...
export type WhereInput<T> = Partial<{
  [K in keyof T]: T[K] | { equals?: T[K]; not?: T[K]; in?: T[K][]; notIn?: T[K][]; lt?: T[K]; lte?: T[K]; gt?: T[K]; gte?: T[K]; contains?: string; startsWith?: string; endsWith?: string };
}> & {
...
export type IncludeInput = Record<string, boolean | { select?: Record<string, boolean>; include?: IncludeInput }>;
