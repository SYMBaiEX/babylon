...
import type { PredictionMarket, PerpMarket, FeedPost } from './decision.js'
...
export class BabylonA2AClient {
  ...
  async getPortfolio(): Promise<{ balance: number; positions: Array<Record<string, unknown>>; pnl: number }> {
    ...
    const positions = await this.sendRequest<{ perpPositions: Array<Record<string, unknown>>; totalPnL: number }>('a2a.getPositions', { userId: this.agentId })
   ...
   */
  async getMarkets(): Promise<{ predictions: PredictionMarket[]; perps: PerpMarket[] }> {
    const predictions = await this.sendRequest<{ predictions: PredictionMarket[] }>('a2a.getPredictions', { status: 'active' })
    const perps = await this.sendRequest<{ perpetuals: PerpMarket[] }>('a2a.getPerpetuals', {})
  ...
  async getFeed(limit = 20): Promise<{ posts: FeedPost[] }> {
    const feed = await this.sendRequest<{ posts: FeedPost[] }>('a2a.getFeed', { limit, offset: 0 })
