...
setup('extract auth tokens for integration tests', async ({ page }) => {
  ...
  await page.waitForFunction(
    () => {
      ...
      const privy = (window as { privy?: { ready?: boolean; getAccessToken?: () => Promise<string | null> } }).privy
      return privy?.ready === true && typeof privy.getAccessToken === 'function'
    },
    { timeout: 30000 }
  )
  ...
  const { accessToken, userId } = await page.evaluate(async (apiUrl) => {
    const privy = (window as { privy?: { getAccessToken?: () => Promise<string | null> } }).privy
    ...
    if (!token) {
      throw new Error('Could not get access token - user may not be authenticated')
    }
