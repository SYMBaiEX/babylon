...
describe('Follow System API Integration Tests', () => {
  let testUser1: { id: string; username: string; privyId: string }
  let testUser2: { id: string; username: string; privyId: string }
  ...
  describe('User-to-User Follow', () => {
    it('should create follow relationship', async () => {
      ...
      expect(follow.follower.username).toBe(testUser1.username)
      expect(follow.following.username).toBe(testUser2.username)
    ...
    it('should update counts after unfollow', async () => {
      ...
      expect(updatedUser1!._count.following).toBe(initialUser1!._count.following - 1)
      expect(updatedUser2!._count.followedBy).toBe(initialUser2!._count.followedBy - 1)
  ...
  describe('User-to-NPC Follow', () => {
    ...
    it('should update user following count after actor follow', async () => {
      ...
      expect(user!._count.userActorFollows).toBeGreaterThan(0)

      console.log(`✅ User actor follows: ${user!._count.userActorFollows}`)
    })
  ...
  describe('Follow Lists', () => {
    it('should retrieve followers list', async () => {
      ...
      expect(followers.some(f => f.follower.id === testUser1.id)).toBe(true)
    ...
    it('should retrieve following list', async () => {
      ...
      expect(following.some(f => f.following.id === testUser2.id)).toBe(true)
  ...
  describe('Profile API with Follow Counts', () => {
    it('should return correct follower/following counts in profile', async () => {
      ...
      expect(user2Profile!._count.followedBy).toBeGreaterThan(0)

      const totalFollowing = 
        user2Profile!._count.following + user2Profile!._count.userActorFollows

      console.log('✅ Profile counts:')
      console.log(`   Followers: ${user2Profile!._count.followedBy}`)
