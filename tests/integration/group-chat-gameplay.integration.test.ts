...
async function createTestUser(options: {
  ...
  username?: string;
  displayName?: string;
}): Promise<{ id: string; username: string; displayName: string; isAgent: boolean }> {
...
describe('Group Chat Gameplay Mechanics', () => {
  ...
  describe('Asymmetric Information Value', () => {
    test('group chats should provide alpha info not available on public feed', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'Alpha Trading Group', npcAdminId: npc.id });
      
      // Add participants
      await addChatParticipant({ chatId: chat.id, userId: npc.id });
      await addChatParticipant({ chatId: chat.id, userId: user.id, invitedBy: npc.id });
      await createGroupMembership({ chatId: chat.id, userId: user.id, npcAdminId: npc.id });
      
      // NPC posts candid alpha info
      await createMessage({
        chatId: chat.id,
        senderId: npc.id,
        content: 'Between us: METAI is about to announce a major partnership. I\'m loading up.',
      });
    ...
    test('kicked users should lose access to alpha info', async () => {
      ...
      const chat = await createTestGroupChat({ name: 'VIP Group', npcAdminId: npc.id });
      
      // Add participant
      await addChatParticipant({ chatId: chat.id, userId: npc.id });
      const participantId = await addChatParticipant({ chatId: chat.id, userId: user.id, invitedBy: npc.id });
      const membershipId = await createGroupMembership({ chatId: chat.id, userId: user.id, npcAdminId: npc.id });
      ...
      await db.chatParticipant.update({
        ...
        data: { isActive: false, kickedAt: new Date(), kickReason: 'Over-posting' },
      });
      await db.groupChatMembership.update({
        where: { id: membershipId },
        data: { isActive: false, removedAt: new Date(), sweepReason: 'Over-posting' },
      });
  ...
  describe('Participation Requirements', () => {
    ...
    test('posting too much should trigger exponential kick probability', () => {
      ...
      const results: Array<{ messages: number; prob: number; category: string }> = [];
      ...
      for (const msgCount of [16, 20, 25, 30, 35]) {
        ...
        results.push({ messages: msgCount, prob: result.probability, category: result.category });
      }
      
      // Probability should increase as messages increase
      for (let i = 1; i < results.length; i++) {
        expect(results[i]?.prob).toBeGreaterThanOrEqual(results[i-1]?.prob ?? 0);
      }
    ...
    test('not posting at all should eventually lead to removal', () => {
      ...
      expect(result.probability).toBe(0.90); // 90% kick probability
    });
  ...
  describe('Dynamic Threshold Scaling', () => {
    test('thresholds should scale with group size', () => {
      ...
      const smallGroupResult = NPCGroupDynamicsService.calculateKickProbability(15, 50, 5, 7);
      
      // Large group: 50 participants with 500 messages
      // Fair share = 10 messages
      const largeGroup = NPCGroupDynamicsService.calculateKickProbability(15, 500, 50, 7);
    ...
    test('thresholds should scale with group activity', () => {
      ...
      const activeGroup = NPCGroupDynamicsService.calculateKickProbability(25, 200, 10, 7);
      
      // Quiet group: 30 messages from 10 participants
      // Fair share = 3 messages
      const quietGroup = NPCGroupDynamicsService.calculateKickProbability(8, 30, 10, 7);
  ...
  describe('User vs Agent Parity', () => {
    test('users and agents should have identical kick probabilities for same behavior', async () => {
      ...
      for (const scenario of scenarios) {
        const userProb = NPCGroupDynamicsService.calculateKickProbability(
          scenario.messages, scenario.total, scenario.participants, 7
        );
    ...
    test('both users and agents can be added to group chats', async () => {
      ...
      const user = await createTestUser({ isAgent: false, displayName: 'Human User' });
      const agent = await createTestUser({ isAgent: true, displayName: 'AI Agent' });
      const chat = await createTestGroupChat({ name: 'Mixed Group', npcAdminId: npc.id });
      ...
      await createGroupMembership({ chatId: chat.id, userId: agent.id, npcAdminId: npc.id });
  ...
  describe('Sweep Mechanics', () => {
    test('sweep should calculate kick chance based on membership activity', async () => {
      ...
      const inactiveUser = await createTestUser({ displayName: 'Inactive User' });
      const activeUser = await createTestUser({ displayName: 'Active User' });
      const chat = await createTestGroupChat({ name: 'Sweep Test Group', npcAdminId: npc.id });
      
      // Add participants
      await addChatParticipant({ chatId: chat.id, userId: npc.id });
      await addChatParticipant({ chatId: chat.id, userId: inactiveUser.id, invitedBy: npc.id });
      await addChatParticipant({ chatId: chat.id, userId: activeUser.id, invitedBy: npc.id });
      ...
      await createMessage({ chatId: chat.id, senderId: activeUser.id, content: 'Hello everyone!' });
      await createMessage({ chatId: chat.id, senderId: npc.id, content: 'Welcome!' });
      
      // Calculate kick chances
      const inactiveDecision = await GroupChatSweep.calculateKickChance(inactiveUser.id, chat.id);
      const activeDecision = await GroupChatSweep.calculateKickChance(activeUser.id, chat.id);
      
      // Inactive user should have higher kick chance
      expect(inactiveDecision.kickChance).toBeGreaterThan(activeDecision.kickChance);
  ...
  describe('Group Limits', () => {
    test('users should be limited in how many groups they can join', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const chat = await createTestGroupChat({ name: `Group ${i + 1}`, npcAdminId: npc.id });
        await addChatParticipant({ chatId: chat.id, userId: npc.id });
        await addChatParticipant({ chatId: chat.id, userId: user.id, invitedBy: npc.id });
        await createGroupMembership({ chatId: chat.id, userId: user.id, npcAdminId: npc.id });
      }
...
describe('Benchmark/Simulation Group Chat Integration', () => {
  test('simulation data generator includes group chat events', async () => {
    ...
    const { BenchmarkDataGenerator } = await import('@/lib/benchmark/BenchmarkDataGenerator');
  ...
  test('simulation interface should handle group chat queries', async () => {
    ...
    const { BenchmarkDataGenerator } = await import('@/lib/benchmark/BenchmarkDataGenerator');
    const { SimulationEngine } = await import('@/lib/benchmark/SimulationEngine');
    const { SimulationA2AInterface } = await import('@/lib/benchmark/SimulationA2AInterface');
    ...
    const response = await interface_.sendRequest('a2a.getChats') as { chats: unknown[] };
