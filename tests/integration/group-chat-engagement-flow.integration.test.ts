...
async function createTestNPC(name: string): Promise<{ id: string; name: string }> {
  const id = await generateSnowflakeId();
  ...
  await db.actor.create({
    data: {
      id,
      name,
      description: 'Test NPC for engagement testing',
      domain: ['test'],
      isTest: true,
      updatedAt: new Date(),
    },
...
async function createFollow(followerId: string, followingId: string): Promise<string> {
  const id = await generateSnowflakeId();
...
async function createComment(userId: string, npcPostId: string, content: string): Promise<string> {
  const id = await generateSnowflakeId();
...
async function createGroupChat(name: string, npcAdminId: string): Promise<string> {
  const id = await generateSnowflakeId();
...
async function cleanupTestData(): Promise<void> {
  ...
  if (testIds.interactionIds.length > 0) {
    await db.userInteraction.deleteMany({ where: { id: { in: testIds.interactionIds } } });
  }
  if (testIds.shareIds.length > 0) {
    await db.share.deleteMany({ where: { id: { in: testIds.shareIds } } });
  }
  if (testIds.reactionIds.length > 0) {
    await db.reaction.deleteMany({ where: { id: { in: testIds.reactionIds } } });
  }
...
describe('Reply Guy Score Calculation', () => {
  ...
  test('should calculate score based on follows', async () => {
    ...
    // Calculate score
    const { score, breakdown } = await NPCGroupDynamicsService.calculateReplyGuyScore(
      user.id,
      [npc.id]
    );
...
describe('Engagement Tracker Integration', () => {
  ...
  test('should track user engagement score with NPC', async () => {
    ...
    await recordUserInteraction(user.id, npc.id, post2, comment2, 0.90);
    await recordUserInteraction(user.id, npc.id, post3, comment3, 0.80);
  ...
  test('should identify top engaged users', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      const comment = await createComment(user1.id, posts[i]!, `User1 comment ${i}`);
      await recordUserInteraction(user1.id, npc.id, posts[i]!, comment, 0.9);
    }
    
    // User 2: Medium engagement (5 comments)
    for (let i = 10; i < 15; i++) {
      const comment = await createComment(user2.id, posts[i]!, `User2 comment ${i}`);
...
describe('Users vs Agents Engagement Parity', () => {
  ...
  test('agents should earn same engagement score as users for same behavior', async () => {
    ...
    const humanUser = await createTestUser({ isAgent: false, displayName: 'Human' });
    const aiAgent = await createTestUser({ isAgent: true, displayName: 'AI Agent' });
