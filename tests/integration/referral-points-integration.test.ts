...
import { db, users, referrals, pointsTransactions, eq, and, isNull, count } from '@/db';
...
async function cleanupTestData() {
  ...
  for (const userId of testUserIds) {
    await db.delete(pointsTransactions).where(eq(pointsTransactions.userId, userId));
  }
...
describe('Referral Points Integration Tests', () => {
  ...
  describe('Basic Referral Flow', () => {
    ...
    it('should award REFERRAL_SIGNUP points to referrer when new user signs up', async () => {
      ...
      const [referrerBefore] = await db.select({
      ...
      const result = await PointsService.awardReferralSignup(referrerId, referredUserId);
      ...
      const [referrerAfter] = await db.select({
        reputationPoints: users.reputationPoints,
        invitePoints: users.invitePoints,
      })
        .from(users)
      ...
      expect(referrerAfter?.invitePoints).toBe(invitePointsBefore + POINTS.REFERRAL_SIGNUP);
    });
    ...
    it('should award REFERRAL_BONUS points to new user who used referral code', async () => {
      ...
      const [referredBefore] = await db.select({
        reputationPoints: users.reputationPoints,
  ...
  describe('Unqualified Referral Limit', () => {
    it('should enforce limit of 10 unqualified referrals', async () => {
      ...
      const result = await PointsService.awardReferralSignup(referrerId, eleventhUserId);
  ...
  describe('Referral Qualification', () => {
    it('should qualify referral when referred user links social account', async () => {
      ...
      const [referral] = await db.select({ qualifiedAt: referrals.qualifiedAt })
        .from(referrals)
      ...
      expect(referrerAfter?.reputationPoints).toBe(pointsBefore + POINTS.REFERRAL_QUALIFIED);
    });
  ...
  describe('Points Tracking', () => {
    ...
    it('should increment referralCount on user record', async () => {
      ...
      const [userBefore] = await db.select({ referralCount: users.referralCount })
        .from(users)
        .where(eq(users.id, referrerId));
      ...
      const [userAfter] = await db.select({ referralCount: users.referralCount })
        .from(users)
        .where(eq(users.id, referrerId));
