...
describe('Group Chat Dynamics Integration Tests', () => {
  ...
  describe('User and Agent Parity', () => {
    test('users and agents should both be eligible for group chat invites', async () => {
      ...
      const user = await createTestUser({ isAgent: false, displayName: 'Regular User' });
      ...
      // Create a group chat
      const chat = await createTestGroupChat({ name: 'Test Group', npcAdminId: npc.id });
    ...
    test('kick logic should treat users and agents the same', async () => {
      ...
      const userProb = NPCGroupDynamicsService.calculateKickProbability(0, 100, 10, 7);
      const agentProb = NPCGroupDynamicsService.calculateKickProbability(0, 100, 10, 7);
  ...
  describe('Group Chat Sweep', () => {
    test('sweep should calculate kick chance based on activity', async () => {
      ...
      const decision = await GroupChatSweep.calculateKickChance(user.id, chat.id);
  ...
  describe('Dynamic Thresholds', () => {
    test('small groups should have appropriate thresholds', () => {
      ...
      const small = NPCGroupDynamicsService.calculateKickProbability(8, 30, 3, 7);
    ...
    test('large groups should have appropriate thresholds', () => {
      ...
      const large = NPCGroupDynamicsService.calculateKickProbability(8, 500, 50, 7);
    ...
    test('domination threshold should scale with group size', () => {
      ...
      const smallDominator = NPCGroupDynamicsService.calculateKickProbability(25, 30, 3, 7);
      
      // In 50-person group with 500 messages, 15 messages = 3% (totally fine)
      // Fair share = 10, ideal max = 15, so 15 is exactly at max = safe
      const largeSafe = NPCGroupDynamicsService.calculateKickProbability(15, 500, 50, 7);
  ...
  describe('Message Cadence Simulation', () => {
    ...
    test('gradual increase in posting should be caught before spam', () => {
      ...
      for (const messages of progression) {
        ...
        expect(result.probability).toBeGreaterThanOrEqual(lastProbability - 0.1);
        lastProbability = result.probability;
      }
      
      // By 40 messages, should definitely be flagged
      const finalResult = NPCGroupDynamicsService.calculateKickProbability(40, 100, 10, 7);
  ...
  describe('Edge Cases', () => {
    test('should handle brand new group with no messages', () => {
      ...
      const result = NPCGroupDynamicsService.calculateKickProbability(0, 0, 5, 7);
    ...
    test('should handle single participant groups', () => {
      ...
      const result = NPCGroupDynamicsService.calculateKickProbability(50, 50, 1, 7);
...
describe('Group Chat Information Access', () => {
  ...
  test('participants should be able to see group messages', async () => {
    ...
    const chat = await createTestGroupChat({ name: 'Alpha Group', npcAdminId: npc.id });
  ...
  test('kicked users should not be able to see new messages', async () => {
    ...
    const chat = await createTestGroupChat({ name: 'Exclusive Group', npcAdminId: npc.id });
