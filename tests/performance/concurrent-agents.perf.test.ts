...
describe('Concurrent Agent Performance Tests', () => {
  ...
  describe('Registration Performance', () => {
    it('should handle 10 concurrent registrations within threshold', async () => {
      const agentCount = 10
      const agents = Array.from({ length: agentCount }, (_, i) => generateTestAgent(i))

      const { duration } = await measureTime(async () => {
        const registrations = agents.map(async (agent) => {
          // Simulate registration via API
          const response = await fetch('http://localhost:3000/api/agents/external/register', {
            method: 'POST',
      ...
      console.log(`Registered ${agentCount} agents in ${duration.toFixed(2)}ms`)
    ...
    it('should handle 50 concurrent registrations without errors', async () => {
      const agentCount = 50
      const agents = Array.from({ length: agentCount }, (_, i) => generateTestAgent(100 + i))
      ...
      console.log(`Registered ${successCount}/${agentCount} agents in ${duration.toFixed(2)}ms`)
  ...
  describe('Message Delivery Performance', () => {
    it('should handle 100 concurrent messages within threshold', async () => {
      ...
      expect(avgTimePerMessage).toBeLessThan(PERF_THRESHOLDS.maxMessageDeliveryTime)
    ...
    it('should handle concurrent discovery queries', async () => {
      ...
      const { duration } = await measureTime(async () => {
        const queries = Array.from({ length: queryCount }, (_, i) =>
          fetch(`http://localhost:3000/api/agents/external/discover?limit=10&offset=${i * 10}`, {
            method: 'GET',
            headers: {
      ...
      console.log(`Executed ${queryCount} discovery queries in ${duration.toFixed(2)}ms`)
  ...
  describe('Trust Calculation Performance', () => {
    it('should calculate trust scores efficiently', async () => {
      ...
      console.log(`Calculated ${connectionCount} trust scores in ${duration.toFixed(2)}ms`)
      console.log(`Average time per calculation: ${avgTimePerCalculation.toFixed(2)}ms`)

      expect(avgTimePerCalculation).toBeLessThan(PERF_THRESHOLDS.maxTrustCalculationTime)
    })
  })
  ...
  describe('Message Routing Performance', () => {
    it('should route messages efficiently across protocols', async () => {
      ...
      const protocols: Array<'a2a' | 'mcp' | 'agent0' | 'custom'> = ['a2a', 'mcp', 'agent0', 'custom']
      ...
      console.log(`Routed ${messageCount} messages across protocols in ${duration.toFixed(2)}ms`)
  ...
  describe('Memory and Resource Usage', () => {
    it('should handle large message history without memory leaks', async () => {
      ...
      console.log(`Memory after cleanup: ${(clearedMemory / 1024 / 1024).toFixed(2)} MB`)
    })
  })
})
