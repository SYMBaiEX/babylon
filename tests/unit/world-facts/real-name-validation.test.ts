...
describe('Real Name Validation System', () => {
  ...
  describe('Database Forbidden Names List', () => {
    ...
    it('should detect names from database list', () => {
      ...
      testNames.forEach(name => {
        const violations = validateNoRealNames(`Today ${name} announced something`);
        expect(violations.length).toBeGreaterThan(0);
      });
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle text with no names', () => {
      const violations = validateNoRealNames('This is just regular text about AI and technology.');
      expect(violations.length).toBe(0);
    });
  ...
  describe('Mixed Content', () => {
    ...
    it('should allow parody names mixed with regular text', () => {
      const text = 'OpenAGI and MetAI partnership with AIlon Musk announced today';
      const violations = validateNoRealNames(text);
      expect(violations.length).toBe(0);
    });
  });
