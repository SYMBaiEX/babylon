 ...
 */
export async function waitForServerHealthy(maxRetries = 5, retryDelay = 2000): Promise<void> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`⚠️ Server health check failed (attempt ${attempt}/${maxRetries}): ${response.status}`)
    } catch (error) {
      console.log(`⚠️ Server health check error (attempt ${attempt}/${maxRetries}): ${error instanceof Error ? error.message : String(error)}`)
    }
