...
export const babylonEnvSchema = z.object({
  ...
  BABYLON_MAX_TRADE_SIZE: z.string()
    .optional()
    .default('100')
    .transform(val => parseInt(val, 10))
    .pipe(z.number().positive()),

  BABYLON_MAX_POSITION_SIZE: z.string()
    .optional()
    .default('500')
    .transform(val => parseInt(val, 10))
    .pipe(z.number().positive()),

  BABYLON_MIN_CONFIDENCE: z.string()
    .optional()
    .default('0.6')
 ...
 */
export function validateBabylonEnvironment(env: Record<string, string | undefined>): {
  success: boolean;
...
export const optionalEnvVars = {
  BABYLON_AGENT_ID: 'Agent identifier for authentication (default: babylon-agent-default)',
  ...
  BABYLON_MAX_POSITION_SIZE: 'Maximum total position size in USD (default: 500)',
  BABYLON_MIN_CONFIDENCE: 'Minimum confidence threshold for trades (default: 0.6)',
} as const;
