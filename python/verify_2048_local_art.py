# Use the local ART codebase instead of the installed package
import sys
import os
sys.path.insert(0, os.path.abspath("ART/src"))
...
# Force re-import of art from local path
import art
import importlib
importlib.reload(art)
print(f"ART location: {art.__file__}")
...
def render_board(game: TwentyFortyEightGame) -> str:
    ...
    max_cell_width = max(
        [len(str(cell)) for row in board for cell in row if cell is not None] or [1]
    )
...
def max_cell_value(game: TwentyFortyEightGame) -> int:
    return max([cell for row in game["board"] for cell in row if cell is not None] or [0])
...
async def main():
    ...
    print("Registering model with local ART...")
    try:
